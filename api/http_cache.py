...
- **하지 않는다**: `Cache-Control: max-age=...`를 임의로 정하지 않는다. 그것은 "사용자가
  며칠 지난 사진을 봐도 되는가"라는 **제품 판단**이다. 정해지면 이 모듈에 한 줄로 붙는다.

  ★ 2026-08-18 Sprint 211 정정 — 위 괄호에 있던 "`document_version_log`가 **구조적으로
  도달 불가**"는 더 이상 사실이 아니다. Sprint 189 의 `requeue_changed_documents()` 가
  `overwrite=True` 경로를 실제로 열었고, 해시가 달라지면 개정 이력이 남는 것까지
  fixture 로 확인돼 있다(`test_asset_pipeline.py` "지문이 다르면 개정 이력이 남는다").

  다만 **아직 한 번도 돌지 않았다** — 예약 작업이 0개라 재수집 자체가 실행된 적이 없다
  (BUGS #123). 그래서 `max-age` 를 정할 **운영 근거(실제 갱신 주기)가 없다**는 결론은
  그대로다. 바뀐 것은 이유다: "경로가 없다"가 아니라 "아직 안 돌았다".
