...
def run_report():
    ...
    if tasks is None:
        ...
        if missing:
            ...
            # ★ 이것을 곧바로 "운영 크롤이 죽었다"로 읽지 않는다 (2026-08-25, BUGS #200).
            #
            #   DOJOONPASS 는 머신을 역할로 나눈다 - 운영 Daily Crawl 은 별도의 운영
            #   머신이 돌리고, 이 저장소로 개발/QA 를 하는 머신은 크롤을 돌리지 않는다.
            #   그 머신에 예약 작업이 없는 것은 **정상**이다.
            #
            #   이 도구는 **이 머신**만 볼 수 있다. 다른 머신의 스케줄러 상태는
            #   여기서 알 수 없으므로 추측하지 않는다.
            print()
            print("    ※ 이것은 **이 머신**의 상태다. 이 저장소는 개발 머신과 운영")
            print("       크롤 머신이 다를 수 있고, 개발 머신에 예약 작업이 없는 것은")
            print("       정상이다. '운영 크롤이 멈췄다'는 결론은 **운영 머신에서**")
            print("       확인해야 한다 - 여기서는 알 수 없다 (docs/BUGS.md #200).")
