...
def test_calc_priority():
    ...
    #
    #   ── 2026-09-01 추가: "일관되지 않다는 근거는 없다"는 이제 사실이 아니다 ──────
    #
    #     Sprint 273 이 `src/lib/format.ts:formatDday()` 를 만들면서, **같은
    #     `auction_date` 에 대해 달력 일수**를 세는 두 번째 정의가 생겼다
    #     (`daysBetween(todayInDisplayZone(), auction_date)` — 날짜끼리 뺀다).
    #
    #         달력 4일 뒤 물건    화면 배지        "입찰 4일전"
    #                             calc_priority   days_left=3 -> 최우선(1)
    #
    #     즉 "며칠 남았나"에 제품이 두 가지로 답한다. 실측(2026-09-01):
    #     달력 4일 뒤와 8일 뒤에서 두 정의가 각각 한 칸씩 갈린다.
    #
    #     그래도 **여기서 고치지 않는다.** 배지는 사용자에게 보이는 문구고 이 값은
    #     큐 정렬용 내부 등급이라, 같아야 할 이유가 자동으로 성립하지는 않는다.
    #     3일/7일 등급의 경계를 옮기는 것은 "어떤 물건을 먼저 받을 것인가"라는
    #     제품 결정이다(docs/CLAUDE.md 의 "모르면 질문한다"). 판단에 필요한 근거만
    #     여기 남긴다 — 고칠 때는 아래 경계 검사 네 개도 함께 옮겨야 한다.
