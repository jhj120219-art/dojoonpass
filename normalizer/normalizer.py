...
def normalize_case_no(case_no: str) -> str:
    """**크롤 원천이 준 사건번호**를 저장 표기로 만든다 — 양끝 공백만 턴다.

    ## 이름이 같은 함수가 하나 더 있다 (2026-09-02 Frankenstein 감사)

    `normalizer/mylist_import.py` 에도 `normalize_case_no()` 가 있다. **같은 패키지,
    같은 이름, 다른 규칙**이라 잘못 가져다 쓰기 쉬운 자리다. 둘은 하는 일이 다르다.

        이 함수 (크롤 경로)        원천을 **믿는다**. 법원이 준 글자를 보존한다.
        mylist_import (가져오기)   사람이 붙여 넣은 잡음에서 **뽑아낸다**.

    합치면 안 되는 이유가 실측으로 있다 (2026-09-02):

        입력                    이 함수            mylist_import
        '2024타채1009'          '2024타채1009'     ''          <- 타경이 아니면 버린다
        '2024타경1009-1'        '2024타경1009-1'   '2024타경1009'  <- 물건번호를 떼어낸다
        '사건번호 없음'           '사건번호 없음'      ''
        '2024 타경 1009'        '2024 타경 1009'   '2024타경1009'

    즉 이 함수를 저쪽으로 위임하면 **타경이 아닌 사건부호(타채 등)의 사건번호가
    통째로 빈 문자열이 된다.** 크롤 원천에는 그런 값이 실제로 올 수 있다.
    반대로 저쪽을 이 함수로 바꾸면 사용자가 친 잡음을 못 걸러낸다.

    ## 그래서 무엇을 고정하는가

    합치지 않는 대신 **정상 표기에서는 둘이 반드시 같은 답을 낸다**는 것을 검사로
    묶는다(`test_normalizer.py`). 위 표의 차이도 함께 고정한다 — 어느 한쪽이 조용히
    바뀌어 식별자가 갈라지면 그 검사가 먼저 운다.
    (`case_no_parts` 가 `crawler/resume.py` 와 짝을 이루는 방식과 같은 관례다.)

    실측(2026-09-02, 이 머신 auction.db): distinct case_no 1,381개 전부에서 두 함수의
    결과가 **동일**하다. 지금 갈라져 있지는 않다.
    """
