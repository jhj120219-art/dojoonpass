...
> **[정정 2026-08-18 Sprint 211] "구조적으로 도달 불가"는 더 이상 사실이 아니다.**
> 원문은 기록으로 남긴다 — 그 시점에는 정확했다.
>
> Sprint 189 의 `requeue_changed_documents()` 가 `done -> refresh` 로 큐를 되돌리면서
> `overwrite=True` 경로가 실제로 열렸다. 해시가 달라지면 개정 이력이 남는 것까지
> fixture 로 확인돼 있다(`test_asset_pipeline.py` "지문이 다르면 개정 이력이 남는다").
>
> **바뀐 것은 이유다.** 지금 0행인 것은 "경로가 없어서"가 아니라
> **"아직 한 번도 돌지 않아서"**다 — 예약 작업이 0개다(BUGS #123).
> `Cache-Control` 을 정할 운영 근거(실제 갱신 주기)가 없다는 결론 자체는 그대로다.
