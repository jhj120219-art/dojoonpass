...
탭 타깃 44px 미만 44/53 (83%)      그중 24px 미만 5개 = WCAG 2.5.8 AA 위반
...
```
1. 헤더 내비 탭 타깃 h>=44px       WCAG 위반이라 논쟁 여지가 가장 적다
...
API 응답에 필드를 더하고 카드 레이아웃을 바꾸는 일이라 **제품 디자인 결정**이다.
(썸네일 한 장이 약 101 KB 라 목록 두 곳에 더 붙이면 트래픽도 늘어난다 —
서버 측 썸네일 생성이 아직 승인 영역인 것과 맞물린다.)

### 구현 경로 (이미 있다)

```
API   search.py 의  SELECT item_id, MIN(seq) ... WHERE item_id IN (...) GROUP BY item_id
      배치 조회를 favorites.py / recent_items.py 에 그대로 옮긴다 (N+1 없음)
화면  src/app/search/ResultThumbnail.tsx 를 그대로 재사용
```

### 그동안 잠근 것

`test_search.py` 가 규칙을 건다 — **어느 화면이든 `thumbnail_url` 을 그리기 시작하면
그 화면의 API 도 그것을 주어야 한다.** 한쪽만 바뀌면 즉시 걸린다(변이로 확인).

### 함께 확인한 것

목록 성격 화면 넷(검색목록·상세·관심물건·최근본물건)의 데이터 계약 **어긋남 0**.
관심물건/최근본물건은 `SELECT ai.*` 라 계약이 **스키마에 묶여 있다** —
컬럼 이름이 바뀌면 화면이 조용히 빈칸이 되므로 `auction_item` 실제 컬럼과 대조한다.
