...
## 향후 기능 1번 "마이리스트 복붙/내보내기"

> **2026-08-28 Sprint 270 갱신 — 두 방향 모두 완료.**
>
> ```
> 내보내기  ✅ Sprint 227  형식 중립 CSV / 탭 구분 클립보드
> 가져오기  ✅ Sprint 270  붙여넣기 -> 파싱 -> 매칭 -> 사용자 확인 -> 담기
> ```
>
> 아래 분석이 "만들 수 없다"고 결론지은 것은 **내보내기의 상대 서비스 전용 포맷**
> 하나뿐이다. 그것은 지금도 만들지 않는다(형식 미확인). **가져오기는 방향이 반대라
> 그 제약을 받지 않는다** — 우리가 상대 형식을 맞출 필요가 없고, 사람이 이미
> 클립보드에 담아 온 것을 읽기만 하면 된다. 아래가 실측한 세 가지 함정
> (병합 사건 22.7% / 물건번호 33.5% / 법원명 60종)은 **가져오기 쪽에서도 그대로
> 지켰다** — 쪼개지 않고, 고르지 않고, 표기 차이로 "없음"이라 말하지 않는다.
> 상세는 `docs/SPRINT270_MYLIST_IMPORT.md`.
>
> "선행 조건: `favorites` 0행" 도 이 기능이 **뒤집는다** — 가져오기는 목록이
> 비어 있을 때 가장 필요하다(`/favorites` 빈 상태에 진입점을 둔 이유).

### (원본) 구현 가능 범위 분석 (2026-08-19)
