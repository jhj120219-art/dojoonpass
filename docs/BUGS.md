...
> **번호 중복 주의 (2026-09-01 확인).** `#254` 는 서로 다른 두 건에 부여돼 있다 — 이 항목(큐 `SKIPPED_EXPIRED`, 2026-08-27 해결)과, 아래 "공개 엔드포인트가 임차인 **실명**을 내보낸다"(2026-08-28, 승인 대기)다.
> 코드·테스트가 **양쪽 다** `#254` 로 참조하고 있어(`storage/database.py`/`test_document_queue.py` 는 이 건, `api/v1/item.py`/`test_public_endpoint_exposure.py` 는 저 건) 재번호는 참조 25곳을 함께 옮겨야 하므로 여기서 하지 않았다. 어느 쪽을 옮길지는 승인 영역이다.
...
**★ 2026-08-31 정정 — 이 저장소에 그 두 파일은 없었다.**
전수 확인: 작업 디렉터리에도 `git log --all` 에도 두 이름이 없다(언급하는 곳은 이 문단뿐).
즉 위 문단이 근거로 든 검사가 **존재하지 않는 상태로 "해결"로 남아 있었다** —
이 저장소가 반복해 잡아 온 "공허한 보증"이다. 실제로 고정돼 있던 것은
행 단위 격리 하나뿐이었다(`test_auction_identity.py` §3 — 그것은 실재한다).

**다시 해결 (2026-08-31).** `test_db_write_failure_modes.py` 를 **실제로 만들었다**
(6절 / 단언 29). BUGS 의 서술을 베끼지 않고 스크래치 DB 에서 각 거동을 먼저 재고
잰 값으로 단언을 세웠다. 파일 두 벌로 나누지 않은 이유: 큐 claim 배타성도 같은
"DB 쓰기 장애 거동"이라 6절에 넣었다 — 목록이 두 벌이면 갈라진다(#204).
...
행 단위 격리는 여기서 다시 만들지 않았다(위 `test_auction_identity.py` §3 과 중복).
변이 3/3 검출(행마다 커밋 / 커밋 실패 삼키기 / claim CAS 의 상태 조건 제거).

★ 출발 배리어를 둔 이유를 적어 둔다 — 첫 실행에서 한 프로세스가 40행을 통째로
비우고 다른 하나가 빈 큐를 봤다(40:0). 동시성을 재려던 검사가 순차 실행을 재고 있었고,
"중복 0"은 그 상태에서도 통과한다. 파일 신호로 둘을 함께 출발시켜 실제로 경합시킨다.
...
> **번호 중복 주의 (2026-09-01 확인).** 위 `#254`(큐 `SKIPPED_EXPIRED`, 2026-08-27 해결)와 번호가 겹친다. 자세한 사정은 그쪽 항목의 같은 주석을 참고한다.
...
**[2026-09-01 재측정 — 노출 범위가 3배 가까이 늘었다]** 위 숫자는 2026-08-28 것이다. 결정이 미뤄지는 동안 크롤이 계속 돌아 대상이 늘었다.

```
                  2026-08-28      2026-09-01      배수
tenant_rights          519행          1,521행       2.9x
occupied_area          475            1,439         3.0x
move_in_date           211              401         1.9x
deposit                125              254         2.0x
fixed_date              81              169         2.1x
monthly_rent            33               78         2.4x
tenant_name(실명)      240              318         1.3x
```

실명 318 은 `[가-힣]{2,4}` 에 맞고 '미상/불명/없음/해당없음/임차인' 이 아닌 값을 센 것이다 — 2026-08-28 의 240 이 어떤 기준으로 셌는지는 기록이 없어 **두 수를 같은 자로 잰 것으로 보지 않는다.** 다른 여섯 줄은 NULL/빈값이 아닌 행 수라 기준이 같다.

판단 자체는 그대로 승인 대기다. 이 재측정은 **결정을 미룰수록 대상이 는다**는 사실만 기록한다.
...
**검색폼의 매각기일 퀵버튼이 "오늘"을 UTC 로 만들어**, 한국 사용자가 오전에 누르면
**어제 날짜로 검색**됐다 (오늘 매각되는 물건이 통째로 안 나온다)

상태

**해결 (2026-09-01).** `src/lib/format.ts` 에 `ymdPlusDays()` 신설,
`SearchForm.tsx` / `ExportButtons.tsx` 가 `todayInDisplayZone()` 을 쓰도록 변경.
회귀: `tests/format.test.mjs` (+7), `tests/export-list.test.mjs` (+2),
`tests/source-contract.test.mjs` (+5, 금지 규칙).

### 실측 — 이 문장을 쓴 시각에 실제로 틀리고 있었다

```
실제 지금            Tue Sep 01 2026 08:48:47 GMT+0900 (한국 표준시)

고치기 전  "당일"     2026-08-31 ~ 2026-08-31      <- 어제
...
`toISOString()` 은 UTC 로 바꾸므로 KST 09:00 **이전에는 언제나 하루 전**이 나온다.
`setDate(+N)` 은 로컬로 더한 뒤 다시 UTC 로 잘리므로 끝 날짜만 경계를 도로 넘어가
"+7" 이 8일 범위가 된다.

### 왜 오래 안 보였나 — 빈 화면이 아니라 "그날은 물건이 없네"로 보인다

오류도 404 도 빈 상태 배너도 아니다. 어제 날짜로 정상 검색돼 **정상적으로 0건**이
나온다. `#17` 이 "404 도 없이 엉뚱한 물건이 열린다"였던 것과 같은 부류의 조용한 오답이고,
증상이 **KST 09:00 이전에만** 보여 낮에 재현하면 멀쩡하다.

### Sprint 273 이 D-day 를 고칠 때 왜 같이 안 잡혔나

Sprint 273 은 **보여 주는 쪽**(`formatDday`, 로케일)을 `DISPLAY_TIME_ZONE` 으로 고정했다.
이 결함은 **입력을 만드는 쪽**이라 그 그물에 걸리지 않았다. 새 정책을 만든 것이 아니라
그때 선언한 기준을 입력쪽에도 적용한 것이다 — 한국에서 보는 화면은 안 바뀐다.

### 검사가 처음엔 공허했다 (같이 기록한다)

금지 규칙을 처음 쓸 때 탐지기를 `new Date().toISOString().slice(` 로 잡았는데,
**실제 결함 코드는 `d.toISOString().slice(...)` 로 한 단계 건너서** 쓰고 있었다.
변이 주입(원래 코드를 되돌려 넣기)이 그것을 잡아, 수신자를 가리지 않는
`\.toISOString\(\)\s*\.slice\(` 로 고쳤다. **자르는 행위만** 금지하고
전체 ISO 직렬화는 허용한다 — 시각이 붙은 ISO 문자열은 시간대가 명시돼 모호하지 않다.

예외 목록을 두지 않으려고 `ymdPlusDays()` 자신도 `toISOString()` 을 쓰지 않고
UTC 자릿수를 직접 조립한다(Sprint 118: "예외 목록은 곧 두 번째 규약이 된다").

### 같은 부류로 함께 고친 것

`ExportButtons.tsx` 의 CSV 파일명(`관심물건_YYYY-MM-DD.csv`)도 같은 UTC 날짜였다.
오전에 내려받으면 어제 날짜가 붙은 파일이 저장된다.

--------

#271

**`audit_viewport.py` 가 "결함 0 / 종료코드 0" 을 찍으면서, 가장 넘치기 쉬운 화면
(목록 카드 · 상세)을 아예 안 재고 있었다** — 감사 도구 자신의 false success

상태

**해결 (2026-09-01). 회귀: `test_audit_selftests.py` 에 소스 계약 9검사 신설(변이 1/1 검출).**

### 어떻게 초록이 나왔나

```
검색 화면 경로   /search?sido=서울
이 DB           기일이 남은 물건 0건  -> 결과 카드 0장
재던 것          카드가 하나도 없는 빈 목록  -> "넘침 0" 은 당연하다
상세             표본 id 를 그 결과에서 뽑는다 -> "찾지 못했다 - 건너뛴다"
종료코드         0
```

Sprint 240 이 좁은 폭에서 찾아낸 결함 3종 중 **둘이 바로 그 카드**에 있었다
(목록 카드 grid 트랙 / 물건종류 배지). 그 화면을 안 재고 초록을 찍고 있었던 셈이다.

이것은 `docs/SPRINT98_FALSE_SUCCESS_AUDIT.md` 가 정리한 그 패턴이고,
`#253` 이 "영구히 붉은 게이트에서는 새 회귀와 아는 부채를 구별할 수 없다"고 적은 것의
**반대 방향**이다 — 영구히 초록인 게이트에서는 아무것도 구별할 수 없다.

### 고친 방식 — 데이터를 만들지 않고, 있는 데이터를 보이게 한다

카드가 0장이면 `include_closed=true` 로 한 번 더 시도한다. 이것은 UI 에도 있는 정식
검색 조건(SearchForm 의 "종결 포함")이라 새 경로를 뚫는 것이 아니다. 카드가 나온
조건으로 **검색 화면 자체를 바꿔 재고**, 상세 표본도 거기서 뽑는다.

어느 쪽에서도 0장이면 **정상이 아니라 측정 불가(종료코드 2)** 로 센다 —
이 도구가 자기 docstring 에 "서버가 없거나 브라우저를 못 띄우면 2, 0으로 뭉개지
않는다"고 적어 둔 규칙을 데이터 부재에도 똑같이 적용한 것이다.

### 실측 (2026-09-01, localhost:3000)
...
**결함 수는 그대로 0이지만 이제 그 0은 실제로 잰 0이다.** 320/360/390/430/900/1400px
여섯 폭에서 카드 20장을 그린 목록이 넘치지 않는 것을 확인했다.

### 남은 것 (승인 영역)

상세(`/properties/{id}`)와 `/favorites` `/properties/recent` `/mypage` 는 로그인이
필요해 헤드리스에서 `/login` 으로 튕긴다. 통과로 세지 않고 `AUTH` 로 따로 찍히므로
숨겨지지는 않는다. 재려면 진짜 세션 쿠키(`--cookie`)가 필요하고 그것은 계정 영역이다.

--------

#272

**두 문서가 이중 차감 방지 근거로 인용하는 `ADJUSTMENT_REASONS` 가
코드에서는 아무도 읽지 않아, 실제로는 아무것도 강제하지 않고 있었다**

상태

**해결 (2026-09-01) — 상수를 지우지 않고 검사가 읽게 해서 인용이 참이 되게 했다.
회귀: `test_registry_credits.py` §6 신설(8검사). 변이 3/3 검출.**

### 실측 — 참조가 정의 한 줄과 문서 둘뿐이었다

추적·미추적 전 파일형식(.py/.sql/.md/.ts/.tsx/.mjs/.bat/.ps1/.json)을 훑었다.

```
api/constants.py:121                          정의
storage/migrations/016_...sql:16              주석에서 인용
docs/CHANGELOG.md:981                         본문에서 인용
그 외 코드 참조                                **0건**
`from api.constants import *` / 동적 접근      0건
```

두 문서는 *"사용(USAGE)을 한도 계산에 넣으면 이중 차감이 된다 — 로그에만 남기고
계산에서는 뺀다(`ADJUSTMENT_REASONS`)"* 라고 적으면서 이 상수를 근거로 든다.
그런데 그 상수를 읽는 코드가 없었다.

### 게다가 상수의 서술 자체가 코드와 달랐다

```
ADJUSTMENT_REASONS 주석   "원장 합계에 실제로 반영되는 사유"
                          집합 = {GRANT, DEDUCT, RESET, EVENT, REFUND}   5종

get_credit_adjustment()   합계에 `reason_type` 조건이 **없다**. 그 달 행을 전부 더한다
                          (RESET 이후로 자르기만 한다)

VALID_REASON_TYPES        실제 게이트 = {GRANT, DEDUCT, RESET}            3종
                          `add_credit()` 과 `admin.py` 가 이것으로 막는다
```

즉 상수는 **존재하지 않는 필터**를, **들어올 수 없는 값(EVENT/REFUND)** 까지 포함해
서술하고 있었다. `registry_credits.py` 의 주석은 정반대로 "EVENT/REFUND 는 로그에만
남는다"고 적어, 두 상수의 서술이 서로 모순이었다.

### 진짜 위험 — 안전이 게이트 하나에 걸려 있다

지금 합계가 안전한 이유는 필터가 있어서가 아니라 **다른 사유가 그 테이블에 못 들어가서**다.
누가 `VALID_REASON_TYPES` 에 `USAGE` 를 더하면 그 순간:

```
add_credit(..., "USAGE", -1)   registry_credits 에 행이 생긴다
get_credit_adjustment()        거르지 않으므로 그 -1 을 합산한다
registry_usage                 같은 사용을 이미 세고 있다
결과                           **이중 차감** — 사용자가 손해를 본다
```

016 마이그레이션과 CHANGELOG 가 경고한 바로 그 상황이다.

### 기존 검사가 왜 못 잡았나

`test_credit_log_written_alongside_ledger()` 는 USAGE 를 `log_credit_event()` 로 직접
남겨 합계가 안 변하는 것을 확인한다. 그것은 **로그 경로**의 검증이라 `add_credit()` 을
부르지 않는다. 위 시나리오는 게이트가 넓어지는 것이므로 그 검사에 걸리지 않는다.
실측: 변이를 넣었을 때 기존 27개 단언이 **전부 통과**했다.

### 고친 방식 — 상수를 지우지 않고 살렸다

§5 의 죽은 코드 판정 기준으로는 삭제가 허용되는 상태였지만, 지우면 두 문서의 인용이
허공을 가리키게 된다. 대신 **검사가 그 상수를 읽게** 했다:

```
원장에 쓸 수 있는 사유 ⊆ 합계에 넣어도 되는 사유   (VALID_REASON_TYPES ⊆ ADJUSTMENT_REASONS)
USAGE 는 어느 쪽에도 없다
합계가 아직 사유로 거르지 않는다는 전제도 함께 고정
```

**동작은 바꾸지 않았다.** 합계에 `reason_type` 필터를 넣는 것은 과금 로직 변경이라
승인 영역이다. 여기서는 지금 성립하는 불변식만 고정한다.

### 남은 것 (승인 필요)

`ADJUSTMENT_REASONS` 의 EVENT/REFUND 가 맞는지, 합계에 실제 필터를 넣을지는
과금 정책 결정이다. 두 상수의 서술이 서로 모순인 상태도 그 결정과 함께 정리해야 한다.

--------

#273

**적용된 마이그레이션 3개의 파일이 사라져 저장소가 자기 스키마를 재현하지 못한다**

상태

**해결 (2026-09-02). 파일 3개를 라이브 결과에서 역복원. 인덱스 계약 검사를
DROP 까지 읽도록 확장. fresh↔운영 스키마 전수 대조 차이 0.**

### 증상

```
migration_history   31건        디스크 storage/migrations/*.sql   26개
   027_drop_redundant_favorite_notes_index.sql   2026-08-31T03:00:02 적용 기록
   028_auction_filed_date.sql                    2026-08-31T03:00:02 적용 기록
   029_drop_measured_prefix_indexes.sql          2026-08-31T03:00:02 적용 기록
   -> 셋 다 디스크에도 git 이력 어디에도 없다(git log --all -- ...027* = 0건)
```

새로 clone 해 `init_db -> migrate_v4_1 -> run_migrations` 하면 라이브와 다른 스키마가
나왔다 — 라이브에만 `auction.filed_date`, fresh 에만 인덱스 3개.

### 조치

라이브 DB 에 남은 **결과**에서 세 파일을 역복원했다. 029 가 지운 접두 인덱스가
정말 지워도 되는 것인지는 021 의 경고("접두 중복을 지우면 느려진다, sido 검색 +540%")
때문에 **다시 쟀다** — 합성 10만행에서 20% 넘게 느려진 쿼리 0개, DB 17.5% 축소.
021 이 문제 삼은 것은 범위/커버링 스캔이고 favorites/recent_items 는 점 조회 전용이라
그 경고가 적용되지 않는 자리였다(각 파일 머리말에 측정치).

`test_schema_hygiene.py` 의 "선언한 인덱스가 새 DB 에 있다" 검사는 `CREATE INDEX` 만
세고 있어 **일부러 지운 인덱스를 결손으로 오인**했다. DROP 도 함께 읽어 마지막 선언으로
판정하고, 반대 방향("지웠다고 한 인덱스가 되살아나지 않았는가")도 신설했다.

--------

#274

**문서를 권리분석으로 바꾸는 마지막 단계가 배선돼 있지 않다 (#245 의 배선 절반)**

상태

**해결 (2026-09-02). `run_doc_worker.bat` 에 배선. 보류 사유였던 DELETE 경로에
대량 삭제 차단기를 먼저 달았다.**

### 왜 미뤄져 있었나

#245 는 탐지만 넣고 배선을 미뤘다 — `load_rights_data.py` / `load_spec_data.py` 의
`purge_orphans()` 에 DELETE 경로가 있고, 안전장치가 `evidence_found == 0` 하나뿐이라
**부분 유실**(동기화 지연/드라이브 절반 마운트)에서는 안전장치가 열린 채 대부분을
지운다. 무인 야간 실행으로 그 일이 벌어지는 것이 위험했다.

### 조치 — 차단기를 먼저 달고 배선했다

`storage/database.py:guard_mass_purge()` — 한 번의 실행이 기존 파생 행의 20% 를 넘게
지우려 하면 거부하고 종료코드 1 을 돌려준다(바닥값 10건 미만은 통과). 규칙은 한 곳에만
두고 두 스크립트가 같이 부른다(#204 의 규칙 복제 금지).

배선 직전 실측 — 지금 지울 대상은 **0행**이었다(rights_summary 413 / tenant_rights 1,069).

```
[6] 파이프라인 마지막 단계          배선 전            배선 후
    2026-09-01  수집 9건 / 파싱     0건 (0%)          9건 (100%)
    STATUS=READY 인데 권리분석 없음  9건               0건
```

`audit_schedule_health.py` 가 배선 여부를 **.bat 을 읽어** 판정하도록 바꿨다 —
문장으로 박아 두면 배선이 풀렸을 때 다시 알려 줄 수 없다.

--------

#275

**최종 실패에 사유가 남지 않아 188건을 진단할 수 없다**

상태

**해결 (2026-09-02). `mark_queue_failed()` 가 사유를 기록한다. 회귀 8검사.**

### 증상 — 약속이 지켜지지 않고 있었다

`mark_queue_failed()` 의 주석은 큐 행에 아무것도 쓰지 않는 선택을 *"실패 사실은 로그와
`document_collect_failures` 에 이미 남는다"* 로 정당화한다. **그 표에 쓰는 코드가
없었다** — INSERT 는 `collect_documents.py` 뿐이고 그 스크립트는 2026-07-15 이후
돌지 않았다.
...
`_record_collect_failure()` 를 신설해 **최종 실패**(재시도 소진)에서만 사유를 남긴다.
`doc_worker.py` 의 4개 실패 경로가 각각 실제 사유를 넘긴다. 기록은 **최선 노력**이다 —
표가 없는 스키마에서 예외가 나가면 큐 상태 전이가 통째로 죽기 때문이다(실제로 한 번
그렇게 죽었다).

`reset_stale_queue()` 가 하루 지난 failed 를 되살리므로, 다음 실행에서 92건이
재시도되며 **사유가 채워진다.**
...
**감정평가서 뷰어가 확장자 없는 주소로 문서를 주는데 `.pdf` 만 인정하고 있었다
— appraisal 성공률 55.7% (다른 종류는 97~99%)**

상태

**해결 (2026-09-02). 판정에 뷰어 경로를 더했다. 회귀 22검사 신설.**

### 증상 — 종류별 성공률이 갈렸다 (2026-09-02 실측)

```
status  done 414 / failed  5   98.8%
spec    done 416 / failed 11   97.4%
image   done 263 / failed  6   97.8%
appraisal done 209 / failed 166  **55.7%**
```

### 원인 — #267 이 고치지 않고 남긴 절반

#267(2026-08-28)은 *"iframe 이 붙었지만 src 가 아직 없는"* 경합을 고쳤고, 판정 규칙은
*"글자 그대로 같다(`src.lower().endswith('.pdf')`)"* 며 그대로 두었다. 그런데 감정평가서
뷰어는 **확장자 없는 경로**로 문서를 준다. 2026-08-30 워커 로그에 실물이 남아 있다:
...
iframe 은 제때 붙었고 src 도 채워졌는데 **우리가 버렸다.** 이 경고는 로그에서 단일
사유 1위(600건대)였다.

### 조치

`_is_appraisal_document_src()` 로 판정을 떼어 내고 `.pdf`(기존 규칙 그대로) **또는**
`KAPANET_BASE` 호스트의 `/view/` 경로를 인정한다. 호스트를 넓히지 않는다 —
"iframe 이 하나뿐이면 그것"으로 두면 뷰어가 배너를 붙이는 날 엉뚱한 주소를 연다.

**안전성**: 이 함수는 주소를 고를 뿐이고, 저장은 `wait_for_download()` 가 `.pdf` 로
끝나는 새 파일이 도착했을 때만 한다. PDF 가 아니면 예전과 똑같이 아무것도 안 오고
실패한다 — 엉뚱한 것을 저장할 경로가 없다(Sprint 201 과 같은 "더할 뿐" 논리).

진단 근거를 잃지 않도록 **iframe 목록을 함께 찍는 로그**도 복원했다. 그 판본의 로그가
남아 있어서 원인을 찾았는데, 정작 그 코드가 저장소에 없었다(#273 과 같은 유실).
...
**재수집 상한을 넘어 잘린 물건은 "미뤄지는" 것이 아니라 영구히 잃는다.**
게다가 **매번 같은 앞쪽 물건만 뽑힌다** — 뒤쪽 물건에는 차례가 오지 않는다.

**해결 (2026-09-03). 상한을 생산 쪽에서 걷어냈다 — 큐가 그 의도를 보관한다.**

> 이 항목은 2026-08-30 홈 세션이 고쳤다가 08-31 store sync 에서 **유실**된 것을
> 다시 살린 것이다. 경위는 `docs/SPRINT285_CASE_DATE_PRODUCER.md` §2 참고.

### 증상

`storage/database.py:requeue_changed_documents()` 가 `changes[:60]` 으로 잘랐다.
그리고 로그에 이렇게 적었다.

```
재수집 대상 200건 중 상한(60)을 넘는 140건은 이번 실행에서 미룬다
(큐에 그대로 남아 다음 실행에서 다시 후보가 된다)
```

**뒷문장이 사실이 아니다.** 잘린 물건도 `auction_item` 은 그 실행에서 이미 갱신되므로
다음 실행의 변경 감지에 **다시 들어오지 않는다.**

### 재현 (2026-09-03, 스크래치 DB)

200물건의 최저가를 하루 만에 전부 바꿨다.

```
2일차  requeue {items:60, refreshed:60, skipped_over_cap:140}
       document_queue: done 740 / refresh 60
3일차  법원 자료가 2일차와 동일(정상적인 날)
       requeue {} — 변경 0건. 140물건은 다시 후보가 되지 않았다
```

즉 하루에 60물건 넘게 바뀌면 나머지는 **옛 문서로 굳는다.** 사용자는 최저가가 내려간
물건의 옛 매각물건명세서를 계속 본다. `SELECT * FROM auction` 순서라 매번 같은 앞쪽
물건만 뽑혀, 뒤쪽 물건에는 차례가 영영 오지 않는 것도 같은 결함의 일부다.

### 원인 — 상한이 **잘못된 계층**에 있었다

상한 60 의 근거는 워커가 하룻밤에 처리할 수 있는 양이다(창 7,200초 / 물건당 최악
4행 x 24초 -> 69물건). 그건 **소비**의 한계인데 **생산**(큐에 적는 일) 쪽에 걸려
있었다. 큐는 할 일을 담으려고 있는 것이라, 담기 전에 자르면 큐가 존재할 이유가 없다.

### 조치

기본 실행에서는 자르지 않는다. 소비 쪽이 이미 스스로 제한한다.

```
doc_worker   02:00~04:00 창(is_time_up()) — 시간이 되면 멈춘다
claim        ORDER BY priority ASC, auction_date ASC — 긴급도 순
기일 경과     SKIPPED_EXPIRED 가 정리한다
```

전부 적어 두면 오늘 밤은 급한 것부터 하고 나머지는 큐에 남아 다음 밤에 이어서 한다.
`max_items` 를 **명시적으로** 넘기면 여전히 조인다(검사·운영자 수동 실행용).
`REFRESH_MAX_ITEMS_PER_RUN` 은 이제 **보고용**이다 — "오늘 적은 양이 하룻밤 처리량을
넘는가"를 로그로 알려 준다.

### 검증

같은 재현에서 `{items:200, refreshed:200, skipped_over_cap:0}`.
`test_refresh_trigger.py`(808단언) 포함 파이썬 스위트 66파일 전부 통과.

--------

#285

**`auction_case` 의 사건 정보(사건종류 / 접수일 / 배당요구종기)에 생산자가 없었다** —
컬럼도 API 도 화면도 있는데 **채우는 코드만 없었다.**

**해결 (2026-09-03). 세 필드 모두 배선했다. 스키마 변경 없음. 새 크롤 경로 없음.**

> 이 항목도 2026-08-30 홈 세션이 고쳤다가 08-31 store sync 에서 유실된 것이다.
> 그때 남은 것은 `auction.filed_date` 컬럼(값 313행)뿐이었고, 매장 PC 가 그 흔적을
> 보고 `028_auction_filed_date.sql` 을 역으로 복원했다(그 파일이 "복원본"이라고
> 적고 있다). **컬럼은 살아남았고 채우는 코드는 못 살아남았다.**

### 계층별 실태 — 수집만 빠져 있었다

```
DB          auction_case.case_type / filed_date / demand_deadline   있다
API         api/v1/item.py:_CASE_FIELDS 가 그대로 내보낸다            있다
Frontend    properties/[id]/page.tsx 가 '사건종류/접수일/배당요구종기일' 표시  있다
생산자      **없다**  -> 화면에는 늘 `-`
```

`migrate_execute.py` 의 `auction_case` INSERT 가 컬럼을 나열만 하고 값은
`VALUES (?, ?, ?, NULL, NULL, NULL, ?, ?)` 였다.

### 원천은 서로 다르다 (2026-09-03 실측)

**접수일 — 상세페이지에 이미 있다.** 새 크롤이 필요하다는 이전 판단은 틀렸다.
`parse_basic_info()` 가 페이지의 모든 th/td 를 훑으므로 **이미 잡고 있었고**,
`AuctionItem.basic_info` 가 저장되지 않아 매일 버려지고 있었다.

```
서울중앙 / 수원 4물건 4/4
  사건접수 2008.08.26 / 2024.03.20 / 2024.10.14
  배당요구종기 2008.11.28 / 2024.06.04
  경매개시일 2008.08.27 / 2024.03.22
```

**사건종류·배당요구종기 — 이미 받아 둔 spec.pdf 1쪽에 있다.**

```
보유 spec.pdf 466개
  사건종류      457건 (98.1%)    분포: 임의경매 280 / 강제경매 177
  배당요구종기   444건 (95.3%)
  열기 실패       0건
```

1쪽만 읽는다 — 전 쪽을 훑어도 결과가 같고 spec.pdf 는 30쪽짜리도 있다.

### 조치

```
filed_date        normalize_item() 이 basic_info['사건접수'] 를 싣는다 (normalize_date 재사용)
                  -> auction.filed_date (028) -> migrate_execute -> auction_case
                  신규 사건은 INSERT 에, 기존 사건은 filed_date IS NULL 인 행만 보충

case_type         load_spec_data.py 를 확장했다(새 모듈 없음)
demand_deadline     parse_case_info()   순수 함수 — 검사가 직접 태운다
                    extract_case_info() spec.pdf 1쪽
                    update_case_info()  값이 같으면 안 쓴다, 못 읽은 필드는 COALESCE 보존
                  호출은 **임차인 경로보다 앞**에 둔다 — 임차인 표를 못 찾아 조기
                  반환하는 물건(실측 139건)도 사건 정보는 정상적으로 들어 있다
```

API 와 화면은 처음부터 이 필드들을 그리고 있었으므로 **계약을 하나도 바꾸지 않았다.**

### 접수일은 왜 `COALESCE` 가 아니라 `IS NULL` 인가

사건이 접수된 날은 한 번 정해지면 바뀌지 않는다. 그래서 값이 있으면 문장 자체를
만들지 않는다. **빈 문자열을 NULL 로 바꾸는 것이 그래서 중요하다** — `''` 가 들어가면
`IS NULL` 가드에 걸려 다시는 진짜 값으로 갱신되지 않는다(변이 M5 가 이것을 태운다).

### 검증 (2026-09-03)

```
filed_date        실크롤(인천지방법원) -> auction_case 217 -> 227, 형식 위반 0
                  API 왕복: GET /api/v1/item/13919 -> case.filed_date '2025-03-24'
                  (DB 값과 일치, 갓 크롤한 사건). 값 없는 사건은 null -> 화면 '-'
case_type         사본 DB 적재: 370 -> 411
demand_deadline   사본 DB 적재: 360 -> 399
                  재실행 멱등(완전 동일), 형식 위반 0
운영 DB           md5 b43c5638... 세션 전후 불변 (전부 사본에서 실행)
```

### 회귀

```
test_migrate_incremental.py §14     크롤 -> 정규화 -> auction -> auction_case 끝에서 끝까지
                                    (신규/보충/불변/빈문자열/문장수/028 미적용 DB)
test_rights_data_load.py            사건 정보 파서 / 갱신 멱등·보존 / **배선**
test_pipeline_integrity.py §15      세 컬럼을 producerless 목록에서 뺐다
```

★ 마지막 **배선 검사는 이번에 새로 만들었다.** 되살린 두 검사는 파서와 갱신자를 각각
직접 부르므로, `load_item()` 안의 호출 한 줄을 지워도 **둘 다 통과했다**(변이 N4 생존).
조각은 옳은데 이어져 있지 않으면 화면에는 아무것도 안 나온다 — `load_item()` 을 통째로
돌려 `auction_case` 까지 확인하도록 닫았다.

### 남은 것 (승인 영역)

`경매개시일` / `청구금액` 은 상세페이지에 함께 있지만 `auction_case` 에 컬럼이 없어
스키마 변경이 필요하다. 상세페이지의 `배당요구종기` 를 크롤에서도 받으면 문서 없는
사건까지 채워지지만 역시 `auction` 에 컬럼 추가가 필요하고, **연기 표기**를 먼저
정해야 한다(실측: `2025.05.16(연기)/2025.01.02` — 한 칸에 두 날짜).
...
**병합사건의 구성요소 **순서**가 바뀌면 같은 물건이 두 행이 된다.**

**부분 해결 (2026-09-03). 탐지·상한 고정까지. 근본 수정은 승인 영역.**

### 증상 (2026-09-03 실측, 실크롤 결과)

법원은 관련 사건을 묶어 한 줄로 공고하고, 크롤러는 그 줄의 사건번호를
`" / ".join(case_nos)` 로 이어 붙인다. 그 문자열이 식별키의 일부다
(`auction` 의 `UNIQUE(court_code, case_no, item_no)`).

**순서는 법원 페이지가 정한다.** 오늘 크롤에서 같은 물건이 어제와 다른 순서로 나왔다.
...
`auction_case`(1272/1273) 와 `auction_item`(178/8185) 에도 그대로 두 벌이 생겼다.

### 사용자 영향

같은 물건이 **검색 결과에 두 번** 나온다. 문서 큐도 두 벌 쌓이고, 관심물건/최근본이
어느 쪽에 걸리는지가 그날 크롤 순서에 좌우된다. 한쪽 행만 갱신되므로 다른 쪽은
옛 기일·옛 최저가로 굳는다(실측: 178 은 기일 2026-09-08, 8185 는 빈 값).

### 기존 탐지기가 못 잡는 변종이다

`detect_merged_case_duplicates_dryrun.py` 는 *"조각이 단독 행으로도 존재하는가"*
(병합 전/후)를 본다. 이 순서 뒤집힘은 **조각 집합이 완전히 같아서** 걸리지 않았다.

### 규모

```
병합사건(' / ' 포함) 행   auction 638 / auction_case 405
지금 갈라진 그룹          1
```

즉 지금은 1건이지만 **638행이 같은 위험에 노출**돼 있다.

### 왜 근본 수정을 하지 않았나 (승인 영역)

`case_no` 를 정규 순서로 만들면 **이미 저장된 638행의 식별키가 전부 바뀐다.**
다음 크롤이 옛 순서 행을 못 찾아 새 행을 또 만들어 중복이 오히려 늘어난다.
producer 수정만으로는 안 되고 **기존 행 재키잉(데이터 마이그레이션)이 함께**
가야 한다 — `docs/CLAUDE.md` 기준 승인 사항이다.
어느 행을 남길지도 제품 판단이다(관심물건/최근본이 걸려 있을 수 있다).

### 이번에 한 것

```
탐지기 확장   detect_merged_case_duplicates_dryrun.py 에 scan_order_split() 추가
              (읽기 전용, --apply 없음 그대로)
상한 고정     test_pipeline_integrity.py — 갈라진 그룹이 1을 넘으면 실패
              정규형 자기 검증 포함(순서 무관 정규화가 실제로 동작하는지)
```

변이로 확인: 상한을 0 으로 낮추면 실패(실제로 세고 있다), 정규형을 무력화하면
자기 검증이 실패(그룹 0 으로 조용히 통과하지 않는다).

### 곁가지 — `auction_date` 두 표 불일치 1건

이 중복 때문에 오늘 `auction.auction_date=''` / `auction_item.auction_date='2026-09-08'`
가 됐다. `migrate_execute` 의 "크롤 값이 비면 기존 정상값을 지우지 않는다"가 **옳게
동작한 결과**다(지웠다면 화면의 매각기일이 사라졌다). `SYNC_MISMATCH_CEILING` 에
근거와 함께 등록했고, 순서 문제가 해소되면 이 항목도 함께 지운다.

--------

#290

**상세 화면의 즐겨찾기가 연타 가드를 `await` **뒤에** 세우고 있었다.**
...
async function handleToggleFavorite() {
  ...
  setFavBusy(true)          // <- 여기서야 세운다
```

토큰이 아직 캐시되지 않은 **첫 조작**에서는 그 await 동안 `favBusy` 가 여전히
false 라, 빠른 두 번째 클릭이 위 가드를 그대로 통과해 요청이 두 번 나갔다.

### 왜 놓쳤나 — 같은 화면 두 곳의 규칙이 달랐다

`src/app/search/FavoriteButton.tsx` 는 **이미 옳았다**. 그 파일 주석이 이유까지
적어 두고 있다: *"getSession()이 끝나기 전에 재클릭이 들어오면 이 시점의 favBusy는
아직 false라 위 가드를 그대로 통과한다 — await 이전에 동기적으로 busy 처리해
재진입을 막는다."*

Sprint 39 가 상세 화면의 **등기부/구독/결제 4개 핸들러**를 그 패턴으로 통일했는데
**즐겨찾기 핸들러만 남아 있었다.** 검색 카드와 상세가 같은 동작에 서로 다른 연타
규칙을 갖고 있던 셈이다.

### 영향 범위

백엔드는 `UNIQUE(user_id, item_id)` + `FAVORITE_ALREADY_EXISTS` 로 이미 안전해서
**데이터가 깨지지는 않았다.** 고친 것은 불필요한 중복 요청과 두 화면의 규칙 불일치다.

### 조치

`setFavBusy(true)` 를 가드 바로 뒤로 옮기고, 로그인 리다이렉트로 빠지는 길목에서
반드시 다시 풀어 준다(안 풀면 하트가 영영 눌리지 않는다).

### 회귀

`test_favorites_lifecycle.py` §5 가 **두 화면 모두** 가드와 `setFavBusy(true)` 사이에
`await` 가 없는지 본다(주석은 걷어내고 코드만 본다 — 하필 그 자리의 주석이
"await 이전에"라고 설명하고 있어 첫 판본이 스스로 오탐했다).
변이(가드를 await 뒤로 되돌림)로 사망 확인.
