...
|---|---|---|---|
| 등기부/구독 핸들러에도 `idRef` 가드 적용 | `performRegistryRequest()` 계열은 `requestId` 를 잡지 않는다. 물건을 넘긴 직후 이전 물건의 **안내 문구**가 새 화면에 뜰 수 있다 | 작음 | 데이터가 아니라 문구이고, `[id]` 효과가 물건 전환 시 초기화한다. `handleSubscribe` 가 `performRegistryRequest` 를 이어서 부르므로 가드를 잘못 걸면 **정상 흐름이 막힌다** — 별도 검증이 필요하다 (BUGS #210 말미) |
