...
# 부록 B-2. 속도 제한 (2026-08-26 신설)

| 변수 | 기본값 | 설명 |
|---|---|---|
| `RATE_LIMIT_PER_MINUTE` | `1200` | 클라이언트 IP 당 분당 허용 요청 수. **0 이하면 제한하지 않는다.** 숫자가 아니면 기본값으로 떨어진다(부팅을 막지 않는다) |
| `RATE_LIMIT_ENABLED` | `1`(켜짐) | `0`/`false`/`no`/`off` 로 끈다 |
| `RATE_LIMIT_TRUST_FORWARDED` | `0`(꺼짐) | `1` 이면 `X-Forwarded-For` 의 첫 값을 클라이언트로 본다. **신뢰할 수 있는 프록시 뒤에서만 켤 것** — 그 헤더는 클라이언트가 위조할 수 있어, 아무 데서나 켜면 매 요청 다른 값을 보내는 것만으로 상한이 무의미해진다 |

- 대상은 `/api/` 로 시작하는 경로뿐이다. 그 밖(루트 등)은 세지 않는다.
- 초과하면 `429` + `Retry-After`(초). 보안 헤더는 429 응답에도 붙는다.
- **한계**: 프로세스 안에서만 센다. 워커/인스턴스를 늘리면 실효 상한이 그만큼 곱해진다.
  자세한 것은 `api/rate_limit.py` 상단 주석과 `docs/BETA_RELEASE_CHECKLIST.md` P1-5.
