...
- 자동 실행 시 확정된 활성 DB: `C:\Users\Administrator\Desktop\dojun-pass\auction.db`
...
- 실행 대상: `C:\Users\Administrator\Desktop\dojun-pass\run_daily.bat`
  ```bat
  @echo off
...
- `run_daily.bat`의 `cd /d C:\Users\Administrator\Desktop\dojun-pass` 라인. 제거/변경 시 상대경로 `"auction.db"`가 다른 파일을 열게 됨
...
2. 2026-07-11 이후 `migrate_execute.py` 자동 실행 기록 없음 (Task Scheduler 미등록, run_daily.bat 미호출 상태였음)
3. `Desktop\dojun-pass\`와 `Desktop\기타\dojun-pass\` 중복 폴더 존재, 각각 다른 `auction.db` 보유. 중복 존재 이유: 아직 결정되지 않음
