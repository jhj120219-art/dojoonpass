...
## Sprint 189 (2026-08-18) — 신규 `test_refresh_trigger.py` (12절 74검사)

변경 기반 재수집 사슬 전체를 고정한다. selenium/DB 서버 없이 돈다 —
스크래치 DB는 **부트스트랩 3단계를 실제로 돌려** 만든다(스키마를 손으로 베끼지 않는다).
...
| 절 | 지키는 것 |
|---|---|
| 1 | 필드 -> 자산 매핑이 단일 소스이고, 유찰(기일/최저가)만으로는 **사진을 다시 받지 않는다** |
| 2 | `done`만 되돌린다 — failed / SKIPPED_UNSUPPORTED / in_progress는 건드리지 않는다 |
| 3b | **기일이 지난 물건은 되돌리지 않는다**(되돌리면 성공 기록만 잃는다) |
| 3 | 기일이 미래로 다시 잡힌 `SKIPPED_EXPIRED`만 부활, 그것도 `pending`으로 |
| 4 | `refresh` claim -> `in_progress_refresh` + `overwrite=True`, 원자적 클레임 유지 |
| 5 | **재시도와 stale 회수가 재수집 의도를 각자 제자리로 되돌린다** |
| 6 | 재수집도 `MAX_DOC_RETRY`로 종결된다(무한 순환 없음) |
| 6b | **재수집 실패가 이미 READY/NO_IMAGE인 것을 FAILED로 덮지 않는다**(대조군 포함) |
| 7 | 우선순위 재계산이 `refresh` 행을 빠뜨리지 않는다 |
| 8 | 상한이 **조용히 자르지 않는다**(잘린 건수를 반환값에 남긴다) |
| 9 | `overwrite`가 `doc_worker` -> `collect_document`까지 실제로 도달한다 |
| 10 | 재수집일 때는 **형제 물건 복사 지름길을 쓰지 않는다** |
| 11 | 큐 상태 어휘가 한 곳에서만 정의된다(하드코딩이 다시 들어오면 FAIL) |
| 12 | 매일 배치가 트리거를 **커밋 뒤에** 부르고, 실패가 크롤링을 죽이지 않는다 |

### 증분

```
test_asset_pipeline.py         5-D  사진 형식 교체 왕복(8)  — 두 지문 공식의 일치를 고정
                               5-E  같은 순번 중복 파일(2)
                               7-B(5) 사진이 **추가**된 경우(3) — 6가지 상황 중 유일하게
                                     이름이 없던 것
test_doc_storage_atomicity.py  7d   기존 PDF 원자적 덮어쓰기 + 교체 직전 크래시 주입(6)
                               7e   비원자적 이동 **AST 전수 가드**(2)
```

### 가드가 헛돌지 않는지 확인하는 법 (7e)

전수 가드는 "결함이 없어서 통과"와 "검사가 아무것도 안 봐서 통과"가 겉으로 같다.
그래서 **변이를 주입해 실제로 FAIL하는지** 확인한 뒤 복원했다:

```bash
# move_into_place(...) 를 shutil.move(...) 로 되돌린다
python test_doc_storage_atomicity.py
#   [FAIL] 수집/저장 계층에 목적지 직접 쓰기가 없다: ['crawler/doc_crawler.py:331']
# 복원 후 다시 ALL TESTS PASSED
```

문자열 grep이 아니라 AST를 쓰는 이유도 여기서 나왔다 — `doc_crawler.py`와 이 테스트가
**결함을 설명하는 문장**에 `shutil.move(...)`를 그대로 적고 있어서, 문자열 검색은
산문을 코드로 오판한다(실제로 그렇게 걸렸다).


## Sprint 189 이어서 (2026-08-18) — 재수집 정확도/비용 회귀

```
test_doc_storage_atomicity.py  7f  현황조사서 지문이 수집 시각/키 순서에 안 흔들린다(6)
                               7g  내용 무변경이면 파일을 다시 쓰지 않는다(6)
                               7h  같은 PDF는 목적지를 건드리지 않는다(7, 대조군 포함)
test_asset_pipeline.py         5-F 재수집이어도 같은 사진은 다시 쓰지 않는다(9, 대조군 포함)
```

### mtime 으로 무엇을 단언할 수 있고 무엇을 없는가

작성 중 실제 플레이크를 겪고 정리한 규칙이다.

```
"안 썼다"  -> mtime 으로 단언할 수 있다 (쓰지 않았으면 절대 안 바뀐다)
"썼다"     -> mtime 으로 단언할 수 **없다** — 두 쓰기가 파일시스템의 타임스탬프
              갱신 간격보다 가까우면 같은 값이 나온다. **내용**으로 확인한다.
```

### 테스트 잔해가 다음 실행을 죽이지 않게 (BUGS #126)

`documents/`는 OneDrive 동기 대상이라 디렉터리에 **R 속성**이 붙는다(실측 `0o555`).
맨 `shutil.rmtree()`는 거기서 `PermissionError`로 죽고, 그러면 **지우지 못한
디렉터리가 남아 다음 실행도 같은 자리에서 죽는다**(실측 6벌 누적).
정리는 반드시 `_force_rmtree()`(속성을 풀어 가며 지운다)를 쓴다 — 자기 디렉터리든
이전 실행 잔해든 **같은 함수**로.
...
## Sprint 190 (2026-08-18) — 이스케이프 파손 회귀 가드

`test_schema_hygiene.py`에 절 추가(2검사). 추적 텍스트 파일 **315개**를 훑는다.
...
셸 heredoc 은 본문의 백슬래시 이스케이프를 해석한다. 문서에 Windows 경로를 넣다가
`\r` 이 캐리지 리턴이 되면 문장이 두 줄로 쪼개진다. 실제로
`docs/BETA_RELEASE_CHECKLIST.md` 43행이 그렇게 깨져 있었다:

```
조치는 `.<CR><LF>egister_scheduler_tasks.ps1 -Apply` 한 줄이며 ...
```

사람이 읽으면 바로 보이지만 **아무 검사도 이것을 보지 않았다** — 마크다운이라 빌드도
린트도 통과하고, 내용이 아니라 표기라 문서 드리프트 감사에도 안 걸린다. 그런데 그
문서는 **운영자가 그대로 복사해 실행하는 명령**을 담고 있다.

두 가지 표지를 본다(정상 문서에는 나타나지 않는다):

```
(1) 줄 끝이 백틱+점 또는 홑 백슬래시 -> 다음 줄이 소문자로 시작
(2) 줄이 알려진 파일명의 첫 글자가 잘린 조각으로 시작 (register -> egister 등)
```

### 헛돌지 않는지 확인했다

`docs/API_KEY_CHECKLIST.md`에 파손을 주입해 **즉시 FAIL하는 것을 확인**한 뒤 복원했다
(`git status` 깨끗함까지 확인). 전수 가드는 "결함이 없어서 통과"와 "아무것도 안 봐서
통과"가 겉으로 같으므로, 이 저장소에서는 **변이 확인을 붙여야 가드로 인정한다**.


## Sprint 191 (2026-08-18) — 불변식 가드로 방식 전환

```
test_asset_pipeline.py         5-G 사진 감소 시 파일 정리 (D/B/C 대조군)      12검사
...
`#120`(형식 변경 시 고아 파일)을 고칠 때 **`#127`(순번 감소 시 고아 파일)을 놓쳤다.**
둘은 같은 결함의 다른 인스턴스인데, 개별 검사를 하나씩 늘리는 방식은 다음 인스턴스를
구조적으로 못 잡는다. 그래서 5-I는 **시나리오 표 + 불변식**이다.
...
새 시나리오는 표에 한 줄. 새 결함 유형이 생겨도 세 근거 중 둘이 갈라지는 순간 잡힌다.

### 7i 도 같은 방식이다

문서 완료 기준과 서빙 대상의 정합성을 **목록을 손으로 맞춰서** 검사하지 않는다 —
`api/v1/documents.py:DOC_TYPE_FILES`에서 파일명을 읽어 `DOC_REQUIRED_FILES` 안에 있는지
대조한다. 새 문서 종류가 생겨도 저절로 따라간다.

### 새 가드는 반드시 변이 확인을 붙인다

```
5-I  `_remove_files_not_in()` 호출 무력화     -> 4건 FAIL (복원 후 PASS)
§15  CLAIM_RACE_MAX_ATTEMPTS = 1 로 되돌림    -> 3건 FAIL (복원 후 PASS)
```

전수/구조 가드는 "결함이 없어서 통과"와 "아무것도 안 봐서 통과"가 겉으로 같다.
이 저장소에서는 변이 확인이 붙어야 가드로 인정한다.

### 출력 리터럴의 cp949 제약을 다시 확인했다

새 검사 제목에 EM DASH(`—`)를 썼다가 `test_console_encoding.py`에 잡혔다.
cp949 콘솔에서 `print` 하면 `UnicodeEncodeError`로 죽고, 그러면 **판정문에 `[FAIL]`이
하나도 없이 종료 코드만 1**이 된다. 주석·docstring 은 무관하고 **출력 리터럴만** 제약된다.


## Sprint 198~200 (2026-08-18) — 관측 설계 / 용량 / 배선

```
test_refresh_trigger.py §17  재수집 상한이 실행 창 안에 들어간다        6검사
test_refresh_trigger.py §18  능력 19개가 실행 경로에 배선돼 있다        7검사
```
...
값만 60 으로 고치면 다음에 누가 창을 줄이거나 수집이 느려지거나 대상 종류를 늘리면
조용히 다시 넘친다. **못 잡는 것도 적어 뒀다** — `DOC_COLLECT_SECONDS_PER_ROW` 는
실측을 사람이 옮겨 적은 값이라, 현실이 느려졌는데 상수를 안 고치면 통과하면서 넘친다.

### §18 — "기능 존재"와 "호출됨"은 다르다

이 저장소의 최악 결함은 고장이 아니라 **배선 누락**이었고(Sprint 144/150/189),
셋 다 테스트는 통과하고 있었다. 함수를 직접 호출해 검사했기 때문이다.
그래서 §18 은 동작이 아니라 **배선**을 본다.

**진입점은 `run_*.bat` 에서 유도한다** — 손으로 적었다가 `refresh_queue_priority` 를
거짓 경보로 잡은 뒤 고쳤다.

### 관측 설계에 대한 교훈 (Sprint 198)

"0건"을 보면 **관측 설계부터 의심한다.** 같은 CSV 데이터에서 비교 축을
연속 -> 물건별 처음<->마지막 으로 바꾸는 것만으로 **0 -> 132건**이 나왔다.
측정 도구는 도메인 시그니처로 검증한다(저감률 0.7/0.8 = 법정 저감률).


### PDF 수집 회귀: 7j / 7k 는 **탭을 절대 만들지 않는 드라이버**로 돌린다

BUGS #135/#136 은 "새 탭이 떴는가"를 성공 조건으로 삼은 결함이다. 그러므로 회귀는
탭이 뜨는 대역으로는 **아무것도 증명하지 못한다**. 실제 조건(PDF 외부열기 설정)을
그대로 재현하는 `_FakeNoTabDriver` 로 돌린다.

| | 감정평가서 7j | 명세서 7k |
|---|---|---|
| 탭 없음 + 파일 도착 | 저장한다 | 저장한다 |
| 탭 없음 + 아무것도 없음 | 실패한다(대조군) | 실패한다(대조군) |
| 가짜 PDF 도착 | 거부한다 | (7j 가 공통 검사) |
| 뷰어 경로(탭 있음) | - | **그대로 성공한다(대조군)** |

7k 의 마지막 줄이 중요하다. #136 수정은 뷰어 경로를 `else:` 로 옮기므로,
**고치면서 정상 경로를 깨뜨릴 수 있는 모양**이다. 대조군 없이는 그걸 못 잡는다.

### 전수 가드는 "무엇을 안 봤는지"를 스스로 말해야 한다 (Sprint 202, BUGS #137)

`test_wait_for_download_callers_check_the_result` 는 AST 를 훑으며 모든 호출 지점이
결과를 확인하는지 본다. 그런데 이렇게 훑고 있었다.

```python
body = getattr(parent, "body", None)   # <- body 만 본다
```

`ast.If` 는 `orelse` 를, `try` 는 `finalbody` 를 따로 갖는다. 그래서
**`else:` / `finally:` 안의 코드는 감사 대상에서 통째로 빠져 있었다.**
#136 을 고치며 뷰어 경로를 `else:` 로 옮기자 호출 지점 3개 중 1개가 조용히 사라졌다 —
**검사는 계속 통과하는데 보지 않는 상태.**

BUGS #133(BOM 파일을 조용히 건너뜀)과 같은 부류다. 이 세션에서 두 번 나왔다.
전수를 표방하는 가드에는 다음을 요구한다.

1. **훑는 범위를 이름으로 열거**한다(`body`/`orelse`/`finalbody`) — 새 문법이 생겨도
   속성 이름으로 따라간다.
2. **찾은 개수를 하한으로 고정**한다 — 대상이 사라지면 0건 통과가 아니라 FAIL 이다.
3. **변이 검증**은 `else:` 안쪽에도 결함을 넣어 본다 — 정상 경로에만 넣으면
   사각지대를 못 찾는다.


### 27. CRLF 로 커밋된 파일을 LF 로 다시 쓰지 않는가 (Sprint 202, BUGS #138)

`core.autocrlf=true` 인데도 blob 자체에 CRLF 가 든 파일이 **73개** 있다. git 은
index 의 blob 에 CR 이 있으면 그 경로의 정규화를 끄므로, 그 파일들은 작업본이
**글자 그대로 CRLF 를 유지해야** 한다. LF 로 다시 쓰면 전 파일이 diff 에 뜬다.

**8번(BOM)과 달리 `git diff --name-only` 로 후보를 고르면 안 된다.** 줄끝 문제는
diff 에 안 나타나는 경우가 있어서, 후보를 diff 로 고르면 그 순간 못 보게 된다.
추적 파일 **전체**를 blob 과 직접 대조한다(`git cat-file --batch` 한 번).

**범위를 한 번 틀렸다.** 처음엔 "모든 파일의 줄끝이 HEAD 와 같아야 한다"로 만들어
정상 체크아웃 97개를 잡았다. LF blob 은 autocrlf 가 정규화하므로 작업본 줄끝이
자유다 - 규약 위반이 아니다. **가드를 넓게 만드는 것과 정확하게 만드는 것은 다르다.**

**하한을 함께 고정한다** — `CRLF blob >= 50`. 배치 파싱이나 경로 인코딩이 깨지면
이 가드는 0개를 대조하고 조용히 초록이 된다. 변이(열거를 5개로 자름)로 확인했다.

한계를 밝혀 둔다: 이미 혼재된 blob 에서 **일부 줄만** LF 로 바뀌는 것은 못 잡는다.
잡는 것은 "규약이 통째로 뒤집힌 경우"다.


### 전수 가드에는 하한을 함께 고정한다 (Sprint 205)

"나쁜 것이 없다"만 단언하는 검사는 **대상을 하나도 못 찾았을 때도 통과**한다.
열거가 깨지는 방식은 여러 가지고 전부 조용하다 - 인코딩(BOM), 문법 블록(`else:`),
CSV 헤더 위치, 제외 목록, 경로 구분자.

그래서 스캔으로 대상을 발견하는 가드에는 다음을 함께 요구한다.

1. **찾은 개수를 단언한다.** 실측값보다 낮되 "절반이 사라져도 안 걸리는" 값은 피한다.
2. **개수를 출력한다.** 하한을 통과해도 숫자가 줄고 있으면 사람이 본다.
3. **변이는 대상 쪽에도 넣는다** - 열거를 비우거나 잘라 FAIL 이 나는지 본다.
   나쁜 값만 넣어 보면 이 계열의 결함은 절대 드러나지 않는다.

**하한이 성립하지 않는 경우도 있다.** `test_source_bom_matches_head` 는
`git diff --name-only` 로 후보를 고르므로 작업트리가 깨끗하면 0개가 정답이다.
그런 자리는 하한 대신 **이유를 적어 둔다** - 적어 두지 않으면 다음 사람이
"빠뜨린 하한"으로 오해하고 잘못된 값을 박는다.


### 가드가 결함을 **지키고** 있을 수 있다 (Sprint 217)

BUGS #144 를 고치자 기존 검사 두 개가 깨졌다.
...
둘 다 **의도는 옳았다** — 스킵 경로가 파일을 다시 받지 않는 것은 지켜야 할 성질이다.
틀린 것은 **증거**다. "다시 쓰지 않았다"를 빈 목록으로 증명했고, 그 빈 목록이야말로
`_record_doc_raw()` 가 실체 기록을 건너뛰게 만든 원인이었다. 즉 그 두 단언은
결함이 사라지지 않도록 **잠그고** 있었다.

교훈은 "테스트를 고치면 안 된다"가 아니다.

1. 단언은 **성질**을 걸어야 한다. 그 성질의 부산물을 걸면, 부산물이 바뀔 때
   성질이 아니라 **구현이 고정된다.**
2. 고칠 때는 **의도를 옮기고 증거만 바꾼다.** 여기서는 "다시 쓰지 않았다"를
   파일 mtime/크기 무변경으로 다시 증명했다 — 더 강한 증거다.
3. 회귀가 깨졌을 때 **먼저 물어볼 것**은 "코드가 틀렸나"가 아니라
   "이 단언이 무엇을 지키려 했나"이다.

같은 계열을 다시 만들지 않도록 구조 검사를 하나 걸었다
(`test_false_success.py` 5) — 수집기가 `success=True` 를 쓰는 모든 분기에
`files_saved`/`images`/`no_asset` 중 하나가 함께 있어야 한다. 정적 검사라
**도달 가능성은 보지 않는다**(그 한계는 그 자리에 적어 뒀다). 실제 값이 옳은지는
fixture 가 본다.


### 감사 도구도 세 단계로 검사한다 — 정상 / 결함 주입 / 복구 (Sprint 217)

"invariant 를 추가했다"로 끝내면 그 invariant 가 **실제로 우는지** 아무도 모른다.
이번에 그 방식으로 두 가지를 잡았다.

```
test_schema_hygiene.py 6-B   BOM 파일 31개의 1행 import 를 못 보고 있었다 (BUGS #146)
run_python_tests.py          집계기 자신에게 검사가 없었다 (BUGS #147)
```

**한 번은 회귀를 새로 쓰고도 무효였다.** BOM 검사를 처음 붙였을 때 "utf-8 로는 못 읽고
utf-8-sig 로는 읽는다"는 **차이만** 단언했다. 스캔 루프가 어느 쪽을 쓰는지는 보지
않았으므로, 인코딩을 되돌리는 변이가 **그대로 통과했다.** 스캐너를 함수로 분리해
탐침 파일을 직접 먹이도록 바꾼 뒤에야 변이가 잡혔다.

    검사가 존재한다  !=  검사가 회귀를 잡는다

그래서 새 가드에는 반드시 세 단계를 돌린다.

    정상        -> PASS
    결함 주입    -> FAIL   (여기가 없으면 그 가드는 장식이다)
    복구        -> PASS

### 관측되지 않는 성질은 관측되게 만든다 (Sprint 217)

"읽지 못한 파일을 조용히 삼키지 않는다"를 단언했는데 **변이가 통과했다** —
정상 저장소에는 읽지 못하는 파일이 0개라 그 성질이 애초에 관측되지 않았기 때문이다.
없는 경로를 일부러 먹여서 "미확인이 미확인으로 보고되는지"를 직접 보게 고쳤다.

같은 이유로 `audit_schedule_health.py` 는 **칸 수가 어긋난 CSV 행을 몇 개 버렸는지**
세어서 보고한다. 버려진 행 하나가 곧 "감사에서 사라진 예약 작업 하나"이고,
그 결과는 "그런 작업 없음"이라는 **정상과 똑같은 모양**으로 나온다.

### 하한은 열거 방식별로 잡는다 (Sprint 217)

`test_error_logging.py` 의 진입점 목록을 하드코딩에서 git 목록으로 바꾸자
훑는 파일이 **58 -> 86개**가 됐다(28개가 검사 밖이었고, 그중 3파일에 bare `except:`
4곳이 있었다). 그런데 하한은 40 그대로였다 — 실측 86 에 하한 40 은
"절반이 사라져도 안 걸리는 값"이고, 그것은 이 문서가 하지 말라고 적어 둔 하한이다.

git 목록이면 80, 폴백이면 40 으로 **열거 방식에 따라 다르게** 요구하고,
어느 쪽을 썼는지 출력에 남긴다.

### 테스트가 운영을 멈추지 않게 한다 (Sprint 217)

`test_doc_worker_recovery.py` 의 락 검사들은 **운영 경로**(`logs/doc_worker.lock`)를
그대로 쓴다(모듈 상수를 갈아 끼우면 검사 대상이 실물이 아니게 되므로 옳은 선택이다).
대신 값을 치른다 — 검사가 중간에 죽어 락이 남으면 **다음 실제 doc_worker 실행이
5시간 동안 건너뛴다.** 회수 토큰(`.reclaim`)이 남으면 회수 자체가 같은 시간만큼 막힌다.

정리를 `finally` 로 옮기고 `run()` 전체를 감싸는 안전망을 뒀다.
중단을 주입해 확인했다 — 종료코드 1 로 죽어도 락 파일은 남지 않는다.

덧붙여 스위트 전체 실행 전후의 파일 목록을 대조했다:
**새로 생긴 파일 0개 / 사라진 파일 0개**, 크기가 변한 것은 로그 하나뿐이다.


### 픽스처가 파이프라인이 거부할 데이터를 쓰고 있지 않은가 (Sprint 218)

BUGS #148 을 고치자 기존 검사 **10건이 깨졌다.** 원인은 코드가 아니라 **픽스처**였다 —
100~600바이트짜리 사진으로 `auction_image 2행 / images_status=READY` 를 단언하고
있었는데, 그 크기는 서빙 계층이 **404 로 거절한다**(`MIN_IMAGE_BYTES` = 1,024).

즉 그 검사들은 **파이프라인 전체가 받아들이지 않는 데이터로 "정상"을 그리고** 있었다.
통과하고 있었지만 증명한 것은 아무것도 없었다.

교훈:

1. 픽스처 데이터는 **파이프라인의 모든 관문을 통과할 수 있는 값**이어야 한다.
   하나라도 거절하는 값이면 그 검사가 그리는 "정상"은 실제로 존재할 수 없는 상태다.
2. 하한이 있는 곳(크기·길이·형식)은 픽스처에서 **상수를 참조**한다.
   숫자를 손으로 적으면 하한이 바뀔 때 조용히 갈라진다.
   (`MIN_FIXTURE_PAD` 를 두어 한 번에 올렸다.)
3. 검사가 무더기로 깨졌을 때 **먼저 물어볼 것**은 "코드가 틀렸나"가 아니라
   "이 픽스처가 실제로 가능한 상태를 그리고 있었나"이다.

### 부분 문자열로 "규칙이 하나인가"를 판정하지 않는다 (Sprint 218)

`MIN_IMAGE_BYTES` 를 세 판정처가 같이 쓰는지 보려고 본문에 그 **문자열이 있는지**만
확인했다. 변이(`_MIN_IMAGE_BYTES = 1024` 를 함수 안에 박음)가 **그대로 통과했다** —
새 이름이 옛 이름을 부분 문자열로 포함했기 때문이다.

AST 로 두 가지를 본다:

    숫자 상수 대입이 있는가        `X = 1024` 형태면 그 자체가 규칙 이중화다
    단일 소스에서 import 하는가     `from crawler.image_assets import MIN_IMAGE_BYTES`

이름이 비슷한 것과 **같은 값을 공유하는 것**은 다르다.


### 고칠 수 없는 것은 **상한으로 잠근다** (Sprint 219)

접근성 수치(작은 글자·낮은 대비)는 **제품 디자인 결정**이라 임의로 고칠 수 없다.
그렇다고 재고 넘어가면 다음 사람은 그 수치가 좋아졌는지 나빠졌는지 알 수 없다.

그래서 `test_frontend_accessibility.py` 는 **ratchet** 으로 만들었다.

    현재값을 상한으로 박는다  ->  줄면 통과(개선은 언제나 환영), 늘면 실패

`test_schema_hygiene.py` 가 "추적되면 안 되는 파일"에 쓰는 것과 같은 방식이다 —
**정리는 나중에 하더라도 늘어나는 것만은 막는다.**

함께 건 것: 열거 하한(파일 12개 이상 — glob 이 깨지면 0개를 훑고 조용히 통과한다),
그리고 **실측 수치가 문서에 남아 있는지**. 문서가 사라지면 상한 숫자의 뜻을
아무도 알 수 없으므로, 그때는 이 검사가 먼저 운다.

### 주석 속 코드를 실제 코드로 세지 않는다 (Sprint 219)

`alt` 없는 `<img>` 를 찾는 검사가 **주석 안의 `<img>`** 를 잡아 멀쩡한 파일을
결함으로 보고했다 — `ResultThumbnail.tsx` 의 주석이 Next.js 오류 메시지를
인용하고 있었다.

소스를 문자열로 훑는 검사는 **주석을 먼저 걷어낸다.** 단, 지울 때
**줄 번호를 보존**한다(공백으로 치환) — 그러지 않으면 보고된 줄 번호가 원본과
어긋나 오히려 사람을 헤매게 한다.

이번 세션에서 측정/조사 도구가 틀린 것은 **여섯 번**이다
(검색 대조 쿼리 정규화 · check-then-act 스윕 범위 · lazy 이미지 · oklch 색 파싱 ·
부분 문자열로 상수 중복 판정 · 주석 속 `<img>`).
전부 **멀쩡한 코드를 결함으로 올릴 뻔했거나, 결함을 못 본** 경우다.
규칙: **결함처럼 보이면 도구를 먼저 의심하고, 알려진 정답으로 도구를 검증한다.**


### "정상"이라고 쓸 수 없을 때 무엇을 잠그는가 (Sprint 220)

모바일 뷰포트로 재지 못했다(도구 제약). 그때 선택지는 셋이다.

```
1. "모바일 정상" 이라고 쓴다        <- 거짓말이다
2. 아무것도 안 한다                  <- 다음 사람이 처음부터 다시 판단해야 한다
3. **원인이 없다는 것**을 잠근다      <- 이번에 택한 것
```

가로 넘침을 만드는 원인(고정 폭 / `w-screen` / `min-w-max` / `<table>` /
반응형 없는 3열 그리드 …)을 목록화해 **0곳임을 전수로 고정**했다.

이것은 **"넘치지 않는다"의 증명이 아니라 "넘칠 원인이 없다"의 증명**이다.
문서와 검사 양쪽에 그 구분을 그대로 적었다. 실기기 확인은 여전히 필요하고,
그것은 **SKIP 으로 기록**했다.

### 계층이 셋이면 대조하는 검사가 있어야 한다 (Sprint 220)

검색목록은 세 파일이 각자 필드를 적고 있었다.

```
api/v1/search.py       row_to_item() 의 키
src/app/search/types.ts  타입 선언
ResultList.tsx           실제 렌더
```

한쪽만 바뀌면 화면은 **오류 없이 빈칸**이 된다 — `item.foo` 가 `undefined` 여도
React 는 조용히 넘어간다. 서버에도 콘솔에도 흔적이 없다.

검사 방향은 **한쪽만**이다: 렌더/타입이 쓰는데 API 가 안 주면 결함,
API 가 주는데 안 쓰는 것은 화면의 선택이다. 방향을 섞으면 정상 코드가 걸린다.

가장 현실적인 사고는 **API 키 이름 변경**이고, 변이로 그것이 타입·렌더 양쪽에서
동시에 우는 것을 확인했다.

### 겉모습으로 결론 내지 않는다 — 빌드 결과를 열어 본다 (Sprint 220)

Sprint 219 에서 *"소스에 `text-xs` 가 111곳이니 대부분 px 고정"* 이라고 적었다.
**빌드된 CSS 를 열어 보니 `--text-xs: .75rem` — rem 기반이었다.**

소스의 클래스 이름은 **의도**를 보여 줄 뿐이고, 실제 값은 빌드가 정한다.
프레임워크가 토큰을 어떻게 컴파일하는지 확인하지 않고 결론을 내면,
**"선행 작업 = 전면 rem 전환"** 같은 과장된 계획이 문서에 남는다
(실제로는 8곳 교체였다).

그래서 이 가드는 **전제 자체도 검사한다** — 빌드 CSS 에서 `--text-xs` 가 rem 인지 보고,
빌드 산출물이 없으면 통과로 위장하지 않고 **"확인하지 못했다"고 출력**한다.


### 계산된 스타일은 "지금 이 순간"만 말한다 (Sprint 221)

포커스 표시가 있는지 보려고 `getComputedStyle(el).outlineStyle` 을 읽었더니
**53개 전부 `none`** 이었다. 결함처럼 보였지만 아니었다 —
그것은 **비포커스 상태의 브라우저 기본값**이다(기본 `outline-style` 은 none).

`el.focus()` 로 강제 포커스한 뒤 다시 쟀는데도 0 이었다.
**프로그래밍 포커스는 Chrome 에서 `:focus-visible` 을 켜지 않는다** —
키보드 상호작용에서만 켜진다.

결국 **CSS 규칙 자체**를 읽어야 답이 나왔다(빌드 CSS 에서 `outline` 선언을 훑었다).

교훈: **상태에 따라 달라지는 성질은 상태를 읽어서 알 수 없다. 규칙을 읽어야 한다.**
`:hover`, `:focus-visible`, `:disabled`, 미디어 쿼리 전부 같다.

### 검출기는 넣기 전에 검증한다 (Sprint 221)

CSS 에서 "전역 포커스 리셋"을 찾는 정규식이 `@supports` 블록과 주석을 선택자로 오인해
**변이를 넣지 않아도 항상 실패**했다. 그 상태로는 무엇을 잠근 것인지 알 수 없다
(항상 빨간 가드는 아무것도 지키지 않는다).

고친 뒤에는 **가드에 넣기 전에** 세 조건으로 검증했다.

    실제 CSS                    -> 0건    (오탐 없음)
    가짜 전역 리셋을 주입        -> 1건    (실제로 잡는다)
    의도된 유틸리티를 주입       -> 0건    (정상을 결함으로 세지 않는다)

세 번째가 특히 중요하다 — 잡아야 할 것만 잡는지가 아니라 **잡지 말아야 할 것을
안 잡는지**까지 봐야 검출기가 쓸모 있다.

### 대체를 검사할 때 "자기 자신"을 세지 않는다 (Sprint 221)

`focus:outline-none` 을 쓴 자리에 대체 표시가 있는지 보는 검사에서,
대체 후보 패턴에 `outline-` 를 넣었더니 **`focus:outline-none` 자신이 자기 대체로**
잡혔다. 무엇을 지워도 통과했고, **변이를 돌려서야 발견했다**.

"A 를 지웠으면 B 가 있어야 한다" 류의 검사는 **B 의 패턴이 A 를 포함하지 않는지**
반드시 확인한다. 이번 세션에서 같은 실수를 두 번 했다
(상수 단일화 검사의 `MIN_IMAGE_BYTES` 부분 문자열도 같은 모양이었다).


### JSX 여는 태그를 정규식으로 자르지 않는다 (Sprint 222)

`<select[^>]*>` 로 여는 태그를 잡았더니 **`onChange={(e) => f(e)}` 의 `>` 에서 잘렸다.**
그 뒤에 있던 `aria-label` 을 못 보고 **"select 7개 전부 이름 없음"** 이라고 보고했다 —
브라우저로는 전부 이름이 있었으므로 **도구가 틀린 것**이었다.

JSX 는 속성 안에 화살표 함수·객체·중첩 JSX 가 들어간다. 여는 태그의 끝을 찾으려면
**중괄호 깊이를 추적**하고 `=>` 의 `>` 를 건너뛰어야 한다.

그리고 그런 추출기는 **자기 검증**을 붙인다 —
이름이 있는 샘플에서 이름을 읽는지, 없는 샘플에서 없다고 읽는지 둘 다.
한쪽만 보면 "항상 있다고 답하는" 추출기도 통과한다.

### 화면 상태에 따라 달라지는 것은 그 상태를 만들고 잰다 (Sprint 222)

`/search` 의 폼 컨트롤을 세었더니 **5개**였다. 실제로는 **93개**다 —
검색 조건 아코디언이 접혀 있었기 때문이다.

접힌 상태로 "이름 없는 컨트롤 3개"라고 보고했다면, 실제 16개 중 13개를 놓친 채
**고쳤다고 착각**했을 것이다. 세기 전에 **펼친다.**

같은 계열: `:focus-visible`(포커스를 줘야 한다), lazy 이미지(로드를 기다려야 한다),
모달 내부 요소(열어야 한다). **"지금 보이는 것"이 전부가 아니다.**



### 코드를 읽어 아는 것과 재서 아는 것을 구분한다 (Sprint 223)

"배치 조회를 쓴다"는 **코드를 읽어** 아는 사실이고, 실제로 SQL 이 몇 번 나가는지는
**재 봐야** 아는 사실이다. Sprint 145가 썸네일 배치 조회를 넣은 뒤로 실제 횟수를 잰 적이
없었다 — 한 줄만 잘못 옮기면 물건마다 한 번씩 나가고, **그때도 화면은 똑같이 잘 보인다.**

`sqlite3.Connection.set_trace_callback` 으로 실행된 문장을 센다.

```
size=1 / 3 / 9   전부 SQL 3회 (COUNT + 페이지 행 + auction_image 배치)
```

같은 계열: "함수가 존재한다"와 "실행 경로에서 호출된다", "저장에 성공했다"와
"브라우저에 보인다".

### 합산 검사는 화면별 결함을 숨긴다 (Sprint 223)

`<main>` 랜드마크 검사가 **저장소 전체 사용 횟수**로 `>=1` 만 봤다.
검색 화면 하나가 갖고 있으면 **나머지 4개 화면이 전부 0이어도 통과**한다 — 실제로 그랬다.

파일 단위로 조인 뒤에도 부족했다. **분기 하나가 랜드마크를 잃어도 통과**했다(변이로 확인).
그래서 화면 루트 개수와 맞춰 세도록 다시 조였다.
*합산은 "어딘가에 있다"만 말한다. 불변식이 "화면마다"라면 화면마다 세야 한다.*

### 문자열 존재로 기능 유무를 판정하지 않는다 (Sprint 223)

포커스 트랩 검사가 `"'focusin'" in src` 로 감시 등록 여부를 봤다.
**등록 줄을 지워도 해제 줄에 같은 문자열이 남아** 변이가 통과했다.
`addEventListener('focusin'` 처럼 **그 동작을 하는 형태**로 좁혀야 한다.

### 검사 범위에 죽은 사본이 섞이지 않게 한다 (Sprint 223, BUGS #154)

`test_console_encoding.py` 가 `.claude/worktrees/...`(옛 커밋의 저장소 통째 사본)까지
훑고 있었다 — **스캔 298개 중 101개(34%)가 그 사본.** 아무도 실행하지 않는 코드에
규칙을 강제하면, 그 안의 위반 하나로 **현재 코드가 멀쩡한데 빨간불**이 켜진다.
범위를 좁힌 뒤에는 **하한도 함께 걸어야** 한다(과하게 좁혀 0개를 훑는 것도 초록으로 보인다).

### 보이지 않는 탭에서 잰 포커스 값은 근거가 아니다 (Sprint 223)

`visibilityState: hidden` 인 탭에서 `element.focus()` 를 부르면 `activeElement` 는 바뀌지만
**focus/focusin 이벤트가 전혀 발생하지 않는다**(직접 단 리스너 0회 호출로 확인).
그 상태에서 잰 값으로 **없는 결함을 만들 뻔했다** — 탭을 보이게 하고 진짜 마우스 클릭으로
다시 재니 정상이었다. 포커스는 창 포커스 상태에 의존한다.

### 조사 도구가 먼저 거짓말할 수 있다 (Sprint 223)

API 라우트를 세려고 `app.routes` 를 읽었더니 **2개**가 나왔다.
이 FastAPI 는 `include_router` 를 `_IncludedRouter` 로 지연 보관해 `path` 가 `None` 이다.
OpenAPI 스키마(실제 서빙 기준)로 바꾸니 42개였다.
**비정상적으로 작은/큰 측정값이 나오면 대상이 아니라 도구를 먼저 의심한다.**
