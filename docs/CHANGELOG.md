...
## 2026-08-24 — Sprint 251~254 (동시성 결함 4건 발견·수정)

### 방법

**"아무 테스트도 실행하지 않는 제품 코드"** 를 전체 스위트 합산 커버리지로 찾아
그 자리를 하나씩 열었다(`audit_test_reality.py` 의 역방향). 결함마다
**결정적 재현 -> 수정 -> 회귀 -> mutation** 순서를 지켰다. 재현에는 스레드를 쓰지
않고 커넥션/파일시스템 래퍼로 창을 직접 벌렸다 - 확률이 개입하면 방어선이 아니다.

### 고친 것 (전부 코드 범위, 승인 불필요)

| # | 결함 | 변이 |
|---|---|---|
| #180 | `sync_expired_status()` 가 그 사이 확정된 구독 상태를 조건 없이 덮어쓴다. 해지(CANCELLED, 최종 상태)가 EXPIRED 로 뒤집혀 **금지된 전이가 DB 에 남고** 해지된 구독이 재구독으로 되살아난다 | 3/3 |
| #181 | stale 회수로 큐 행을 빼앗긴 실행이 뒤늦게 그 행을 자기 것처럼 종결한다(좀비 워커). 같은 문서를 두 번 받고, 성공한 문서가 'failed' 로 뒤집히며, 남의 재시도 예산이 깎인다 | 7/7 |
| #182 | 브라우저가 죽어도 드라이버 재시작 복구가 **한 번도 발동하지 않고**, 그 법원이 "기일 없어 스킵" 으로 요약된다(#47 계열 - 배치가 사실이 아닌 것을 말한다) | 7/7 |
| #183 | 락 자리에 파일이 아닌 것이 있으면 `acquire()` 가 예외를 올려 **배치가 죽는다**. 락을 못 얻는 것은 실패가 아닌데 실패로 끝난다 | 6/6 |

앞선 세션(251~253)에서는 프런트 요청 타임아웃 부재(전 경로), 타임아웃이 **헤더까지만**
적용되던 문제, 호출자 `AbortSignal` 유실을 고쳤다(`src/lib/api.ts`, 변이 8/8).
...
crawler/image_assets.py     85%  ->   90%    JPEG 마커 순회 경계값
crawler/doc_paths.py        89%  ->   92%    형제 문서 재사용의 **거절** 조건
storage/database.py         93%             미실행 41줄 -> 37줄 (큐 claim 경쟁 분기 2곳)
```

`api/auth.py` 는 좁은 부분집합으로 재면 63% 로 보이는데 인증을 태우는 회귀 15개를
합치면 **100%** 다 - 잰 값과 재는 방법을 같이 적어야 한다는 사례.

### 결함이 아니었던 것 (재서 확인하고 잠갔다)

- **조건부 요청** 13가지 경우가 전부 RFC 9110 대로 동작했다. 미검증이었을 뿐이라
  17개 검사로 잠갔다(변이 7/7).
- **사진 크기 판독** 실제 인코더 출력 48건 / 운영 사진 45장 전부 일치. 바이트 수준
  경계값 11개로 잠갔다.
- **`api/v1/payments.py` 의 남은 미실행 줄 6곳** 은 전부 구조적으로 도달 불가능하다
  (근거는 `docs/SPRINT254_CLAIM_RACE_BRANCHES.md` §11 에 줄 단위로 남겼다).

### 문서 정정

- `docs/BUGS.md` #130 의 "변이 확인: 즉시 3건 FAIL" -> 실측 **15/17(약 88%)**.
  스레드 기반이라 **8번에 한 번꼴로 결함 있는 코드를 통과시킨다.** 방어선을 결정적
  검사로 옮기고 스레드 검사는 보조로 남겼다.

### 회귀 상태

```
python   53 PASS / 1 FAIL / 3 SKIP / 1 NO-VERDICT   단언 8,123건
node     191 tests / 187 PASS / 1 FAIL / 3 SKIP
tsc 0 / eslint 0
```

양쪽의 1 FAIL 은 같은 원인이다: **기일이 남은 물건 0건**(P0-A, 스케줄러 미등록 =
승인 영역). 운영 DB 는 세션 시작 시점과 동일하다
(`auction_item 1,876 / document_queue 3,498 / payments 0 / integrity_check ok / FK 0`).
