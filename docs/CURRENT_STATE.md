...
### 부분 적용 위험 ― 관측했고, 2026-08-27에 **고쳤다**

> 이 절은 원래 "위험은 있지만 발현된 적이 없으므로 코드는 바꾸지 않는다"로 끝나 있었다.
> 그 결론은 뒤집혔다. 아래에 옛 관측과 뒤집은 이유를 함께 남긴다.

`conn.executescript()`는 실행 전에 **암묵적으로 커밋**하고, 스크립트를 트랜잭션 **밖에서**
실행한다. 직접 재현했다.
...
#### 왜 "안 고친다"를 뒤집었나 (2026-08-27, 크롤->DB 경로 감사)

옛 사유는 세 가지였고 **셋 다 더는 성립하지 않는다.**

1. *"가드 없는 문장을 안 쓰면 된다"* → **쓸 수 없는 문장이 이미 있다.**
   SQLite 에는 `ALTER TABLE ... ADD COLUMN` 용 `IF NOT EXISTS` 가 **없다.**
   025 주석이 그 사실을 인정하면서 "정상 경로에서는 두 번 실행되지 않는다"에 기댔다 ―
   그런데 부분 적용이 바로 그 "비정상 경로"를 만든다.

2. *"지금까지 발현된 적이 없다(18개 전부 정상 적용)"* → 그 뒤 **021~025 가 들어왔다.**
   023/024 는 SQLite 의 제약 변경을 위해
   `CREATE _new` → `INSERT SELECT` → `DROP 원본` → `RENAME` 을 돈다.
   DROP 과 RENAME **사이**에서 죽으면 원본 테이블이 사라진 채 확정되고, 데이터는 `_new`
   라는 엉뚱한 이름 밑에 남는다. 재실행은 `INSERT ... SELECT FROM 원본` 에서
   `no such table` 로 죽는다. 대상이 `payment_webhooks` / `registry_credits` ― 결제 테이블이다.
   실측(`test_migration_atomicity.py` 2번):
...
3. *"영향이 부트스트랩 전체라 위험하다"* → 영향 범위는 그대로지만 **비용이 반대다.**
   러너가 raise 하면 `run_daily.bat` 3단계가 exit 1 이고, 그러면 **mvp_scraper.py 가 아예
   실행되지 않는다.** 한 번의 중간 실패가 매일 06:00 크롤을 영구히 세운다. 이 저장소가
   이미 한 번 겪은 "9일간 크롤 중단을 몰랐다"와 같은 계열이고, 이쪽은 회복 경로조차 없다.

#### 어떻게 고쳤나

SQLite 는 MySQL/Oracle 과 달리 **DDL 도 트랜잭션에 참여한다.** 그래서 파일 하나 +
`migration_history` INSERT 를 **한 트랜잭션**으로 묶는 것만으로 위 세 가지가 전부 사라진다.
묶으려면 `executescript()` 를 버리고 문장을 하나씩 `execute()` 해야 하므로 경계를 알아야 하는데,
그 판정은 **직접 하지 않는다** ― `sqlite3.complete_statement()` 가 SQLite 자신의 토크나이저라
주석 안의 세미콜론(`-- 경계가 아니다;`)과 문자열 리터럴 안의 세미콜론(`DEFAULT 'a;b'`)을
정확히 건너뛴다. 옛 사유가 걱정한 "SQL 안의 세미콜론 처리"가 바로 이 지점이고,
정규식으로 쪼갰다면 실제로 위험했다(013/016/023 에 여러 줄 CREATE TABLE 이 있다).

측정으로 확인한 결과:

```
실제 .sql 25개 -> 143문장으로 분해, 전부 complete_statement() 통과, 주석뿐인 조각 0개
빈 DB 부트스트랩(init_db -> migrate_v4_1 -> 마이그레이션 25개)  119ms, 26테이블, 미적용 0
라이브 사본에 021~025 적용                                        90ms 후 migrate_execute OK
```

검사도 반대로 뒤집었다 ― `test_schema_hygiene.py` §7 은 이제 "실패한 파일은 앞 문장까지
통째로 롤백된다"와 "**고친 뒤 재실행이 성공한다**"를 고정한다. 후자가 이 수정으로 얻은
실제 가치다. 별도 회귀는 `test_migration_atomicity.py`(4묶음)에 있고, 고치기 전 코드에서
**9건 실패**하는 것을 확인한 뒤 도입했다(검출력 확인).
...
DOJOONPASS_DAILY (수동 등록, register_scheduler_tasks.ps1이 모르는 이름)
  ...
  ★ 2026-08-18 Sprint 189 정정: 이 작업은 **현재 존재하지 않는다.** 전체 예약 작업 249개
    중 이 저장소를 가리키는 것이 0개다(실측). logs/daily_run.log 마지막 기록 2026-08-11,
    auction.crawl_date 최신 2026-08-12 — 세 근거가 맞아떨어진다. BUGS #123 참고.
...
```
★ 2026-08-18 Sprint 189 정정: 1번은 **완료됐다**(2026-08-17T09:03:19 적용, BUGS #117 해결).
  2번은 여전히 미완이고, 게다가 위에서 "이미 있다"고 본 DailyCrawl까지 지금은 없어서
  **세 작업 전부 신규 등록 대상**이다(BUGS #123).
...
### Sprint 189 — 변경 기반 재수집 트리거 (2026-08-18, 상세는 `docs/SPRINT189_CHANGE_DRIVEN_REFRESH.md`)

Sprint 186/187이 "기계는 완성됐는데 **아무도 `overwrite=True`를 넘기지 않는다**"로 남겨
둔 마지막 한 칸을 채웠다. 재수집 범위는 그때 "제품 결정"으로 SKIP했지만, 이번 목표가
그 결정을 명시했다 — **법원이 바꾸면 다음 주기에 따라 간다.**

**만든 것**

```
어휘 확장    document_queue.status 에 refresh / in_progress_refresh 추가
             (TEXT + CHECK 없음 -> 값 추가는 스키마 변경이 아니다. 승인 불필요)
트리거       migrate_execute() 가 필드 단위 변경을 관측한 물건을 그대로
             requeue_changed_documents() 에 넘긴다 (Sprint 185의 "관측"을 "행동"으로)
매핑         REFRESH_DOC_TYPES_BY_FIELD 하나가 유일한 정의처
             기일/최저가/상태 -> spec·status,  감정가 -> appraisal·image
             (사진은 유찰로 값만 내려갈 때 안 바뀐다 -> 매일 수천 장 재수집 방지)
배선         claim_next_queue_item() 이 overwrite 를 계산해 넘기고
             doc_worker 가 collect_document(..., overwrite=) 로 그대로 전달
보존         재시도/stale 회수가 재수집 의도를 **각자 제자리로** 되돌린다
             (하나로 합치면 첫 실패에서 의도가 조용히 사라진다)
상한         REFRESH_MAX_ITEMS_PER_RUN=300, 잘린 건수는 로그+반환값에 남긴다
스위치       DOJOONPASS_REFRESH_ON_CHANGE=0 으로 끌 수 있다(기본 켬)
```

**실측 (실제 `auction.db` 사본, 운영 DB는 읽기만)**

```
2024타경126346-1  최저가 134,144,000 -> 146,489,678 / 기일 08-19 -> 09-30 (유찰 -> 재매각)
  -> 관측 {'auction_date': 1, 'minimum_bid_price': 1}
  -> 예약 {'items': 1, 'refreshed': 1, 'revived_expired': 0, 'skipped_over_cap': 0}
  -> 큐   ('status','done') -> ('status','refresh')   나머지 3,497행 무변경
```

**이번에 고친 결함 3건** (전부 재수집을 켜는 순간 도달하는 경로)

- **BUGS #120 — 사진 형식이 바뀌면 옛 파일이 고아로 남고 지문 공식이 갈라진다.**
  파일명이 `<순번>.<확장자>`라 JPEG -> PNG 교체가 `01.jpg` 옆에 `01.png`를 만든다.
  그러면 `_existing_set_hash()`가 같은 순번을 두 번 세어 수집 쪽 공식과 갈라지고,
  **이후 매 수집이 거짓 개정**이 된다 — Sprint 186이 docstring에 적어 둔 바로 그 경고가
  형식 변경 한 번으로 영구히 현실이 된다. 쓰기 성공 직후 옛 확장자를 정리하고,
  지문 쪽도 중복 순번을 보면 비교를 포기하도록 이중으로 막았다.
- **BUGS #121 — 기존 PDF를 덮어쓸 때만 저장이 비원자적이 된다.** Windows에서
  `shutil.move()`는 목적지가 있으면 `os.rename()` 실패를 잡아 **조용히 copy2 폴백**한다
  (실측: 목적지 없음 -> RENAME / 있음 -> COPY). 잘린 PDF가 남으면 `doc_exists()`가
  완성본으로 취급해 영구히 굳는다. `collect_documents.py`/`image_crawler.py`는 이미
  `os.replace()`를 쓰고 있었고 **두 수집기만 빠져 있었다**(#113/#115 계열의 네 번째).
  `move_into_place()` 신설 + **AST 전수 가드**(문자열 grep은 결함을 설명하는 산문을
  코드로 오판해 못 쓴다 — 실제로 그렇게 걸렸다).
- **BUGS #122 — 재수집 최종 실패가 이미 보여 주던 문서를 "수집실패"로 뒤집는다.**
  이것은 기존 결함이 아니라 **이번 Sprint가 도입할 뻔한** 결함이다. 화면은
  "수집실패"인데 파일 서빙은 200으로 옛 문서를 내려 주는, BUGS #50 계열의 어긋남이다.
  최종 실패 시 화면 상태가 READY/NO_IMAGE면 그대로 유지한다(큐는 failed로 남아
  실패 사실은 유실되지 않는다).

**★★ 이번 감사에서 나온 가장 큰 발견 — 예약 작업이 0개다 (BUGS #123, Release Blocker)**

Sprint 187이 "DOJOONPASS_DAILY가 매일 03:00에 정상 동작 중(2026-08-17 exit 0)"이라고
적어 둔 그 작업이 **이 환경에 존재하지 않는다.** 직접 조회한 실측:

```
전체 예약 작업                              249개
그중 이 저장소를 가리키는 것                  0개
register_scheduler_tasks.ps1 (dry-run)      3개 전부 "(신규)"
...
네 근거가 서로 맞아떨어진다. **2026-08-20부터 검색 결과가 0건이 된다.**
등록은 승인 영역이라 SKIP하며, 스크립트와 선행 조건은 이미 확인해 두었다
(`.\register_scheduler_tasks.ps1 -Apply` 한 줄).

**BUGS #117(마이그레이션 020 미적용)은 해결됐다.** 2026-08-17T09:03:19에 적용돼 있고
(`migration_history` 20행), 실제 API 재확인 결과 `/search` 200 / `/item/505` 200 /
사진 5장 200 / 문서 3종 200 / `If-None-Match` -> 304다. `Cache-Control`이 없어
브라우저가 항상 재검증하므로 **캐시가 최신화를 막지 않는다**(프런트도 전 경로
`cache: 'no-store'`).

**테스트**: 신규 `test_refresh_trigger.py`(12절 74검사) + 기존 3종 증분 21검사.
전체 **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT**(단언 4,649 -> 5,038).
프런트 113/113, tsc 0, eslint 0. 전수 가드는 **변이를 주입해 실제로 FAIL하는지 확인**한
뒤 복원했다(항상 통과하는 검사와 결함이 없어서 통과하는 검사를 구분하기 위해).


### Sprint 189 이어서 — 재수집이 "정확히 개정만 기록하고, 안 바뀐 것은 건드리지 않는가"

트리거를 붙인 뒤 그 다음 질문을 따라갔다. **켜 놓고 보니 두 가지가 더 틀려 있었다.**

- **BUGS #124 — 현황조사서 지문이 우리가 찍은 수집 시각 때문에 매번 달라진다.**
  `status.json`에는 `extracted_at`(수집 시각)이 들어 있는데 변경 감지 지문을 그 파일
  **전체**에서 떴다. 즉 법원 자료가 하나도 안 바뀌어도 두 지문이 항상 다르다 —
  재수집을 켜면 `document_version_log`가 **거짓 개정으로 가득 차고**
  `doc_raw.doc_version`이 매일 오른다(`api/v1/item.py`가 그 값을 사용자에게 그대로
  싣는다). 이미지 #113/#120과 **같은 결과를 다른 경로로** 만들고 있었다.
  지문의 근거를 파일이 아니라 **내용(`fields`)** 으로 바꿨고, 디스크 쪽과 수집 쪽이
  같은 함수를 쓰게 했다. 형제 재사용 경로도 함께.
  이 저장소는 원인을 이미 알고 있었다 — Sprint 145 주석이 *"차이는 우리가 찍는
  extracted_at 하나뿐"*이라고 실측해 적어 두었다. **그 관찰이 변경 감지 쪽으로
  연결되지 않았을 뿐이다.**

- **BUGS #125 — 내용이 하나도 안 바뀐 재수집이 모든 브라우저 캐시를 무효화한다.**
  같은 바이트를 다시 써도 mtime이 바뀌고, ETag는 (mtime, size)로 만들어진다.
  `api/http_cache.py`가 304로 아끼려던 바이트를 재수집이 매번 되살리는 셈이다
  (검색 1페이지 약 2MB / 물건당 사진 1.3~1.9MB / 감정평가서 3.4MB).
  세 저장 경로(사진 / status html+json / spec·appraisal PDF) 전부 **바이트 지문이
  같으면 쓰지 않는다**로 바꿨다. 목표 문서의 상황 A가 정확히 이것이다.

- **BUGS #126 — 테스트 정리의 두 호출 지점 중 하나만 강건했다.** OneDrive가
  `documents/` 아래에 붙이는 R 속성 때문에 맨 `shutil.rmtree()`가 실패하는데,
  그 방어(`_force_rmtree`)를 **잔해 정리에만 쓰고 자기 디렉터리에는 안 쓰고 있었다.**
  한 번 실패하면 지우지 못한 디렉터리가 남아 **그 뒤 모든 실행이 같은 자리에서 죽는다**
  (실측 6벌 누적). 작업 중 실제로 이 연쇄를 겪었다. 호출 지점을 하나로 합쳤다.

**검사 설계에서 배운 것**: "썼다/안 썼다"를 mtime으로만 단언하면 안 된다.
**"안 썼다"는 mtime으로 확실히 말할 수 있지만, "썼다"는 말할 수 없다** — 두 쓰기가
파일시스템의 타임스탬프 갱신 간격보다 가까우면 같은 값이 나온다(실제 플레이크를
겪고 나서 내용 비교로 바꿨다).

**전체 재검증**: 40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 **5,084개**
(Sprint 189 시작 시점 4,649 -> +435). 연속 2회 동일. `documents/` 잔해 0.


### Sprint 190 — 매일 갱신 체인 실측 감사 (상세는 `docs/SPRINT190_REFRESH_CHAIN_AUDIT.md`)

Sprint 189가 **코드로** 체인을 완성했으니, 이 Sprint는 그 체인이 **실제 환경에서**
어디까지 서 있는지를 쟀다. 결론: **배관은 전부 살아 있고, 아무도 밸브를 안 열었다.**

**★ 가장 중요한 실측 — 크롤러가 아직 법원 원천을 읽는다**

`auction.crawl_date`가 2026-08-12 이후 0건인 원인이 예약 작업 미등록(BUGS #123)인 것은
확인했지만, **그것만으로는 크롤러 자체의 생존을 알 수 없었다** — 그 사이 법원 DOM이
바뀌었다면 예약 작업을 등록해도 0건이다. 한 법원만, **DB에 쓰지 않고** 확인했다.

```
서울중앙지방법원 (60곳 중 1곳)   수집 9건 / 186.2초
검증 PASS 9 / FAIL 0 / 정확도 100.0%
핵심 필드(사건·물건·법원·기일·최저가·감정가·상태·주소) 전부 채워짐
실행 후 DB 무변경 확인 (auction 1,876행 / max(crawl_date) 2026-08-12 / 큐 분포 그대로)
```

파생: 1곳 186초 x 60곳 ≈ **전체 크롤 3.1시간**. 06:00 시작이면 09:00 전후 종료이고
문서 수집 창은 다음 날 02:00이므로, **법원 변경 -> 문서/사진 반영까지 약 17시간**이
설계상의 지연이다(파이프라인 순서는 `docs/CLAUDE.md` 그대로, 바꿀 이유 없음).

**성능 — 어휘 확장이 비용을 바꾸지 않았다**

```
claim  이전(= 'pending') 0.30 ms  /  현재(IN 2값) 0.30 ms   (200회 평균, 계획도 동일)
재수집 예약  물건 300 x 4종 = UPDATE 2,400회 -> 0.005초 (물건당 0.02 ms)
우선순위 재계산  2,753행 검토 0.021초, 실제 변경 17행
```

`USE TEMP B-TREE FOR ORDER BY`는 여전히 남지만 1회 0.30ms라 인덱스를 추가하지 않는다
(roadmap의 기존 결정과 같은 판단, 스키마 변경은 승인 영역이기도 하다).

**어휘 확장 파급 전수 확인** — `document_queue.status`를 세거나 분기하는 곳을
`git ls-files` 기준으로 전부 훑었다. 고칠 것은 둘뿐이었고(`doc_stats.py`,
`measure_endless_collecting.py`) 나머지 5곳은 상태 무관하거나 이미 위임 구조였다.

**신규 코드 죽은 것 0** — Sprint 189가 추가한 함수 8개·상수 14개 전부 참조된다.
이 저장소가 반복해 겪은 "준비만 되고 배선 안 됨"을 만든 그 Sprint 안에서 세어 둔다.

**TODO/FIXME/HACK 전수 3건** — 전부 `SearchForm.tsx`의 미지원 검색 필터이고
스키마 변경(승인 영역)이다. `test_search.py`가 "백엔드에 그 이름이 생기면 실패"로
고정해 두어 방치가 아니라 **관리되는 부채**다.

**문서 드리프트 4건 정정** — 서로 다른 세션의 기록이 충돌했고, 다시 재서 판정했다.
Sprint 187의 "DOJOONPASS_DAILY 정상 동작 중"과 roadmap의 "등록 0건은 사실이 아니다"가
**둘 다 틀렸고**, 오래된 `BETA_RELEASE_CHECKLIST.md` P0-A의 "0개"가 맞았다.
`BUGS.md` #117은 해결로 정정했다. 그리고 체크리스트 43행이 셸 이스케이프 사고로
깨져 있던 것(`` `.<개행>egister_...` ``)을 고치고, 추적 파일 315개를 같은 패턴으로
전수 검색해 잔여 0건을 확인했다.


**★★ Sprint 190 마지막 실측 — 실제 브라우저로 재수집 1건 완주**

큐가 `refresh` 로 바뀌는 것까지는 앞에서 확인했고, 그 **뒤**(워커가 법원에 다시 가서
받아 오고 → 무변경을 판정하고 → 기록을 남기거나 남기지 않는 부분)를 한 번 완주시켰다.
DB 사본 + 스크래치 문서 루트로 격리했고, 법원 부하는 물건 1건 x 문서 1종이다.

```
[INFO] 법원 선택: 서울중앙지방법원                              <- 실제 사이트 진입
[INFO] [2024타경126346-1] status 내용 무변경 - 파일을 다시 쓰지 않는다(브라우저 캐시 보존)
[INFO] doc_raw 기록 생략: 내용 변경 없음 (version=1 유지)
[INFO] [2024타경126346-1] status 처리 성공 (재수집)
종료코드 0 / 27.2초 | 큐 refresh -> done | version_log 0->0 | doc_raw 556->556
```

**재수집이 실제로 일어났는데 아무것도 거짓으로 기록되지 않았다.**
Sprint 189 이전 코드였다면 같은 실행이 거짓 개정 1행 + `doc_version` 1->2 +
파일 재작성(캐시 전량 무효화)을 남겼을 것이다 — 세 가지가 전부 일어나지 않았다.

실행 후 실 데이터 무변경 확인: `auction` 1,876행 / 큐 분포 그대로 /
`document_version_log` 0 / `doc_raw` 556 / 실제 문서 mtime 2026-08-12 14:45 그대로.

**추가 회귀** (전체 40 PASSED / 0 FAILED, 단언 **5,145개**):
- `test_refresh_trigger.py` §13 — 매일 06:00 `enqueue_documents()` 가 대기 중인
  `refresh` 를 덮지 않는다. 모든 refresh 행은 09:00 예약 -> 다음 날 02:00 소진 사이에
  **반드시 enqueue 를 한 번 통과**하므로, 여기서 상태가 덮이면 재수집은 실행되기도 전에
  사라진다. 기일이 바뀌는 경우/안 바뀌는 경우 둘 다 본다.
- `test_refresh_trigger.py` §14 — `GET /api/v1/document-stats` 를 **스크래치 DB 에
  네 상태를 전부 심고 실제로 호출**해 집계를 맞춘다. 적재한 모든 행이 어느 칸엔가
  잡히는지까지 확인한다.
- `test_api_regression.py` 의 같은 검사가 **틀렸는데도 통과하고 있었다** —
  `queue_in_progress` 를 `in_progress` 하나로만 세고 있어서, 실 DB 에 아직
  `in_progress_refresh` 행이 없는 동안만 우연히 맞았다(BUGS #119 계열).
  단일 소스 참조로 고치고 `queue_refresh` 검사를 추가했다.


**Sprint 190 마지막 — 사진 쪽도 실제 브라우저로 완주, 그리고 로그 거짓말 1건**

문서(status)에 이어 사진도 완주시켰다(수집 방식이 근본적으로 다르므로 따로 확인해야 한다).

```
2024타경3528-1 (사진 5장, 기일 2026-08-19) / 큐 image refresh -> done / 21.4초
5장 전부 크기 동일, mtime 그대로(캐시 보존) | version_log 0->0 | auction_image 5->5 | READY
```

이 표본은 **한 물건 안에 jpg 4장 + gif 1장**이 섞여 있다 — 확장자가 장마다 다를 수 있다는
실증이고, BUGS #120(형식 변경 시 고아 파일)이 가정한 상황이 실제 데이터에 있음을 뒷받침한다.

**그 실행에서 잡은 것**: 완료 로그가 `사진 5장 저장 완료`를 **한 장도 쓰지 않았는데도**
찍고 있었다(BUGS #125 스킵을 넣으며 문구를 같이 안 고친 결과). BUGS #47 이래 반복해 잡아 온
"로그가 거짓을 말한다" 부류이고, **단위 검사로는 안 보인다**(반환값만 보고 로그 문구는
안 보므로). 문구를 사실대로 바꾸고 `written`/`unchanged` 를 반환값에도 담아
로그 파싱 없이 검증되게 했다(`test_asset_pipeline.py` 5-F +3검사).

최종: 전체 **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,148개**
(Sprint 189 시작 4,649 -> **+499**). 프런트 113/113, tsc 0, eslint 0.


### Sprint 191 — 자산 3근거 불변식 + 큐 경쟁 정직성 (상세는 `docs/SPRINT191_ASSET_INVARIANT_AND_QUEUE_RACE.md`)

Sprint 189가 재수집을 켜고 190이 실측했으니, 이번에는 **켜 놓고 나서야 도달 가능해진
나머지 경로**를 계열 전수 검색으로 훑었다. **같은 결함이 네 군데 더 있었다.**

이 저장소의 사진 결함은 전부 한 문장이다 — *"근거 둘이 갈라졌다."*

```
#113 수집 결과엔 지문이 있는데 비교 대상(디스크)을 안 봤다
#114 디스크는 줄었는데 DB를 지웠다(부분 수집인데)
#120 형식이 바뀌자 디스크에 순번이 둘이 됐다
#127 DB는 줄었는데 디스크가 안 줄었다              <- 이번
#128 법원은 0장인데 DB/디스크가 그대로였다          <- 이번
#129 완료 기준은 json 인데 서빙은 html 이었다       <- 이번(문서)
```

**BUGS #127 — 사진이 줄어도 파일은 안 줄었다.** `save_auction_images()`는 DB 행만
지운다. 재현: 5장 -> 3장 재수집 후 디스크에 5개가 그대로 남고 `previous_hash`(5장 기준)
!= `new_hash`(3장 기준)가 영구히 성립했다 — **#120과 완전히 같은 실패 방식**이다.
`_remove_files_not_in()` 신설(부분 수집이면 절대 안 지운다) + DB 행 삭제를
`seq > max_seq` -> **집합 차집합**으로(법원이 **가운데 순번**을 빼는 경우를 `>` 비교는
못 잡아 그 행이 살아남고 파일은 이미 없다).
검사도 고쳤다 — §11이 `save_auction_images()`를 **누적 빌더처럼** 쓰며 옛 semantics를
굳히고 있었다("이번에 안 준 순번의 행을 남긴다"가 참이어야 통과하는 검사였다).

**BUGS #128 — 법원이 사진을 전부 내려도 사용자는 영원히 그 사진을 본다.**
0장 케이스만 `save_auction_images()`에 도달하지 않는다(`doc_worker`의
`if result.get("images")` 가드 — 빈 목록은 전체 실패와 구별 안 되니 **그 가드는 옳다**).
결과: `document_status`만 NO_IMAGE가 되고 행/파일은 그대로, `_images_status()`는
"행이 있으면 무조건 READY"라 **READY**를 답한다.
**두 번 연속 확인** 규칙으로 고쳤다 — 1회차는 남기고(경고), 2회차에 정리한다.
새 컬럼 없이 `document_status` 자체가 1회차를 기억한다. 순서는 DB 행 -> 파일.

**BUGS #129 — 완료 기준과 서빙 대상이 서로 다른 파일이었다.**
`doc_exists(status)`는 `status.json`만 보는데 뷰어는 `status.html`을 서빙한다.
json만 남으면 "완료"로 오판해 영구히 재수집에서 빠지고 뷰어는 404다.
실측 결과 실데이터 json-only **0건**(html+json 163건)이라 지금 터지는 버그는 아니지만,
**정의가 두 벌인 상태 자체**를 없앴다(`DOC_REQUIRED_FILES` 신설 + 서빙 표에서 파일명을
읽어 대조하는 구조적 가드).

**BUGS #130 — 큐 경쟁에서 진 것을 "큐 비었음"으로 오해해 실행이 조기 종료된다.**
`claim_next_queue_item()`이 "가져갈 것이 없다"와 "경쟁에서 밀렸다"에 똑같이 `None`을
돌려주고, `doc_worker`는 그것을 종료 신호로 읽었다. 실측(스레드 12 / 대기 행 4):
**중복 claim은 원래 0건**(원자적 클레임 정상)이었지만 claim 성공이 3건뿐이고 행이 남은
채 None을 받은 스레드가 9개였다. 수정 후 4건 전부 claim. 로그의 `대기열 비어있음`도
사실이 아니었다(BUGS #47 계열).

**★ 방식을 바꿨다 — 인스턴스 대신 불변식.** `#120`을 고칠 때 `#127`을 놓친 이유가
"개별 검사를 하나씩 늘렸기 때문"이라, `test_asset_pipeline.py` 5-I에 **시나리오 표 +
불변식**을 넣었다:

```
완전 수집이면  set(디스크) == set(수집 결과) == set(DB)
부분 수집이면  디스크/DB 는 줄어들지 않는다
언제나        DB 가 가리키는 파일은 실제로 존재한다
```

시나리오 8종(신규/동일/형식변경/추가/가운데삭제/뒤삭제/부분수집/재완전수집)을 돌린다.
새 시나리오는 **표에 한 줄**만 추가하면 된다.

**가드 2종 전부 변이 확인** — 5-I는 `_remove_files_not_in()` 무력화 시 4건 FAIL,
§15는 `CLAIM_RACE_MAX_ATTEMPTS=1`로 되돌리면 3건 FAIL. 복원 후 전부 PASS.

**콘솔 인코딩 가드가 내 실수를 잡았다** — 새 검사 제목의 EM DASH가 cp949 콘솔에서
`UnicodeEncodeError`를 내고, 그러면 **판정문에 `[FAIL]`이 하나도 없이 종료 코드만 1**이
된다(`run_python_tests.py`가 종료 코드를 1순위로 쓰는 이유가 또 증명됐다).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,359개**
(Sprint 190 종료 5,148 -> **+211**).


### Sprint 192 — 삭제 경로 봉쇄 + 변경 감지 커버리지 증명

**BUGS #131 — DB 값으로 파일을 지우는데 경로 탈출 방어가 없었다.**
Sprint 191 이 만든 `remove_stored_image_files()` 는 `auction_image.storage_path` 를
지운다. 같은 출처에 대해 **서빙 쪽(`api/v1/images.py`)은 이미 봉쇄를 하고 있었고,
더 위험한 삭제 쪽에만 없었다.** 실측: 수정 전에는 `..` 탈출 경로로 `documents/` 밖
파일이 실제로 지워졌다. `is_inside_document_root()` 신설로 막았다.

**계열 전수**: 소스의 삭제 지점 **8곳**을 AST 로 분류한 결과 **DB 값을 지우는 곳은
그 하나뿐**이었다(나머지는 다운로드 폴더/`image_path()`/`*.tmp`/상수 — 전부 코드가
구성한 경로). 전수 검색 자체를 회귀에 넣어 새 삭제 지점이 생기면 드러나게 했다.

**★ 목표가 요구한 "유찰횟수 변경 감지"를 구조적으로 증명했다.**
`migrate_execute()` 가 실제로 비교하는 것은 넷이다(기일/최저가/상태/감정가).
`fail_count`(유찰횟수)와 `bid_rate` 는 목록에 없는데 **빠뜨린 것이 아니라 파생 필드**다:

```
fail_count = extract_fail_count(status)                     <- 입력이 관측 대상
bid_rate   = calc_bid_rate(appraisal_price, minimum_bid_price)  <- 입력이 관측 대상
```

즉 **입력이 안 바뀌면 절대 안 바뀐다.** `test_refresh_trigger.py` §16 이 그 논증을
코드로 고정한다 — 관측 목록을 소스에서 읽어 확인하고, 파생 함수의 **시그니처가 관측
대상만 받는지**, 그리고 **결정적인지**까지 본다. 파생 관계가 깨지면(예: fail_count 가
다른 입력을 받게 되면) 여기서 실패하고, 그때는 관측 목록에 추가해야 한다는 뜻이다.
마지막으로 관측 필드 4개가 전부 재수집 매핑에 연결돼 있는지도 확인한다
(관측만 하고 행동이 없으면 무의미하다 — Sprint 185 가 그 상태였다).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,387개**
(Sprint 191 종료 5,359 -> +28).


**Sprint 192 이어서 — Data Integrity Audit 실측 GREEN + 감사기 신설**

로직은 `test_asset_pipeline.py` 5-I 가 지키지만, **운영 데이터**는 과거의 결함·수동
조작·중단된 실행 때문에 어긋나 있을 수 있다. 그 어긋남은 조용하다(화면은 READY 인데
열면 404, 아무도 안 보는 파일이 디스크를 먹는다). 실 DB 로 전수 확인했다 — **읽기 전용**.

```
[1] auction_image -> 파일             45행 / 파일 없는 행 0
[2] 디스크 <-> auction_image 집합      어긋난 물건 0 / 고아 파일 0
[3] document_status READY -> 필요 파일  556개 / 모자란 것 0   <- #129의 엄격해진 기준으로도 0
[4] doc_raw -> 파일                    556행 / 파일 없는 행 0
[5] 큐 <-> 화면 상태                   어긋남 없음
VERDICT: GREEN
```

`#129`가 STATUS 완료 기준을 **html+json 둘 다**로 좁혔는데도 [3]이 0이라는 것이
중요하다 — 기존 READY 556건이 전부 새 기준을 만족한다(회귀 없음).

**`audit_asset_integrity.py` 신설** (읽기 전용, `*_dryrun.py` 관례). 종료 코드로
판단한다(0=정상 / 1=어긋남 / 2=실행 실패). **절대 고치지 않는다** — 무엇을 지우고
무엇을 다시 받을지는 상황마다 다르고 잘못 지우면 되돌릴 수 없다.

**감사기가 눈이 멀지 않았는지 스스로 확인한다** (`--selftest`). 감사 스크립트는
조용히 아무것도 안 보게 되기 쉽고, 그 상태는 정상과 겉으로 완전히 같다. 사본에 결함을
심어 잡히는지 본다(운영 DB 무변경). 실행: 기준선 0 / 파일 없는 사진 행 검출 /
READY 인데 파일 없는 문서 검출 — 전부 PASS.

**★ 이 과정에서 가드가 내 실수를 두 번 잡았다.**

1. 처음에는 감사기 자체 검사를 `test_asset_pipeline.py` 에 넣었다.
   `test_schema_hygiene.py` 의 **"추적 파일이 미추적 파일을 import하지 않는다"** 가
   즉시 잡았다 — `audit_asset_integrity.py` 는 아직 미추적이고, 그 상태로 커밋하면
   `ModuleNotFoundError` 로 부팅이 깨진다(BUGS #105와 같은 상태). `git add` 는 승인
   영역이라, 가드를 우회하지 않고 **의존 자체를 없앴다**(자체 검사를 감사기 안으로 이동).
2. 감사기가 `from storage.database import DB_PATH` 로 경로를 **복사**해 오고 있었다.
   그러면 테스트가 스크래치 DB 를 가리켜도 **실 DB 를 읽는다** — 실제로 그렇게 한 번
   틀렸고 감사 결과가 통째로 무의미했다. 모듈을 통해 매번 읽도록 고쳤다.

**승인 SKIP 추가**: `git add audit_asset_integrity.py`.
추적되기 전까지 이 파일은 회귀 스위트가 참조할 수 없다(위 1번 이유).
추적되면 `--selftest` 를 회귀 스위트로 옮기는 것이 맞다.


### Sprint 193 — 운영 데이터 무결성 감사 (상세는 `docs/SPRINT193_ASSET_INTEGRITY_AUDIT.md`)

Sprint 191 이 **로직**에 3근거 불변식을 걸었으니, 이번에는 **운영 데이터**를 쟀다.
`audit_asset_integrity.py` 를 일곱 검사로 완성하고 실측했다 — 양방향으로 본다.

```
[1] auction_image -> 파일      45행 / 없는 파일 0
[2] 디스크 <-> auction_image   어긋난 물건 0 / 고아 파일 0
[3] READY -> 필요 파일         556개 / 모자란 것 0   <- #129 로 좁힌 기준에서도 0
[4] doc_raw -> 파일            556행 / 없는 파일 0
[5] 큐 <-> 화면 상태            어긋남 없음
[6] 디스크 -> DB (문서)        대응 물건 없는 디렉터리 1개 / 빈 디렉터리 1,674개
[7] 큐 -> 물건                 고아 큐 18행 (그중 수집을 시도할 대기 행 12개)
```

[1]~[5] GREEN. [6][7] 이 드러낸 것은 **같은 사건번호가 두 법원에 있는 잔재**다:
`고양지원/2024타경2803`(문서 12.7MB + 큐 done, **물건 없음**) vs
`춘천지방법원/2024타경2803-1`(실제 물건, 큐 pending). 법원을 식별키에 넣는 문제
(BUGS #14/#18/#103/#107)의 잔재이고, **지금 코드는 법원 포함 키를 쓰므로 새로 생기지
않는다.** 사용자 피해도 없다(서빙은 `auction_item` 기준). 정리는 승인 영역.

**★ 이번 Sprint 에서 내가 만든 결함 셋을 전부 기존 방어가 잡았다.**

1. **추적 파일이 미추적 파일을 import** — 감사기 자체 검사를 회귀 스위트에 넣었더니
   `test_schema_hygiene.py` 가 즉시 잡았다(그대로 커밋하면 BUGS #105 재발).
   `git add` 는 승인 영역이라 **가드를 우회하지 않고 의존을 없앴다**(자체 검사를
   감사기 안 `--selftest` 로 이동).
2. **DB 경로를 복사해 옴** — `from storage.database import DB_PATH` 는 스냅숏이라
   자체 검사가 스크래치를 가리켜도 **실 DB 를 읽었다**. 매번 읽도록 고쳤다.
3. **정규화를 빼먹어 거짓 경보 36건** — 디렉터리 이름은 `sanitize_path_segment()` 를
   거친 값인데 DB 원본과 그대로 비교했다(실데이터에 `2008타경25092 / 2015타경19958`
   같은 중복사건이 있다). 37건 -> 1건. **감사기의 거짓 경보는 아무 경보도 없는 것만큼
   나쁘다** — 규칙을 베끼지 말고 쓰는 쪽과 같은 함수를 쓴다(Sprint 146 의 교훈).


### Sprint 194 — 매일 크롤의 동시 실행 방지 (BUGS #132)

목표가 명시한 "중복 실행 방지"를 배치별로 대조했더니, `doc_worker.py` 에는 락이 있고
**매일 크롤(`mvp_scraper.py`)에는 없었다.** 이 배치가 건드리는 공유 자원:

```
logs/checkpoint.json   전체를 읽어 고쳐 쓴다 -> 겹치면 진행 상황이 서로 덮인다
법원 서버              전체 크롤 약 3.1시간(Sprint 190 파생) -> 겹칠 창이 넓다
```

예약 작업끼리는 `MultipleInstances=IgnoreNew` 로 안 겹치지만 **수동 실행과 스케줄
실행이 겹치는 경우**는 막지 못한다 — doc_worker 가 락을 둔 것과 같은 이유다.

구현을 **베끼지 않고** `storage/checkpoint.py` 의 `RunLock` 으로 올렸다.
`CheckpointManager`(어디까지 했는가) / `RunLock`(누가 하고 있는가)은 같은 요구에서
나왔고 같은 규율(원자적 쓰기 + 시간 기반 죽은 소유자 판정)을 쓴다.

**실제로 `main()` 을 돌려 고정했다** — 정상(잡고/돌고/놓는다) / 락 충돌(아무것도 안 하고
exit 0, **남의 락을 지우지 않는다**) / 예외(전파되되 락은 놓인다).
진 쪽이 남의 락을 지우면 **먼저 돌던 실행이 무방비가 된다** — 그래서 그 검사가 핵심이다.

**★ 가드가 또 막았다.** 처음엔 `storage/runlock.py` 를 새로 만들었는데
`test_schema_hygiene.py` 의 "추적 파일이 미추적 파일을 import하지 않는다" 가 잡았다 —
그대로 커밋하면 **매일 크롤과 워커가 둘 다** `ModuleNotFoundError` 로 죽는다(BUGS #105).
`git add` 는 승인 영역이라 **이미 추적된 모듈로 옮겼다.** 이번 세션에서 이 계열 가드가
막아 준 것이 네 번째다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,404개**.


### Sprint 195/196 — 가드 사각지대 + 재수집 처리량 근거 (상세는 `docs/SPRINT195_GUARD_BLINDSPOT_AND_CAPACITY.md`)

이번에는 **내가 만든 가드들을 감사**하고, **근거 없이 정해져 있던 숫자에 근거를** 만들었다.

**BUGS #133 — 전수 가드가 BOM 파일 16개를 조용히 건너뛰고 있었다.**
Sprint 191/192 가 추가한 AST 가드 둘이 `encoding="utf-8"` 로 읽고
`except SyntaxError: continue` 했다. 이 저장소 소스 70개에 BOM 이 있어 그 파일들이
감사에서 통째로 빠졌다 — 스캔 범위 안에서만 16개, 그중 `crawler/image_crawler.py` 에는
**실제 삭제 지점이 3곳** 있었다(가드는 "8곳"이라 했지만 진짜는 **12곳**).
**기존 가드 7개는 전부 `utf-8-sig` 를 쓰고 있었다** — 규칙이 이미 있었는데 새 코드가
안 따랐다. 인코딩을 고치고, **못 본 파일이 있으면 그 사실을 단언**하도록 바꿨다
("결함이 없어서 통과"와 "안 봐서 통과"는 겉으로 같다).

**BUGS #134 — `REFRESH_MAX_ITEMS_PER_RUN = 300` 이 실행 창의 400% 였다.**
실측: 창 7,200초 / 최악 소요 28,800초(8.0시간). 최악 기준 안전선은 69 물건이고
여유를 둬 **60**으로 정했다(최악 5,760초 = 80%). 값만 바꾸지 않고 **산술을 고정**했다 —
`test_refresh_trigger.py` §17 이 `상한 x 물건당 최악 행 x 행당 초 <= 실행 창`을 검사한다.

**이미지 재수집 트리거 실측** (★ 아래 결론은 Sprint 198 에서 절반 정정됐다 —
연속 비교의 0건은 **관측 공백**이었고, 물건별 처음<->마지막으로 재니 44물건이 바뀐다.
문서 재수집은 실제로 발동하고, 사진 재수집만 발동하지 않는다.
`docs/SPRINT198_TRIGGER_OBSERVABILITY_CORRECTION.md` 참고)
CSV 스냅숏 25개(7/02~8/12)의 연속 diff, 공통 물건 **4,455건 비교 → 변경 0건**.
비교가 무의미한 것이 아니다(공통 120건의 status 는 '유찰 10회'~'유찰 19회'로 다양).
원인은 `MAX_ITEMS=10`(법원당) — 물건이 유찰되어 기일이 바뀔 즈음엔 이미 상위 10건
슬라이스에서 빠져 있다(스냅숏 간 이탈 163 / 신규 158). **트리거는 구조적으로 옳지만
관측 기회 자체가 적다.** 트리거 확대도 `MAX_ITEMS` 조정도 제품/용량 판단이라 SKIP.

**★ 조사 도구 자체의 결함을 두 번 만들었다(기록).**
`git check-ignore --stdin` 배치 조회를 믿어 "미추적 production 67개"라는 결과를 냈는데,
파일마다 다시 물으니 **2개**였다(둘 다 이미 승인 SKIP 항목). CSV 비교는
`row.get(k,"")` 로 컬럼 없는 파일을 조용히 뭉갰다. **감사 도구도 감사 대상이다** —
특히 `.get(default)` 와 배치 조회는 실패를 "그럴듯한 값"으로 바꿔 놓는다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,419**. 변이 검증 3건.


**Sprint 197 — 워커 비용 / image 큐 부재 원인 / 문서 "원래 없음" 결론**

**doc_worker 비용: 이동이 63%다.** 행당 24.0초 = 이동 15.2 + 수집 6.8 + sleep 2.0.
같은 물건의 doc_type 마다 따로 claim 하고 **같은 상세페이지에 다시 들어간다**
(형제 재사용은 다른 물건 사이에서만 동작한다). 물건 단위로 묶으면 96.0초 -> 50.4초
(**47% 절감 여지**). 착수하지 않는다 — 행 단위 원자적 claim·재시도 예산·부분 실패
처리와 전부 얽혀 있고, **지금은 대기 중이며 기일이 남은 물건이 2개뿐**이라 얻을 것도 없다.
큐가 다시 수천 행으로 자란 뒤(BUGS #123 해소 후) 재평가할 일이다.

**★ `image` 큐 행이 0개인 이유 — 코드 결함이 아니다.**
큐 `enqueued_at` 최대가 **2026-08-12**, `image` doc_type 추가는 **2026-08-17**(Sprint 144).
**마지막 적재가 image 추가보다 앞선다.** 그 뒤로 크롤이 돌지 않아 생길 기회가 없었다
(`enqueue_documents()` 는 4종을 넣고 회귀 §15 가 그것을 지킨다).
`auction_image` 45행의 crawl_date 가 08-17 인 것도 앞뒤가 맞는다 — 큐를 거치지 않은
수동 실행분이다. **즉 사진은 스케줄러 등록 후 첫 크롤 전까지 자동 수집되지 않는다.**

**문서의 "원래 없음" — 신뢰할 positive signal 이 없다.** 사진은 DOM 에 박혀 오므로
"요소가 하나도 없다"가 양성 신호지만(no_asset), 문서는 **정적 표에서 온 버튼 id** 로
클릭하므로 "버튼을 못 찾았다"가 (a) 법원 미제공 (b) DOM 변경 (c) 페이지 미완성 을
구분하지 못한다. 후보 물건(FAILED 1건)은 기일이 지나 **법원 사이트 조회 자체가 안 된다.**
**새 상태를 만들지 않는다** — "버튼 없음 = 원래 없음"으로 단정하면 DOM 이 바뀐 날
전 물건의 문서가 조용히 "원래 없음"으로 기록된다.


### Sprint 198 — "변경 0건"의 원인 분리 (★ Sprint 196 결론 정정)

Sprint 196 은 CSV 연속 스냅숏에서 **변경 0건**을 보고 *"트리거는 옳지만 관측 기회가
거의 없다"* 고 결론지었다. **그 결론은 절반이 틀렸다.**

같은 데이터를 비교 축만 바꿔 다시 쟀다.

```
                      연속 비교    처음<->마지막(1,228물건)   빠졌다 돌아온 물건(45개)
...
**값이 바뀐 44물건이 전부 "빠졌다 돌아온" 45물건 안에 있다** — 변경은 **정확히 관측
공백 동안** 일어난다. `MAX_ITEMS=10` 이라 유찰로 값이 바뀔 즈음엔 이미 슬라이스에서
빠져 있기 때문이다.

**측정 도구를 도메인 시그니처로 검증했다**: 최저가 비율이 **0.7(34건) / 0.8(10건)**
두 값에만 몰린다 — 한국 경매의 법정 저감률(30%/20%)과 정확히 일치한다. 우연히 나올 수
없는 모양이고, `status` 도 84%가 `유찰 N회 -> N+1회` 형태다.

**정정된 판정**
- 기일/최저가/상태 -> **변경 실재**. 문서 재수집(spec+status = 2행/물건)은 실제로
  발동한다. 빈도 44물건/약 1개월 ≈ **하루 1.5물건 = 3행 = 72초**(창의 1%).
- 감정가 -> **정말로 안 바뀐다**(처음<->마지막에서도 0/1,228). **사진 재수집만**
  발동하지 않는다. 이건 관측 공백 탓이 아니다.

상한 60 은 실측 대비 40배 여유로 **안전 쪽으로 틀렸다** — 바꾸지 않는다.
상세는 `docs/SPRINT198_TRIGGER_OBSERVABILITY_CORRECTION.md`.

**교훈**: "0건"을 보면 **관측 설계부터 의심한다.** 같은 데이터에서 축만 바꿔 0 -> 132.

### Sprint 199 — 물건 단위 묶기 실증 + "실패 != 없음" 직접 증거

**호출 그래프**: `collect_spec/status/appraisal/images` 어느 것도 **이동 호출이 없다**.
새 탭을 여는 둘도 `finally` 에서 닫고 원래 창으로 돌아온다.

**그러나 코드 구조만으로 판정하지 않고 실브라우저로 확인했다**(DB 무변경, 임시 문서 루트,
법원 부하 1물건 x 2종).

```
go_to_case_detail        True   18.2초
collect_document(spec)   False  32.9초   창 1개 유지 / URL 그대로
상세페이지 생존 확인      사진 요소 5개 (이동 재시도 없이 읽힘)
collect_document(image)  True    0.0초   사진 5장

묶어서 51.1초 vs 현재 69.3초 -> 18.2초 절감(26%, 2종 기준)
```

**사진 수집은 0.0초다** — 사진의 전체 비용은 사실상 이동 하나뿐이다.

**★ 뜻밖의 소득**: 그 실행에서 `spec` 이 타임아웃으로 실패했는데, 그 물건의 spec 은
**이미 정상 수집돼 디스크에 392KB 로 있고 상태도 READY** 다. 즉 **실재하는 문서가
재시도에서 실패했다.** Sprint 197 이 "문서의 '원래 없음'에는 신뢰할 positive signal 이
없다"고 판단한 근거를 직접 실증한 셈이다 — "실패 = 미제공"으로 상태를 만들었다면 이
실행 하나로 **실재하는 문서가 '원래 없음'으로 기록**될 뻔했다. BUGS #122 의 방어
(재수집 실패가 READY 를 덮지 않는다)가 정확히 이 상황을 위한 것임도 확인됐다.

**묶기는 구현하지 않는다** — 가능함은 증명됐지만 claim 단위·재시도 예산이 전부 행 단위
전제이고(Sprint 189~194 가 막 고친 부분), 지금 대기 물건이 2개라 이득이 0이다.
착수 조건과 실측으로 확정된 설계 제약은 `docs/SPRINT199_BATCHING_FEASIBILITY.md` 에 남겼다.


### Sprint 200 — "기능 존재"와 "실행 경로에서 호출됨"을 구조적으로 분리

이 저장소가 반복해 겪은 최악의 결함 유형은 **고장이 아니라 "만들었는데 아무도 안 부른다"**
였고, 셋 다 **테스트는 전부 통과하고 있었다**(함수를 직접 호출해 검사했기 때문).

```
Sprint 144  doc_raw 를 채우는 코드가 있는데 운영 경로에 없었다 (556행 -> 0행)
Sprint 150  get_auction_images() 참조 0건
Sprint 189  collect_document(overwrite=True) 경로가 완성돼 있는데
            **아무도 True 를 넘기지 않았다** — 재수집이 통째로 죽어 있었다
```

`test_refresh_trigger.py` §18 이 그 유형을 구조적으로 막는다. 재수집 사슬의 **능력
19개**(requeue / claim / overwrite 전달 / 3근거 정리 3종 / 원자적 저장 / 내용 지문 /
무변경 미기록 / stale 회수 …)가 **실제로 정의되고 호출되는지** AST 로 확인한다.

**진입점을 손으로 적지 않는다.** 처음엔 3개만 적었더니 `refresh_queue_priority` 가
"배선 끊김"으로 잡혔는데, 실제로는 `refresh_priority.py`(run_priority_refresh.bat)가
부르고 있었다 — **가드의 거짓 경보는 아무 경보도 없는 것만큼 나쁘다.**
이제 `run_*.bat` 이 실행하는 .py 를 읽어 진입점을 유도하고 `crawler/`·`storage/` 를
통째로 더한다. 새 배치가 생기면 저절로 따라간다.

**변이 검증**: `doc_worker` 에서 `overwrite=overwrite` 한 줄을 제거(= Sprint 189 이전
상태, 재수집이 조용히 죽는 그 상태)하니 **3건 FAIL**. 복원 후 PASS.

**한계도 적어 뒀다** — "어딘가에서 호출된다"까지만 본다. 호출하는 쪽이 자기도 죽은
함수면 통과한다(간접 죽은 코드). 잡으려는 것은 Sprint 189 형태의 "배선 아예 없음"이다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,426**.


### Sprint 201 — 받아 놓고 버린 문서 (상세는 `docs/SPRINT201_DOWNLOADED_BUT_DISCARDED.md`)

**내 문서를 감사하다가 시작됐다.** Sprint 199 가 `spec -> image` 두 종류만 확인하고
"4종 묶기 가능"으로 일반화해 적은 것이 근거 부족이라, 4종 전부로 다시 쟀다.
그러다 훨씬 큰 것이 나왔다.

**★ BUGS #135 — 감정평가서 PDF 가 실제로 내려왔는데 버려지고 있었다.**
`plugins.always_open_pdf_externally: True` 때문에 Chrome 은 PDF 를 렌더링하지 않고
곧바로 내려받는다 → `window.open()` 탭은 **그릴 것이 없어 뜨지도 않는다.**
그런데 `collect_appraisal()` 은 탭을 성공 조건으로 삼고, 안 뜨면
**`wait_for_download()` 를 부르지도 않고** 실패로 끝냈다.
**다운로드가 성공할수록 탭이 안 생기는 구조**였다.

```
로그            "appraisal PDF 탭 생성 실패"  success=False
실제 downloads/  2,528,908 B PDF 도착, 기존 appraisal.pdf 와 sha256 일치
고아 누적        downloads/ 최상위 8개 / 14.0MB
                그중 4개는 Chrome 중복 이름 (1)(2)(3) = 같은 문서를 네 번 받아 네 번 버림
```

수정 후 **실환경 재확인: `appraisal success=True`** (19.8초). 실제 법원에서 실제
다운로드가 이제 저장된다. 회귀 7j 는 **탭을 절대 만들지 않는 가짜 드라이버**로
3경우(도착→저장 / 없음→실패 / 가짜PDF→거부)를 고정하고 변이 검증도 마쳤다.

이 발견은 Sprint 197 의 결론을 더 무겁게 만든다 — "수집 실패와 원래 없음을 구분할
신호가 없다"고 했는데, **실패의 상당수가 사실은 성공이었다.**

**BUGS #136 — 명세서도 고아가 있다(원인 미확정).** 고아 8개 중 5개가 매각물건명세서다.
가설은 "30초 타임아웃 뒤 도착 → 다음 실행의 `before_files` 에 이미 포함 → 영원히
회수 불가"지만 **확정하지 못했으므로 코드를 바꾸지 않았다.** 타임아웃 조정은 운영 로그
없이 정할 수 없고, 고아를 물건에 붙이는 것은 이름만으로 소유자를 확정할 수 없다.
대신 `audit_asset_integrity.py` **[8] 상시 탐지**를 붙였다.

**묶기 실증 4종 완료** — 가장 불리한 순서(새 탭 2종 연속 → 오버레이 → DOM)로도
창 1개·URL 유지·사진요소 5개 생존. **절감 실측 44%**(Sprint 197 추정 47%와 근접).
여전히 착수하지 않는다(claim 단위·재시도 예산이 행 단위 전제, 대기 물건 2개).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,463**.


### Sprint 202 — #135 계열 전수검색: 명세서에도 같은 결함이 있었다

**BUGS #136 확정·수정.** Sprint 201 은 명세서 고아의 원인을 가설로만 두고 탐지만
붙였다. 계열 전수검색에서 `collect_spec()` 이 `collect_appraisal()` 과 **완전히 같은
모양**임을 확인했다 — 탭이 안 뜨면 `wait_for_download()` 를 부르지도 않고 실패.
법원이 명세서를 뷰어 대신 PDF 로 바로 내려 주면 그 파일을 버린다.
`downloads/` 고아 8개 중 **5개가 명세서**였던 것이 그 흔적이다.

수정: 탭이 없으면 짧게 도착을 확인하고, 왔으면 **뷰어 단계를 건너뛰고 바로 저장**한다.
회귀 7k 는 **뷰어 경로가 그대로 동작하는지**까지 대조군으로 고정한다.

**★ BUGS #137 — 가드가 `else:` 블록을 통째로 안 보고 있었다.**
#136 을 고치며 뷰어 경로를 `else:` 로 옮기자 그 안의 `wait_for_download()` 호출이
**가드에서 사라졌다**(3개 중 2개만 검사). 원인은 `getattr(parent, "body")` 만 훑은 것 —
`ast.If` 의 `orelse`, `try` 의 `finalbody` 는 안 봤다.
**BUGS #133(BOM 파일 누락)과 같은 부류**로, 가드가 "전수"라면서 전수가 아니었다.
문장 리스트를 갖는 속성을 전부 훑도록 고쳤고, 변이로 확인했다.

이 세션에서 **가드 자체의 사각지대가 두 번**(#133 인코딩, #137 문법 블록) 나왔다.
전수 가드는 **"무엇을 안 봤는지"를 스스로 말하게** 만들어야 한다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,473**.
Frontend 113/113, tsc 0, eslint 0 (서버를 띄워 실측).

### Sprint 202 (2) - 내 도구가 만든 가짜 변경 3,681줄 (BUGS #138)

문서를 붙인 뒤 `git diff --numstat` 을 보다 발견했다.

```
docs/CHANGELOG.md    +3854 / -3560      실제 내용은 294줄
config/settings.py   +136  / -121       실제 내용은 15줄
```

이 저장소는 `core.autocrlf=true` 인데도 **blob 자체에 CRLF 가 든 파일이 73개** 있다
(Windows 에서 autocrlf 없이 커밋되던 시절의 흔적). git 은 index 의 blob 에 CR 이 있으면
그 경로의 정규화를 끄므로, 그 파일들은 작업본이 **글자 그대로 CRLF 를 유지해야** 한다.

원인은 내 문서 덧붙이기 헬퍼였다.

```python
s = io.open(p, encoding="utf-8-sig").read()   # 텍스트 모드 = 개행 자동 변환
crlf = "CRLF 가 있는가"                        # <- 원리적으로 항상 False
```

파이썬 텍스트 모드는 CRLF 를 읽는 즉시 LF 로 바꾼다. **"확인했다"고 믿었지만 확인할 수
없는 방법으로 확인하고 있었다.** 기능 결함은 아니지만 피해는 리뷰 불가능이다 -
가짜 3,681줄 속에 진짜 15줄이 묻힌다.

되돌렸고(가짜 변경 소멸), 판정을 바이트 기준으로 바꿨고, 가드 27 을 신설했다.

**가드를 한 번 잘못 만든 것도 남긴다.** 처음엔 "모든 추적 파일의 줄끝이 HEAD 와 같아야
한다"로 만들어 **정상 체크아웃 97개**를 잡았다. LF blob 은 autocrlf 가 정규화하므로
작업본 줄끝이 자유다. **넓은 가드와 정확한 가드는 다르다.**

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,473**,
Frontend 113/113, tsc 0, eslint 0.

### Sprint 203 - 검색 면적/특수조건 필터 감사 (상세는 `docs/SPRINT203_SEARCH_FILTER_DEPENDENCY_AUDIT.md`)

**면적은 이미 DB 안에 있다.** `auction_item.full_address` 끝 대괄호에
`[집합건물 철근콘크리트구조 29.95㎡]` / `[토지 대 420㎡]` 형태로,
**1,876행 중 1,852행(98.7%)**. 법원 목록 소재지 칸이 그대로 저장된 것이고,
의도한 보존이 아니라 손대지 않아서 남아 있다.

못 만들고 있는 이유는 데이터가 없어서가 아니라 **"어느 면적인가"가 안 정해져서**다.
실측한 함정: 다층 건물 44건(첫 값만 쓰면 최대 **103배** 과소), 지분 128건(6.8%),
평 단위 5건, 대괄호 중첩 2건, 현황 병기 11건. 상세페이지를 열면 한 물건에
1987.50 / 5.10 / 2142 / 1.57 이 동시에 있다 - 아무거나 집으면 400배 틀린다.

**특수조건은 반대다 - 소스에 있는데 크롤러가 읽지 않는다.**
법원 목록 표 **셀5**(특별매각조건)에 문장으로 들어 있다(실측:
"임차보증금반환채권승계인 주택도시보증공사의 매수인에 대한 대항력 포기조건 매각").
`collect_list_items()` 는 셀 0·1·2·3·6·7 만 쓴다. 셀5는 코드에 없다.

**★ 더 큰 발견 - 수집해 놓고 버리는 경계.** `crawl_detail()` 이 모으는 다섯 덩이 중
`property_list`(목록내역 = 등기부 표제부 전문, 면적 포함)/`schedule`/`nearby_cases` 는
**코드 전체에서 읽는 곳이 없다.** `normalize_item()` 이 21개 필드만 통과시킨다.
즉 크롤러는 매 물건마다 이 표들을 긁는 비용을 치르고 결과를 버린다.

**한 것**: 필터는 만들지 않았다(제품 결정 = 승인 영역). 대신 **이미 가진 데이터를
조용히 잃는 경로**를 막았다 - `test_normalizer.py` 주소 끝 대괄호 보존 가드
(사례 4 x 검사 3). 대괄호를 떼면 이 저장소에서 면적이 완전히 사라지는데
그것을 지키는 검사가 하나도 없었다. 변이 2종으로 확인(8건/3건 FAIL).

기대값을 처음에 틀리게 적었고 실측으로 고쳤다 - 도로명주소 `...로7길 37(명륜3가)` 는
지번이 빈 문자열로 나온다. 기대값은 "이래야 옳다"가 아니라 **"지금 이렇게 나온다"**를
고정한 것임을 주석에 명시했다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,485**.

### Sprint 203 (2) - 간헐 실패를 한 번 놓쳤다 (BUGS #139)

`test_doc_storage_atomicity.py` 가 전체 실행에서 **0.1초 만에 25단언에서** 죽었다.
단독은 18.1초에 138단언 통과, 이후 전체 실행 4번 전부 통과 - 재현율 1/5.

**원인을 못 본 이유가 더 중요하다.** 러너가 실패 출력을 `-v` 일 때만 보여 줬다.
발견하는 순간은 대개 `-v` 없이 돌린 순간이고, 재실행하면 통과하는 간헐 실패는
**그 한 번이 유일한 기회**다. traceback 은 그대로 버려졌다.

가설(확정 아님): 저장소가 **OneDrive 동기화 폴더 아래**에 있고, 이 테스트는
`documents/qa-atomic-*` 를 저장소 안에 만들었다 지운다. 동기화 잠금과 겹치면
일시적 PermissionError 가 난다. 확정하지 못했으므로 **코드를 고치지 않았다** -
임시 디렉터리를 밖으로 빼면 "실 DOCUMENT_ROOT 아래에서 동작하는가" 검사가 무뎌진다.

대신 러너가 실패 파일의 마지막 12줄을 `-v` 없이도 그 자리에서 찍게 했다.
변이(일부러 죽는 테스트)로 확인했다. 다음에 같은 일이 나면 증거가 남는다.

### Sprint 204 - 예약 실행 "등록 후 검증"에 도구가 없었다

Backlog 1은 등록 자체는 SKIP 하되 **전제조건과 등록 후 검증은 계속 감사**하라고 한다.
전제조건은 이미 다 확인돼 있었다(dry-run 재실행: 배치 3개 OK, PATH python 확인,
머신 PATH 불가 -> SYSTEM 등록 금지). 없던 것은 **등록한 뒤에 그것이 실제로 돌았는지
확인하는 방법**이다.

**왜 필요한가 - 이 저장소는 같은 착각을 이미 했다.** Sprint 187은
`DOJOONPASS_DAILY`(run_daily.bat, 매일 03:00, LastTaskResult 0)를 보고
"물건 수집은 정상 동작 중"이라고 기록했다. 작업 스케줄러의 성공(0)은
**프로세스가 0으로 끝났다**는 뜻이지 이 저장소에 무엇이 쌓였다는 뜻이 아니다.

`audit_schedule_health.py` 신설(읽기 전용). 세 축을 따로 재고 **어긋나는 지점을
지목한다**: 등록 / 마지막 실행 / 저장소에 남은 흔적. 오늘 실측은 세 축이 전부 일치했다.

```
등록   schtasks 전체 249개 중 이 저장소를 가리키는 작업 0개
흔적   logs/daily_run.log 마지막 갱신 2026-08-11 17:05
       (마지막 완료 표시 "Finished at 2026-08-02  6:02:49")
데이터 auction_item.crawl_date 최신 2026-08-12 (단발 9건)
잔여   앞으로 기일이 남은 물건 9건, 전부 2026-08-19
```

`docs/BETA_RELEASE_CHECKLIST.md` 의 Sprint 187 문단에 재정정 표시를 붙였다(원문 보존).

**★ 그 감사기가 처음에 스스로 그 함정에 빠졌다.** 첫 실행에서 "스케줄러 전체 작업 0개"를
찍었다 - 실제로는 249개다. `schtasks /fo csv /v` 의 **1번 칸은 "호스트 이름"이고 작업
이름은 2번 칸**인데, 첫 칸만 보고 헤더를 찾았다. 파일 맨 위에 "조용한 0건을 막겠다"고
써 놓고 그 아래에서 조용한 0건을 만들었다.

고친 뒤 두 가지를 함께 넣었다 - (1) 읽을 것이 있었는데 하나도 못 뽑으면 **0개가 아니라
조회 실패(None)** 로 돌려준다, (2) 한글/영문 헤더 픽스처로 selftest 를 고정한다
(전체 9검사).

**가드 3개 추가** (`test_schema_hygiene.py` 14-B). config 주석이
"예약 작업 등록 시각과 같아야 한다"고 적어만 두고 지키는 것이 없었다.

```
DOC_WORKER_START_TIME  vs 등록 스크립트 DocWorker 시각      변이 -> FAIL 1
우선순위(01:50) < 문서수집(02:00) 순서                       변이 -> FAIL 2
ExecutionTimeLimit(240분) >= worker 실행 창(120분)          변이 -> FAIL 1
```

순서 검사가 특히 조용한 자리다 - 뒤바꾸면 그날 우선순위 갱신이 **이미 끝난 수집에**
적용된다. 오류도 빈 결과도 아니고 임박 물건이 뒤로 밀릴 뿐이다.

**SKIP(승인 영역)**: `register_scheduler_tasks.ps1 -Apply`,
`audit_schedule_health.py` 의 `git add`.

### Sprint 205 - "공허한 가드" 전수 감사

이 세션에서 **가드가 아무것도 안 보면서 초록이 되는 일**이 세 번 나왔다.

```
BUGS #133   BOM 파일을 조용히 건너뛴 AST 스캐너 (77개 중 40개 미검사)
BUGS #137   `body` 만 훑어 `else:` 블록을 통째로 놓친 호출부 가드
Sprint 204  내가 만든 감사기가 249개 작업을 0개로 읽음 (헤더 위치 오독)
```

세 번이면 우연이 아니라 계열이다. 전수로 훑었다.

**훑는 방법과 그 한계를 먼저 적는다.** 테스트 44개(추적 43 + 미추적 1)를 AST 로 파싱해
`check(..., X, [])` 형태의 "비었음" 단언을 찾고, 같은 함수에 **하한 단언**이 있는지 봤다.

```
1차 (모든 '비었음' 단언)                      115개 함수
2차 (스캔으로 대상을 발견하는 것만)              4개 함수
```

1차 115개는 대부분 오탐이다 - 고정 픽스처로 만든 상황에서 "저장 0장", "doc_raw 0행"
처럼 **그 값이 정답인** 단언이다. 스캔이 깨질 여지가 없으니 하한도 의미가 없다.
그래서 `os.walk`/`glob`/`ast.parse`/`git ls-files` 로 **대상을 발견하는** 함수만 남겼다.

**남은 4개 중 실제 결함은 2개였다.**

| 대상 | 판정 |
|---|---|
| `test_auction_identity` 법원 스코프 SQL 스캔 | **결함** - 하한 없음 |
| `test_schema_hygiene` .ps1 BOM 스캔 | **결함** - 하한 없음 |
| `test_bootstrap` CLAUDE.md 주장 검증 | 오탐 - 이미 `check_true(bool(numbers))` 로 하한이 있다 |
| `test_schema_hygiene` BOM vs HEAD 대조 | 하한을 둘 수 없다 - 작업트리가 깨끗하면 0개가 정답이다 |

고친 2개는 실측 기반 하한을 붙였다.

```
법원 스코프 : 프로덕션 .py 86개 / 쓰기 문장 15개  -> 하한 40개 / 8개
.ps1 BOM    : 비ASCII .ps1 1개                  -> 하한 1개
```

변이 검증: 파일 열거를 2개로 자르면 FAIL 2건, `.ps1` 열거를 비우면 FAIL 1건.
둘 다 **수정 전이었다면 조용히 통과**했다.

**★ 내 탐지기도 한 번 틀렸다.** 하한으로 `X > N` 같은 비교만 인정하고
`check_true("...", bool(numbers), ...)` 형태를 못 알아봤다. 그래서
`test_bootstrap` 을 결함으로 잡았다(오탐). 4개까지 좁혔기에 눈으로 확인해 걸렀지
115개였다면 그대로 "결함 3개"라고 적었을 것이다. **좁히는 것 자체가 검증 수단이다.**

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,495**.

### Sprint 206 - 낡은 "미구현/미지원" 주장 전수 감사

이 세션은 낡은 주장 때문에 세 번 헛돌았다(CLAUDE.md 의 requirements.txt 부재 주장,
Sprint 187 의 스케줄러 정상 동작 주장, BUGS #136 의 "원인 미확정"). 그래서
프로덕션 소스 전체에서 미완/미지원 표시를 훑었다(테스트 파일 제외).

```
검사 대상 소스 196개 -> 표시가 있는 파일 14개 / 22건
```

**22건 중 살아 있는 거짓 주장은 1건이었다.**

`src/app/search/types.ts` — "아래 4개 필드는 DB에 **대응 컬럼/데이터가 없어**
현재 백엔드 미지원". 절반만 맞다. 컬럼은 없지만 **데이터는 이미 있다** —
Sprint 203 이 실측한 대로 `full_address` 대괄호에 1,852/1,876행(98.7%).
이 문장을 그대로 두면 다음 사람이 "면적을 쓰려면 재크롤부터"라고 판단한다.
정정하고 실제 장애물(의미 정의)을 적었다.

**나머지 21건은 여전히 참이거나 이미 스스로 정정돼 있었다.**

| 자리 | 판정 |
|---|---|
| `api/v1/item.py` 서버 썸네일 없음 | 참 (Pillow 가 requirements 에 없다 - 확인함) |
| `api/v1/payment_providers.py` KG이니시스 미구현 | 참 (NotImplementedError 로 사유까지 명시) |
| `SearchForm.tsx` TODO 3건 + "준비 중입니다" 2건 | 참 (프런트 계약 가드가 지킨다) |
| `repair_unsupported_status_docs.py` | 이미 인라인 정정 있음 |
| `api/v1/payment_logs.py` | 2026-08-15 정정 문단이 이미 있음 |
| `storage/migrations/014_*.sql` "수신 엔드포인트 아직 없다" | **낡았다. 그러나 고치지 않는다** |

마지막 항이 판단이 필요한 자리다. 그 주장은 지금 거짓이지만
**이미 적용된 마이그레이션 파일**이라 손대지 않는다. 대신 살아 있는 코드
(`api/v1/payment_logs.py`)에 정정이 이미 있고, 마이그레이션 러너는 체크섬이 아니라
파일명만 추적한다는 것도 확인했다(고치는 것이 위험하지는 않다 - 다만 이력을
사후에 고쳐 쓰지 않는 쪽을 택했다).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,495**,
Frontend 113/113, tsc 0, eslint 0.

### Sprint 207 - CVE 가드가 낡은 안전선을 가리키고 있었다

`test_schema_hygiene.py` 8번은 `next` 의 CVE 를 추적하며
**"16.2.11 이상으로 올리면 해소된다"** 고 안내한다. `npm audit` 을 실제로 돌려 보니
그 안내가 낡았다.

```
취약 범위     9.3.4-canary.0 - 16.3.0-preview.10
npm 수정본    16.3.1  (isSemVerMajor = false)
설치본        16.2.9
```

16.2.11 로 올리면 CVE-2026-64641 하나만 벗어나고 **나머지 8건이 그대로 남는다.**
"올렸으니 됐다"는 잘못된 종결로 이어질 자리라 상수와 문구를 오늘 실측에 맞췄다.

**그리고 이 가드는 next 하나만 보고 있었다.** 실측 결과 권고가 걸린 패키지는 **7개**
(moderate 1 / high 6)이고 나머지 6개(postcss / sharp / nanoid / js-yaml /
brace-expansion / @tailwindcss/postcss)는 아무 검사도 받지 않았다.

새 CVE 를 오프라인에서 알아낼 방법은 없다. 그래서 **알 수 있는 것만** 잠갔다 -
8-B 신설: 7개의 설치본이 오늘 스냅샷보다 **낮아지면 실패**한다(package-lock.json 실측).
변이 2종 확인(버전 후퇴 -> FAIL, 스냅샷 패키지가 lock 에서 사라짐 -> FAIL).

업그레이드 자체는 승인 영역이라 하지 않았다. 다만 `next@16.3.1` 이 메이저가 아니고
postcss/sharp 의 수정 경로도 같은 항목을 가리킨다는 사실을
`docs/BETA_RELEASE_CHECKLIST.md` P1 에 표로 남겼다 - 승인 판단에 필요한 정보다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,499**.

### Sprint 208 - 이미지 트리거 실행 경로 관통 조사 (상세는 `docs/SPRINT208_IMAGE_CHAIN_TRACE.md`)

"코드가 있다"와 "실제 실행된다"를 12단계로 분리해 따라갔다. 체인은 이어져 있었지만
**두 곳이 잘못 이어져 있었고 한 곳은 사실상 발동하지 않는다.**

**★ BUGS #140 - 사진을 적기 전에 성공을 먼저 기록했다.** `mark_queue_done()` 이
`save_auction_images()` 보다 먼저라, 뒤엣것이 실패하면 `document_status`=READY 인데
`auction_image`=0행이 된다. fixture 로 재현했고 순서를 바로잡았다.
원인은 **문서와 사진의 비대칭** — `doc_raw` 는 `mark_queue_done()` 트랜잭션 안에서
쓰여 원자적인데 사진만 밖에 있었다.

**★ BUGS #141 - API가 그 자기모순을 그대로 전달했다.** `READY + 0장 -> "READY"`.
`COLLECTING` 으로 낮췄다(두 번째 방어선). 다만 **상세페이지는 이미 안전했다** -
사진 배열을 먼저 보고 없으면 "수집 중"으로 저하된다. 사용자 증상은 없었다.

**★ 이미지 재수집 트리거는 41일간 발동할 수 없었다.** `image` 를 트리거하는 필드는
`appraisal_price` 하나뿐인데, CSV 백업 25개(07-02~08-12)로 물건별 처음<->마지막을
비교하니 **0.0%**(0/1,228)다. 나머지 세 필드는 각 3.6%로 함께 움직인다.
"이미지가 안 바뀌었다"가 아니라 **"재수집이 발동할 수 없었다"**이다.
매핑 확대는 사진 갱신 정책 + 재크롤 부하라 제품 결정 - SKIP.

**`image` 큐 행 0개는 코드 결함이 아니다(사본 실증).** 운영 DB 사본에
`enqueue_documents(rows)` 를 실제로 돌리니 image 10행이 생겼다
(`added=10, skipped_expired=1866`). 0개인 이유는 마지막 적재(08-12)가 image 종류
추가(08-17)보다 앞서기 때문이고 **다음 크롤 1회로 해소된다.** 그런데 크롤이 돌지 않는다.
기일 지난 1,866건이 걸러지는 것은 1차 방어선의 정상 동작이라, 오늘 크롤이 돌면
image 행은 **10개**(지금 살아 있는 물건 전부)만 생긴다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,529**.

### Sprint 209 - 문서 증거 정합성 감사 (증거보다 강한 표현 찾기)

Backlog #2 는 "Sprint 199 가 2종만 실증하고 4종으로 일반화했다"를 지목했다.
확인해 보니 **그 지적은 그 시점에 정확했고, Sprint 201 이 이미 4종 실측으로
뒷받침했다.** 그런데 Sprint 199 문서만 읽는 사람은 그 사실을 알 수 없다.
그래서 문구를 고치는 대신 **증거 등급 표**를 붙였다(원문 보존).

| 주장 | Sprint 199 의 증거 | 최종 증거 |
|---|---|---|
| 수집기가 페이지를 떠나지 않는다 | 코드(AST) | 그대로 |
| 이동 1회 뒤 2종이 이어서 수집된다 | **실브라우저 2종** | 그대로 |
| 4종도 같다 | **외삽** | Sprint 201 실브라우저 4종 |
| 절감률 | 2종 26% | Sprint 201 4종 **44%** |

**★ 같은 계열을 훑다 더 큰 것이 나왔다 - "3.1시간"이 실측으로 둔갑해 있었다.**

`1곳 186초 x 60곳 = 3.1시간` 은 **파생**인데 네 곳이 "실측"이라고 적고 있었다
(`BUGS.md` 2곳, `CURRENT_STATE.md`, `crawler.md`). 전부 "파생"으로 고쳤다.

원 측정의 실제 내용도 확인했다 - 서울중앙 **1곳**에서 **9건 / 186.2초**다.
`MAX_ITEMS = 10` 이 프로덕션 값임을 확인했으므로(`config/settings.py:3` ->
`crawler/court_crawler.py:115`) 곱셈 구조 자체는 성립한다. 다만 60곳이 모두
186초라는 전제는 확인된 적이 없고(기일 없는 법원은 즉시 스킵된다)
**끝까지 돌려 본 적도 없다**(전체 크롤은 2026-08-02 이후 실행 자체가 없다).
Sprint 190 문서에 이 전제를 명시했다.

**★★ 그리고 내 Sprint 203 주장이 틀렸다는 것을 발견했다.**

Sprint 203 은 특수조건 백필 비용을 "재크롤 3.1시간"이라고 적었다.
**비용의 문제가 아니라 불가능이다.** 크롤러는 법원별 **기일 목록**에 올라 있는
물건만 본다(`go_to_schedule()` -> 기일 링크 -> `collect_list_items(MAX_ITEMS)`).
기일이 지난 물건은 목록에 없고 사건번호 직접검색도 안 된다(실측 8건).

```
기일이 지난 1,867건  -> 특수조건을 영원히 채울 수 없다
1회 실행 상한        -> 60곳 x 10건 = 600건 ("전량 크롤"이라는 말 자체가 성립 안 함)
```

올바른 서술: **특수조건은 앞으로 새로 기일 목록에 오르는 물건부터만 채워진다.**
면적은 영향 없다 - 이미 `full_address` 에 있어 DB 안에서 backfill 할 수 있다.

**탐지기 자체의 한계도 적어 둔다.** 처음 만든 정규식은 "항상/절대/전부" 같은 단어를
잡아 245건을 냈는데 대부분 **규칙 문장**("절대 하지 않는다")이었다. 넓은 탐지가
곧 좋은 탐지가 아니다. 결정에 실제로 쓰이는 **수치 주장의 증거 등급**으로 좁혀서야
쓸모 있는 3건이 나왔다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,529**.

### Sprint 210 - 재시도가 소진되면 재수집 의도가 사라졌다 (BUGS #142)

Queue/Retry/Recovery 를 훑다 찾았다. Sprint 189 가 **중간 재시도**의 의도 소실을
막았는데, **재시도 소진** 경로는 그대로였다.

```
refresh -> in_progress_refresh -> 실패 x3 -> failed   (소실)
        -> 하루 뒤 복구 -> pending
        -> claim(overwrite=False) -> "이미 존재. 스킵" -> done
```

그 재시도는 **구조적으로 아무 일도 하지 않는다.** 법원이 바꾼 문서가 영원히 옛것으로
남고 큐는 성공으로 끝난다. fixture 로 프로덕션 순서 그대로 재현했다.

**고침**: 상태값을 새로 만들지 않고, 이미 DB 에 있는 증거로 판정한다 —
`document_status` 가 READY 인 행만 `refresh` 로 되돌린다(실체가 있으면 pending 은
반드시 헛돌기 때문). 실체가 없는 행은 그대로 `pending`.

**기존 테스트 하나가 옛 동작을 고정하고 있었다.** `test_document_status_sync.py` 9-(2)의
기대값을 `pending` -> `refresh` 로 바꿨다. **회귀가 아니라 계약 변경**이고, 그 검사의
본래 의도(READY 를 COLLECTING 으로 덮지 않는다)는 그대로 지켜진다.

**내가 새로 쓴 테스트의 결함도 기존 가드가 잡았다** - 픽스처의
`datetime('now','-2 day')` 에 `localtime` 이 빠져 한국 기준 9시간이 어긋날 수 있었다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,553**.

### Sprint 211 - "도달 불가"가 낡은 채로 결정 근거가 되고 있었다

`document_version_log` 는 지금도 **0행**이다. 그 0행의 **이유**가 문서마다 낡아 있었다.

```
Sprint 145 §6          "구조적으로 도달 불가"          -> 그 시점엔 정확했다
api/http_cache.py:33   같은 문장을 인용해 Cache-Control 미결정의 근거로 씀
docs/SPRINT146 §       같은 인용
docs/roadmap.md:1773   "도달 불가능한 코드로 남아 있다"
```

**Sprint 189 가 그 경로를 열었다.** `requeue_changed_documents()` 가 `done -> refresh` 로
큐를 되돌려 `overwrite=True` 가 실제로 도달하고, 해시가 다르면 개정 이력이 남는 것까지
fixture 로 이미 확인돼 있다(`test_asset_pipeline.py` "지문이 다르면 개정 이력이 남는다").

**바뀐 것은 결론이 아니라 이유다.** 0행인 것은 "경로가 없어서"가 아니라
**"예약 작업이 0개라 아직 한 번도 돌지 않아서"**다. `Cache-Control` 을 정할 운영 근거가
없다는 판단 자체는 그대로 유지된다. 네 곳 전부에 정정 마커를 붙였다(원문 보존).

**★ 구조 가드 28 신설 — 그리고 그 가드가 처음에 공허했다.**

낡은 주장이 되살아나는 것을 막으려고 "정정 없이 살아 있는 '도달 불가' 주장 없음"을
검사로 만들었다. 그런데 정정 인정 조건에 `"정정"` 이라는 흔한 단어를 넣는 바람에,
**정정 마커를 통째로 지워도 주변 900자에서 그 단어가 걸려 통과**했다(변이 실측 FAIL 0건).

이 저장소는 "정정"을 워낙 자주 쓴다. 조건을 `[정정` / `[재정정` /
`더 이상 사실이 아니다` 로 좁히고 창을 600자로 줄이자 **낡은 주장 2건이 더 나왔다**
(`docs/roadmap.md` 1곳은 진짜로 살아 있었다, 나머지 1곳은 취소선 이력이었다).
취소선(`~~`)은 이력으로 인정하고, 스캐너 자기 파일은 제외한다.

변이 2종 재검증: 두 정정 마커를 각각 지우면 각각 FAIL 1건.

**교훈**: 가드의 "예외 인정 조건"이 넓으면 가드가 아니라 통과 장치가 된다.
이번 세션에서 공허한 가드를 네 번째로 잡았다(#133 BOM / #137 orelse /
Sprint 204 감사기 / 이번).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,556**.

### Sprint 212 - 문서 쪽에는 같은 결함이 없다 (그리고 나는 하마터면 없는 것을 고칠 뻔했다)

Sprint 208 이 사진에서 두 가지를 고쳤다(실체보다 성공을 먼저 기록 / API 가 자기모순 전달).
**같은 계열이 문서에도 있는지** 훑었다.

**결론: 없다. 그리고 없는 이유가 구조적이다.**

```
사진   save_auction_images()  -> mark_queue_done() 트랜잭션 **밖**   (그래서 창이 있었다)
문서   _record_doc_raw()      -> mark_queue_done() 트랜잭션 **안**   (원자적)
```
...
`api/v1/item.py:_document_entry()` 의 `available = (status == "READY")` 도
현재 0행이 어긋난다. 사진에서 필요했던 **두 번째 방어선이 문서에는 필요 없다** —
쓰기가 원자적이라 애초에 그 상태가 생기지 않는다. 고치지 않았다.

**★ 그런데 여기서 내가 한 번 헛짚었다.** 앞선 스프린트에서 감사기 출력의

```
[3] document_status READY -> 필요한 파일 전부 존재
    READY 문서 557개 / 파일이 모자란 것 1개
    서울동부지방법원 2022타경55450-8 SPEC (없음: spec.pdf)
```

를 **운영 실측으로 읽었다.** 실제 운영값은 **556개 / 0개**다. 저 출력은
`--selftest` 가 **사본에 일부러 심은 결함**이었다(`결함 B - 문서가 READY 인데 파일이
없다`). 그 물건의 실제 상태는 COLLECTING 이고 READY 였던 적이 없다.

즉 **없는 결함을 근거로 API 를 고칠 뻔했다.**

**도구를 고쳤다.** selftest 가 심은 결함의 감사 출력에 `[selftest 가짜DB]` 접두사를
매 줄 붙인다. 실제 감사에는 붙지 않는다(실측 확인: 실제 실행 출력에 그 문자열 0회).

이 파일 자신이 "감사기가 거짓 경보를 내는 것은 아무 경보도 안 내는 것만큼 나쁘다"고
적어 두었는데, **자기 출력에는 그 원칙을 적용하지 않고 있었다.**

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,556**.

### Sprint 213 - 실행되지 않는 프로덕션 코드 전수 조사 (기술부채)

진입점을 **손으로 적지 않고** 유도했다 — `run_*.bat` 4개 + `api_server.py` +
`run_python_tests.py` + 추적 테스트. 거기서 import 그래프를 닫고 남는 것을 셌다.

```
.py 130개 (추적 127 + 미추적 3) -> 프로덕션 후보 82개
진입점에서 도달하지 않는 것      22개
파싱 실패                        0건
```

**새로 발견된 죽은 코드는 없다.** 22개는 이렇게 갈린다.

| 부류 | 개수 | 판정 |
|---|---|---|
| 수동 운영/일회성 스크립트 | 20 | 의도된 것(backfill/repair/dryrun/verify/audit 등) |
| `filter/scoring_engine.py`, `filter/report_generator.py` | 2 | **진짜 죽은 코드** — 이미 CLAUDE.md 에 기록됨. 삭제는 승인 영역 |

**★ 내 도구가 처음에 8개를 잘못 잡았다.** 패키지 `__init__.py` 는 하위 모듈을
import 하는 순간 파이썬이 **실제로 실행**하는데, import 이름만 따라가는 그래프는
그것을 놓친다. 하위 모듈이 도달했으면 그 패키지의 `__init__` 도 도달로 처리하도록
고치고 다시 쟀다(31 -> 22).

**정밀화 하나**: `docs/CLAUDE.md` 는 `filter_engine.py` 가 "80% 커버리지"라고 적는데,
그 80%는 **실행률이지 검증이 아니다** — `test_filter.py` 는 55줄짜리 출력 스크립트로
단언이 하나도 없고 회귀 실행기가 `NO-VERDICT` 로 분류한다. 즉 `filter/` 세 모듈은
**전부 검증되지 않은 상태**이고 도달 여부만 다르다. 그 문장을 CLAUDE.md 에 덧붙였다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,556**.

### Sprint 214 - 성공판정 A~F 관통 + 같은 계열 문서까지 (BUGS #143)

Sprint 208 의 순서 수정이 **충분한지**를 여섯 시나리오로 끝까지 흘려보냈다.
"예외가 났다"로 판정하지 않고 **큐 최종 상태**까지 봤다. 두 곳이 뚫려 있었다.

```
C 수집기가 준 경로에 파일이 없다   -> done / READY / auction_image 0행
E save 가 saved=0 을 돌려준다      -> done / READY / auction_image 0행
```

`save_auction_images()` 는 **예외를 던지지 않고** 0장을 기록할 수 있는데
호출부가 반환값을 로그로만 썼다. 예외가 없으니 바깥 `except` 도 발동하지 않는다.
**"함수를 불렀다"와 "성공했다"는 다르다.**

**★ 같은 계열이 문서에도 있었고, 그것은 코드 주석에 이미 적혀 있었다.**
`_record_doc_raw()` 가 *"큐/상태는 이미 done/READY로 갔지만 여기서 뒤집지는 않는다
(뒤집으려면 collect_document() 의 성공 판정을 고쳐야 한다)"* 라고 적어 둔 채였다.
재현하니 `queue=done / READY / doc_raw=0행` 에 API 는 `available=true` + `viewer_url`.
그 "고쳐야 한다"를 이번에 했다.

**정책은 바꾸지 않았다.** 부분 수집(`partial`)이 done 인 것은
`collect_images()` docstring 이 명시한 계약이라 그대로 두고 회귀로 고정했다.
실패 시 기존 asset 2장과 READY 화면 상태가 보존되는 것도 함께 잠갔다.

**측정 결과 (수정 후, 기존 asset 유무 2벌 전부)**

```
A 다운로드 실패   pending  기존 2장 보존
B 부분 수집       done     (계약)
C 파일 없음       pending  기존 2장 보존
D 기록 중 예외    pending  기존 2장 보존
E 기록 0장        pending  기존 2장 보존
F 전체 성공       done
```

변이 3종 전부 FAIL 확인(4건 / 5건 / 2건).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,883**.

### Sprint 215 - 이미지 전 구간 관통 fixture (원천 변경 -> API)

19번은 **수집기 진입까지만** 본다(`images: []` 를 돌려주므로 저장/해시/DB/API 는 타지
않는다). 그 뒤를 잇는 검사를 만들었다. **대역은 브라우저 한 곳뿐**이고 나머지는 실함수다.

```
[1] auction 삽입 -> migrate_execute -> auction_item 생성        PASS
[3] 감정가 변경 -> 관측 1건 -> image 행 refresh                 PASS
[4] 진짜 claim -> 수집기에 overwrite=True 도달                  PASS
[5] 파일 2장이 디스크에 실제로 저장                             PASS
[6] auction_image 2행 / 해시 비어있지 않음 / 크기 = 실제 파일    PASS
[7] 큐 done / 화면 READY / document_version_log 1행             PASS
[8] API image_count=2 / images_status=READY / 대표이미지 있음
    / 상세페이지가 읽는 URL 이 순번대로                          PASS
```

**[7] 의 `document_version_log` 1행이 중요하다.** 그 테이블은 운영에서 0행인데,
이 fixture 는 **재수집이 실제로 돌면 개정 이력이 남는다**는 것을 실함수로 보여 준다.
0행인 이유가 "경로가 없어서"가 아니라 "아직 안 돌아서"라는 Sprint 211 의 정정을
행동으로 재확인한 셈이다.

경계는 분명히 적어 둔다 — 상세페이지 **렌더링**은 이 검사의 범위가 아니다.
그 화면이 읽는 **응답 계약**만 확인하고, 렌더링은 `tests/frontend-contract.test.mjs` 가 본다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,918**.

### Sprint 216 - Cache/성능 확인 + 이미지 트리거의 '대가' 고정

**Cache — 확인만.** 조건부 요청 처리(`api/http_cache.not_modified`)가
`documents.py:122` / `images.py:100` 두 곳에서 실제로 호출되고, HEAD 핸들러는 GET 을
그대로 위임하므로 같은 경로를 탄다. 회귀도 있다(`test_http_conditional.py`).
**기능 존재와 호출을 따로 확인했다.** 고칠 것 없음.

**성능 — Sprint 210 이 넣은 복구 루프의 실비용을 사본으로 쟀다.**
`reset_stale_queue()` 는 `doc_worker` 가 기동 시 가장 먼저 부르는 함수이고,
거기에 행마다 `_current_document_status()` 조회가 하나 늘었다.

```
사본에 최악 상황 조성: 대기 2,753행을 전부 '하루 지난 failed' 로
reset_stale_queue()   0.034초  (행당 0.012 ms)
실행 창 120분 대비     0.0005%
원본 무변경 확인       분포 그대로
```

여유 있다. 덧붙여 2,753행 **전부 `pending` 으로 복귀**했다 — 승격(`refresh`) 대상이
0건이라는 뜻이고, 그 행들은 실체(READY)가 없는 미수집분이므로 의도한 대로다.

**★ 이미지 트리거의 대가를 검사로 못 박았다.**
`image` 로 가는 길은 `appraisal_price` 하나뿐인데 그 필드는 실측에서
**0/1,228 (0.0%)** 움직인다. 즉 이미지 재수집은 41일 관측에서 한 번도 발동할 수 없었다.
매핑을 넓히려는 사람이 이 측정을 마주하고 멈추도록 §1 에 단언을 넣었다 —
넓히는 것은 제품 결정이라 조용히 일어나면 안 된다.

변이(기일 변경에도 사진을 붙임) -> FAIL 3건.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 5,919**.


### Sprint 217 - 스킵 경로가 실체를 안 남긴다 (BUGS #144)

BUG #143 후속으로 여섯 시나리오를 다시 흘려보냈다. 아직 검사한 적 없는 칸이 하나 있었다 —
**실체는 저장됐는데 큐 성공기록이 실패하고, 그 뒤 재시도하는 경우.**

```
1회차  spec.pdf 저장 성공 -> mark_queue_done() 실패 -> 전부 롤백, 큐 pending  (설계대로)
2회차  파일이 이미 있다 -> "이미 존재. 스킵" -> files_saved=[]
       -> _record_doc_raw() 가 `if not files_saved: return`
       -> 큐 done / 화면 READY / doc_raw 0행
```

**영구인 것이 핵심이다.** 다음 수집도 파일이 있으니 같은 분기를 탄다. 스스로 회복되는
경로가 없다. API 는 `available=true` 를 주면서 `page_count`/`file_size`/`doc_version`
만 영원히 null 이다 — `mark_queue_done()` 이 "뷰어 페이지 이동이 불가능했던 근본 원인"
으로 적어 둔 바로 그 상태가 다시 만들어질 수 있었다.

**★ 사진 쪽은 같은 자리를 처음부터 복구하고 있었다.**
`_describe_existing()` 의 주석이 그대로 적고 있다 — *"파일은 있는데 `auction_image`
행만 없는 상태(이 저장소의 단골 결함)를 여기서 스스로 복구한다."* 문서만 없던 칸이다.

`existing_doc_files()` 를 신설해 세 분기가 이미 가진 파일을 결과에 담게 했다.
`doc_exists()` 와 **같은 목록·같은 기준**을 쓴다. 파일은 다시 쓰지 않고(mtime 무변경),
`previous_hash`/`new_hash` 는 그대로 비워 거짓 개정을 남기지 않는다.

**★ 이 결함을 기존 검사 두 개가 오히려 고정하고 있었다.**
...
둘 다 **의도는 옳고 증거가 틀렸다.** 증거를 파일 mtime/크기 무변경으로 바꿨다.
가드가 결함을 잡지 못한 것이 아니라, 가드가 결함을 **지키고** 있었다.

**측정 (수정 후)**

```
12-I  spec/appraisal/status  스킵 1회차 -> doc_raw 1행 / 2회차 -> 여전히 1행, 버전 유지
      API                     file_size 408, doc_version 1 (null 아님)
12-J  큐 성공기록 실패        pending(retry 1) / 사진 2행 보존 / 화면 상태 없음 / 개정 0행
      재시도 간격 전          수집기를 부르지 않는다
      간격 뒤 재시도          done / READY / 사진 2행 그대로 / 파일 2개 / 개정 0행
```

변이 6종 전부 검출(21 / 7 / 2 / 16 / 7건 FAIL + UNIQUE 위반으로 종료코드 1).

**구조 검사를 하나 더 걸었다.** 같은 모양의 분기가 두 수집기에 10곳 있다. 하나를 고쳤다고
나머지가 안전하지 않으므로 `test_false_success.py` 5 가 AST 로 훑는다 — 같은 분기에서
`success=True` 를 쓰면 `files_saved`/`images`/`no_asset` 중 하나가 함께 있어야 한다.
정적 검사라 도달 가능성은 보지 않는다는 한계도 함께 적어 뒀다.

**감사기도 두 칸 넓혔다** (`audit_asset_integrity.py`, 읽기 전용, 실 DB 둘 다 0건).

```
[4-b]  READY 인데 doc_raw 행이 없다        기존 [3]/[4] 는 둘 다 통과시킨다
[2-b]  사진 폴더의 이름 규칙 밖 파일        list_stored_images() 가 조용히 건너뛴다
                                          = "없다"가 아니라 "못 봤다" 였다
```

**실측 (사본/읽기 전용, 원본 무변경 확인)**
...
DB 는 병목이 아니다 — 물건당 약 22초의 수집 비용은 브라우저 navigation 이 지배하고,
위 항목을 전부 합쳐도 그 **0.03% 수준**이다. `.bat` 의 `%errorlevel%` 확장도 실제로
돌려 확인했다(블록 밖에서 설정된 값이라 정확히 7 이 찍힌다 — 로그가 거짓말하지 않는다).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,075**.

**★ 오늘이 검색 재고의 마지막 날이다.** 예약 작업은 여전히 249개 중 **0개**이고,
기일이 내일 이후인 물건이 **0건**이다(9건 전부 2026-08-19). 등록(`-Apply`)은 승인
영역이라 하지 않았고, dry-run 으로 선행 조건만 재확인했다(배치 3개 OK / PATH python
OK / 머신 PATH 불가 -> SYSTEM 계정 등록 금지).

#### Sprint 217 이어서 - 실서버로 확인한 것들

**API 가 광고한 URL 을 그대로 다시 요청했다 (전수, 실서버 HTTP).**
DB 와 디스크가 맞아도 서빙 계층에서 갈라질 수 있다. 응답이 준 URL 을 다시 두드렸다.

```
물건 206개 / 사진 URL 45개 / 문서 URL 556개   전부 200
image_count != len(images)                    0건
사진이 있는데 images_status != READY            0건
```

`audit_asset_integrity.py [9]` 로 남겼다. **서버가 없으면 "이상 없음"이 아니라
"확인하지 못함"** 이라고 말한다 — 가짜 서버(사진만 404)로 두 경로가 서로 다르다는 것을
자체 검사에서 증명했다.

**조건부 캐시도 실서버로 쟀다.**

```
/item/502/images/1     GET 200 70,100B   If-None-Match -> 304 0B   If-Modified-Since -> 304 0B
/item/54/documents/SPEC GET 200 406,093B  동일
```

`Cache-Control` 이 없는 것은 의도된 결정이다(`api/http_cache.py`: 실제 갱신 주기를
모르므로 `max-age` 를 지어내지 않는다).

**프런트 계약 테스트는 서버 없이 돌리면 조용히 절반만 돈다.**

```
서버 없음   tests 113  pass 63  fail 0  cancelled 50   <- "fail 0" 을 통과로 읽으면 안 된다
서버 기동   tests 113  pass 113 fail 0  cancelled 0
```

두 서버(uvicorn 8000 + next dev 3000)를 띄우고 다시 돌려 **113/113** 을 확인했다.
같은 조건에서 `test_beta_journey.py` 도 프런트 게이트 단계를 SKIP 없이 실행한다
(단언 62 -> 66). 그 파일은 건너뛴 단계를 요약에 명시하므로 조용히 통과하지는 않는다.

**bare `except:` 4곳을 없앴다.** `collect_documents.py` 가 `BaseException` 까지 잡아
Ctrl-C 와 SystemExit 를 삼켰다(그중 하나는 30초 대기 안이라 창이 넓다).
`api/v1/item.py` 가 **이미 같은 이유로 고친 자리**인데 규칙이 `api/` 밖으로 퍼지지 않았다.

★ 그리고 그 사실을 `test_error_logging.py` 의 주석이 *"bare `except:` — 이 저장소에는
없지만"* 이라고 적고 있었다. **검사 범위가 좁은 것을 "없다"로 적은 것**이다.
주석을 정정하고, 파이프라인 58개 파일 전수 가드를 새로 걸었다(하한 포함, 파싱 실패도
결함으로 보고). 변이 2종(결함 재주입 / 열거 비우기) 전부 FAIL.

**`.bat` 의 `%errorlevel%` 도 추측하지 않고 돌려 봤다.** `if errorlevel 1 (...)` 블록
안의 `%errorlevel%` 는 블록 **파싱 시점**에 확장되는데, 그 시점이 앞 명령 실행 **뒤**라
정확히 7 이 찍힌다. 로그가 거짓 코드를 적지 않는다.

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,083** (프런트 113/113).

#### Sprint 217 이어서 - 잠금이 잠그지 않고 있었다 (BUGS #145)

`RunLock` 은 두 배치가 겹쳐 도는 것을 막는다. 기존 회귀(§9)가 그 계약을 이미 검사하고
있었는데 **전부 순서대로**였다 — 잡고, 또 잡아 보고, 놓고, 회수한다. 그런데 이 락이
막으려는 상황은 순서가 아니라 **같은 순간**이다(운영자 수동 실행 + 예약 실행).

```
수정 전   스레드 8 x 200라운드   ->   200라운드 전부에서 8개가 동시에 성공
```

`exists()` 로 보고 `open("w")` 로 쓰는 사이가 통째로 열려 있었다.
...
지우기 직전 mtime 재확인 추가      그대로 4/1,000   <- "나아졌겠지"가 그대로 틀렸다
os.rename 로 회수 권한 중재        8스레드에서 2/40  <- 되돌리기가 셋 이상에서 남을 친다
회수 구역을 배타 토큰으로 감쌈      1,000라운드 전부 정확히 하나
```

두 번째 줄을 그대로 남긴다 — **재지 않았으면 "창을 좁혔다"고 적었을 것이다.**
창이 좁아진 것이 아니라 창의 종류가 같았다.

회귀는 두 경우를 따로 본다(평범한 경쟁 / 오래된 락 회수 경쟁). **"둘 다 실패"도 결함으로
센다** — 겉보기에 안전해 보이지만 그날 배치가 통째로 안 도는 것이다.
변이 3종 전부 검출(O_EXCL 제거는 기존 §3 이 "두 번째 실행이 큐를 건드렸다"로 죽어
`[FAIL]` 줄 없이 종료코드 1 — 문구가 아니라 종료코드로 판정하는 규칙이 또 값을 했다).

전체: **40 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,134**.

#### Sprint 217 이어서 - 검색이 사실을 말하는가 (실서버 대 SQL)

`total` 을 믿지 않고 같은 조건을 직접 SQL 로 세어 9가지 조합을 대조했다 —
**불일치 0건**, 페이지 전수 수집도 중복 없는 id 1,876개 = total 1,876.

처음엔 불일치 1건(`sido=서울특별시` API 275 / SQL 0)이 나왔는데 **대조 도구의 결함**
이었다. API 는 `extract_sido()` 로 `"서울특별시" -> "서울"` 정규화를 하고 DB 에는
축약형이 들어 있다. 그대로 보고했으면 멀쩡한 코드를 고치러 갔을 것이다 —
**불일치를 만나면 도구를 먼저 의심한다.**

적대적 파라미터 스윕도 함께 돌렸다: 검색/지역 1,100요청 + 자산 경로 190요청 =
**5xx 0건**, 경로 탈출(`..%2f..%2fauction.db`)은 404 0바이트.

#### Sprint 217 이어서 - 감사 도구 자체를 감사했다 (BUGS #146/#147)

goal 의 "Audit Tool 자체를 감사" 항목을 그대로 수행했다. **두 개가 걸렸다.**

**#146 — 전수 가드가 BOM 파일의 1행 import 를 못 본다.**
`test_schema_hygiene.py` 6-B 는 "추적 파일이 미추적 파일을 import 하지 않는가"를 본다.
그 간선 하나가 커밋 순간 API 를 통째로 죽인다(BUGS #105). 그 가드가 `utf-8` 로 읽고
있었고, BOM 이 남긴 `\ufeff` 는 공백이 아니라 `^\s*(?:from|import)` 에 매치되지 않는다.

```
추적 .py 중 BOM 파일          44개
그중 1행이 import 인 것        31개   <- 이 31개의 1행은 가드가 못 본다
```

`utf-8-sig` 로 고치고, 스캔 루프를 `_scan_import_edges()` 로 **분리**했다 —
함수 밖에 두면 회귀가 "간선 0개"라는 같은 초록을 볼 뿐 되돌림을 못 잡는다.
실제로 처음 쓴 회귀가 그랬다(변이가 통과했다). 지금은 **BOM + 1행 import 인 탐침 파일**
을 스캐너에 직접 먹인다.

**#147 — 집계기 자신에게는 검사가 없었다.**
`run_python_tests.py` 가 44개를 판정하는데 정작 자기를 보는 것이 없었다. 고장 나면
결과가 빨강이 아니라 **초록**으로 기운다(종료코드 검사가 빠지면 실패가 통과로 집계된다).
`test_runner_contract.py` 33단언 신설. 변이 4종(종료코드 검사 제거 / 판정문 없어도 PASSED /
discover 축소 / 실패해도 종료코드 0) 전부 검출.

**그 밖에 고친 것**

```
test_error_logging.py 3    except SyntaxError: continue  -> 파싱 실패를 결함으로 보고
test_error_logging.py 4    하드코딩 진입점 목록 -> git 목록.  훑는 파일 58 -> 86개
                           (28개가 검사 밖이었다. 그중 3파일에 bare except 4곳)
audit_schedule_health.py   칸 수가 어긋난 CSV 행을 조용히 버리던 것 -> 버린 수를 보고
test_asset_pipeline.py     Env 하네스가 문서 서빙 루트를 안 바꿔 뷰어가 **항상 404**
                           였다(12-L 을 쓰다가 걸렸다). 루트 사본 4곳 일치도 가드로 고정.
```

**실브라우저 — 검색 화면까지 그려진다.** `/search` 를 Chrome 으로 열어 확인했다:
총 9건 / 대표 이미지 9장 **전부 렌더링, 깨짐 0**, 전부 seq=1, 콘솔 오류 없음.

★ **첫 측정은 "9개 전부 깨짐"이었고 그것은 틀렸다.** `loading="lazy"` 라 아직 로드가
시작되지도 않은 상태를 읽었다. 스크린샷에는 사진이 멀쩡히 있었다. 이번 세션에서
측정 도구가 틀린 **세 번째** 사례다 — 결함처럼 보이면 도구를 먼저 의심한다.

상세페이지는 열지 않았다(로그인 상태로 열면 `record_view()` 가 운영 데이터를 쓴다).
브라우저 사용 후 `recent_items` 35행 무변경 확인.

전체: **41 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,205**.

#### Sprint 217 마무리 - 호출 순서를 코드로 잠갔다

**사진 전부 내림 경로를 worker 로 관통했다(12-M).** 5-H 는 `clear_images_if_absence_
confirmed()` 를 직접 부르고 그 뒤 `mark_queue_done()` 은 손으로 흉내 냈다. 즉 규칙은
고정했지만 **호출부가 그 규칙을 쓰는지, 올바른 순서로 쓰는지**는 코드를 읽어서만
알 수 있었다.

```
1회차   큐 done / NO_IMAGE / 사진 3행·파일 3개 그대로 / API 는 아직 READY 3장
2회차   큐 done / NO_IMAGE / 행 0 · 파일 0 / API image_count=0, images_status=NO_IMAGE
3회차   아무 일도 없다
```

변이가 순서의 값을 증명했다 — 정리 호출을 `mark_queue_done()` **뒤로 옮기면
1회차에서 사진 3장이 전부 사라진다**(FAIL 4건). 한 번의 관측 실패로 사용자가 보던
것을 빼앗는, 이 파이프라인에서 가장 파괴적인 동작이다. 이제 그것이 코드로 잠겨 있다.

**중복 규칙 전수 검색.** 모듈 최상단 상수를 AST 로 훑어 이름·값이 겹치는 곳을 찾았다.

```
DOCUMENT_ROOT   4개 모듈이 각자 계산   -> 결과 일치를 가드로 고정
PROJECT_ROOT    6개 모듈이 각자 계산   -> 결과 일치를 가드로 고정 (식은 깊이가 달라 원래 다르다)
LOCK_PATH/HOURS 값이 다르다            -> 배치가 다르므로 **정상**
```

`storage/database.py` 의 `PROJECT_ROOT` 는 `to_relative_storage_path()` 가 저장 경로를
접는 기준이라, 어긋나면 DB 에 적힌 경로가 아무 데도 안 맞게 된다. 변이로 확인.

**테스트가 운영을 멈추지 않게 했다.** 락 검사는 운영 경로를 그대로 쓴다(그게 옳다).
대신 중간에 죽으면 다음 실제 배치가 5시간 막힌다 — 정리를 `finally` 로 옮기고
`run()` 전체에 안전망을 뒀다. 중단을 주입해 확인했다.

스위트 전체 실행 전후 파일 목록 대조: **새 파일 0 / 사라진 파일 0.**

전체: **41 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,241** (프런트 113/113).

### Sprint 218 - 검색목록 썸네일까지 하나의 기능으로 (BUGS #148)

"이미지는 상세페이지에만 있으면 된다"가 아니다. **검색목록 썸네일까지가 한 기능**이라는
기준으로 체인을 다시 훑었다. 그러다 저장 계층이 **서빙될 수 없는 사진을 기록**하는 것을
찾았다.
...
241바이트 사진 하나로 재현: DB 1행 → API `image_count=1` / `images_status=READY` /
대표 URL 제공 → 검색목록도 그 URL 을 썸네일로 제공 → **그 URL 은 404**.
상세와 목록 **양쪽에** "사진 있음"으로 표시해 놓고 열면 없다.

`image_exists()` 의 docstring 이 규약을 이미 적어 뒀는데 **행을 만드는 함수만** 밖에
있었다. 운영 영향은 **0건**(45행 최소 크기 35,746바이트) — 수집기가 이미 같은 하한으로
걸러낸다. 막는 것은 잘린 파일·수동 조작·옛 backfill 이다.

**★ 그리고 그 결함을 픽스처 10건이 가려 주고 있었다.** 100~600바이트 사진으로
"READY / 2행" 을 단언하고 있었는데 그 크기는 **한 장도 서빙될 수 없다.**
파이프라인 전체가 거부할 데이터로 "정상"을 그린 셈이다.

**새 관통 검사 3종**

```
12-N  목록 썸네일 == 상세 대표 사진   두 화면이 서로 다른 코드로 대표를 고른다
                                     (MIN(seq) vs images[0]) — 대조한 검사가 없었다
12-O  사진 교체가 목록에 반영되는가    크기가 같은 다른 사진으로 교체해 ETag/200 확인
12-P/Q 기록된 사진은 서빙될 수 있어야  시나리오 + 판정처 3곳의 상수 일치(AST)
```

12-Q 는 처음에 무효였다 — 문자열 포함만 봐서 `_MIN_IMAGE_BYTES = 1024` 변이가 통과했다.
AST 로 숫자 대입과 단일 소스 import 를 보도록 고친 뒤에야 잡혔다.

**실측 — 검색목록 이미지 비용**

```
첫 방문(캐시 미스)   0.91 MB / 장당 p50 4.1ms / 합 48ms
재방문(조건부)       0 바이트 / 전부 304 / 합 46ms      아끼는 바이트 100%
썸네일 한 장 평균     약 101 KB   ← 80x80 으로 그리는데 원본을 그대로 내려 준다
```

서버 측 썸네일 생성은 Pillow 의존성이라 승인 영역. 지금은 사진 보유 물건이 적어
체감이 작지만 보유율이 오르면 1페이지가 곧 수 MB 가 된다.

**확인했지만 고치지 않은 것**: 검색목록은 "사진 없음"과 "수집 실패"를 구분하지 못한다
(이미지 관련 키가 `thumbnail_url` 하나뿐, 상세는 `images_status` 를 준다).
목록 상태 배지는 제품 결정이라 임의로 하지 않는다.

전체: **41 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,365**.

#### Sprint 218 이어서 - 실행 창 대비 처리 가능 물량

```
실행 창 120분 / 행당 24.0초(이동 15.2 + 수집 6.8 + sleep 2.0)
=> 하루 300행 = 75물건

지금 적체 중 **브라우저를 여는 행은 17행**뿐이다(나머지 2,736행은 기일 경과 -> 5.1ms 종결)
=> 적체 처리 6.8분 = 창의 6%, 여유 17.6배
```

`image` 가 0행인 것은 검색에 뜨는 9물건의 사진 수집이 **이미 끝나서**다(각 5장).

**병목은 적체가 아니라 최초 수집 총량**이다 — 1,876물건 x 4종 = 7,504행,
현재 방식으로 **25.0일치 실행 창**이 필요하다.

배칭(이동 1회로 4종)을 4종 기준으로 환산하면 물건당 96.0 -> 44.4초(**54% 절감**),
창 기준 75 -> 162물건, 최초 수집 25.0 -> 11.6일. **착수하지 않는다** —
Sprint 199 가 정한 착수 조건(대기 + 기일 남은 물건 수백 이상)이 지금 17행이라
성립하지 않는다. 데이터 공급이 재개된 뒤가 옳은 시점이다.

**BOM 계열 마무리.** 저장소 소스를 읽는 자리를 전수로 다시 훑었다 —
`encoding="utf-8"` 이 59곳 있지만 대부분 픽스처 파일 입출력이고, **저장소 소스를 읽는
7곳은 전부 부분 문자열 매칭**이라 BOM 영향이 없다(앵커된 정규식은 BUGS #146 한 곳뿐이었다).
그래도 **규칙을 하나로** 만들기 위해 그 7곳을 `utf-8-sig` 로 통일했다 —
다음 사람이 "여기는 앵커가 아니니 괜찮다"를 매번 다시 판단하지 않아도 되게.

### Sprint 219 - 큰글씨/접근성 감사 (실측)

roadmap 의 "큰글씨 기능"이 *"단순 font-size 확대가 아니라 실제 사용 흐름 전체를
감사한다"* 고 적고 있는데, **지금 화면이 얼마나 읽기 어려운지 잰 기록이 없었다.**
...
탭 타깃 53개 중 44px 미만          44개 (83%)
  그중 24px 미만                    5개   <- (★ 2026-08-20 Sprint 225 정정: 위반 아님 — 간격 예외로 적합)
14px 미만 텍스트                  111개
```

**가장 나쁜 것은 "무엇이" 작고 흐린가다.**

```
물건 주소            12px / 2.6:1
"최저입찰가" 라벨     11px / 2.6:1
"감정가 3.8억"       11px / 2.6:1
헤더 내비 5개        12px / 2.6:1 / 탭 타깃 높이 16px
```

가격과 주소 — 이 서비스를 쓰는 이유 그 자체 — 가 화면에서 가장 읽기 어렵다.
소스 분포로는 `text-xs` 111 / `text-gray-400` 106 이고, **상세페이지가 가장 심하다**
(text-xs 55 + gray-400 55). 실제 판단이 일어나는 화면이 그렇다.

**★ 측정 도구가 틀렸다(이번 세션 다섯 번째).** Tailwind v4 는 `oklch()` 로 색을 내는데
숫자만 뽑아 RGB 로 읽어 **대비 1.06** 같은 불가능한 값이 나왔다. canvas 로 변환을
브라우저에 맡기고 **알려진 값으로 도구부터 검증**(흑백 21.0 / `#9ca3af` 2.54)한 뒤 다시 쟀다.

**★ 재지 못한 것도 적는다.** 모바일 뷰포트(390~412px) 레이아웃은 **확인하지 못했다** —
자동화의 창 리사이즈가 페이지 뷰포트에 반영되지 않고(`innerWidth` 1920 고정),
`X-Frame-Options: DENY` 라 iframe 우회도 막혔다(그 헤더 자체는 올바른 보안 설정이다).
즉 **"모바일에서 깨지지 않는다"는 아직 주장할 수 없다.**

**고치지 않고 잠갔다.** 글자 크기·색·간격은 제품 디자인 결정이다.
`test_frontend_accessibility.py`(21단언)가 현재 수치를 **상한으로** 박는다 —
줄면 통과, 늘면 실패. 확대 차단 부재와 `alt` 규약도 함께 고정했다. 변이 4종 전부 검출.

★ 이 가드도 처음엔 **주석 속 `<img`** 를 세어 멀쩡한 파일을 결함으로 올릴 뻔했다.
주석을 걷어내되 줄 번호는 보존하도록 고쳤다.

**개선안(제품 결정 대기)**: ① 헤더 내비 h>=44px(WCAG 위반이라 논쟁 여지가 가장 적다)
② 주소·가격 라벨 14px 이상 + `text-gray-600`(4.6:1) ③ 상세페이지 본문 최소 14px
④ 큰글씨 모드 — **선행 작업이 필요하다**: 지금 크기가 대부분 px 고정이라
루트 글꼴만 키워도 본문이 따라 커지지 않는다. `rem` 전환이 먼저다.

전체: **42 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,386**.

### Sprint 220 - 모바일 구조 감사 / 검색목록 계약 / 큰글씨 전제 정정

**모바일은 재지 못했다 - 그래서 "정상"이라고 쓰지 않았다.**
창 리사이즈가 페이지 뷰포트에 반영되지 않고(`innerWidth` 1920 고정),
`X-Frame-Options: DENY` 라 iframe 우회도 막힌다(그 헤더는 올바른 설정이라 낮추지 않았다).

대신 **가로 넘침의 구조적 원인**을 전수로 훑어 0곳임을 확인했다.

```
고정 w-[NNNpx] / min-w-[NNNpx] / w-screen / 100vw / min-w-max / 인라인 width / <table>
반응형 없는 3열 이상 그리드                                        전부 0곳
컨테이너  max-w-[1320px] mx-auto px-4 md:px-8   (고정 폭이 아니라 상한)
```

**"넘치지 않는다"의 증명이 아니라 "넘칠 원인이 없다"의 증명**이다. 그 구분을 남긴다.

**검색목록 계약을 처음으로 잠갔다.** API 19키 / 타입 19필드 / 렌더 17필드, 어긋남 0.
세 곳이 각자 적혀 있는데 대조하는 검사가 없었다 — `item.foo` 가 `undefined` 면
React 는 **오류 없이 빈칸**을 그린다(조용한 실패의 프런트 판본).
변이 3종 검출, 그중 **API 키 이름 변경**이 타입·렌더 양쪽에서 동시에 울었다.

**데이터 신선도**: 라우터 캐시 헤더 0곳 + `api.ts` fetch 5곳 전부 `no-store` →
법원 데이터가 바뀌면 다음 요청부터 바로 새 값이다. 그 성질을 잠갔다.

**★ 직전 Sprint 의 내 기록을 정정했다.** Sprint 219 는 *"대부분 px 고정이라 루트 글꼴을
키워도 본문이 안 커진다"* 고 적었는데 **빌드 CSS 를 열어 보니 사실이 아니었다** —
`--text-xs: .75rem`, Tailwind v4 의 이름 있는 크기는 rem 기반이다.
닿지 않는 것은 임의값 **8곳**(`text-[11px]` 6 + `text-[10px]` 2)뿐이고,
**선행 작업은 전면 전환이 아니라 그 8곳 교체**다.
*(2026-08-19 Sprint 223에 그 8곳을 같은 크기의 rem 으로 바꿔 **0곳이 됐다** — 아래 참조.)*
*"대부분 px 고정"은 소스의 겉모습만 보고 내린 추정이었다 — 빌드 결과를 열어 봤어야 했다.*

가드 자신도 두 번 틀렸다 — `max-w-[1320px]` 안의 `w-[1` 을 고정 폭으로 오인(토큰 경계),
그리고 출력 리터럴에 **EM DASH** 를 넣어 `test_console_encoding.py` 에 걸렸다.
(goal 이 경고한 함정을 그대로 밟았고, **저장소의 기존 가드가 잡았다.**)

전체: **42 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,412**.

### Sprint 221 - 접속 정보 / 접근성 구조 / 화면 전체 계약

**개발 사이트 접속 정보** (세션 시작 시 3000·8000 둘 다 비어 있었다)

```
WEB   http://localhost:3000/     메인 = 검색 화면. 총 9건 + 썸네일 9장 렌더링(스크린샷 확인)
API   http://127.0.0.1:8000      Swagger /docs 200
로그인 검색/로그인 화면은 비로그인 가능
      /properties·/favorites·/mypage 는 로그인 필요 (src/proxy.ts PROTECTED_PREFIXES)
실행  python -m uvicorn api_server:app --host 127.0.0.1 --port 8000  +  npm run dev
```

**접근성 구조** — 색·크기·간격은 제품 결정이라 건드리지 않고 구조만 봤다.
랜드마크 정상, 이름 없는 대화형 요소 0, 키보드로 못 가는 클릭 요소 0.
포커스 표시는 **전역 리셋이 없고**(빌드 CSS 확인) `outline-none` 5곳 전부 대체 표시를
갖는다. 그 규칙을 가드로 잠갔다.

★ 그 판정에 **두 번 실패했다.** 계산된 스타일의 `outline-style: none` 은 **비포커스
기본값**이고, `el.focus()` 는 Chrome 에서 `:focus-visible` 을 켜지 않는다.
**CSS 규칙 자체**를 읽어야 답이 나왔다 —
*계산된 스타일은 "지금 이 순간"만 말하고, 접근성은 다른 순간의 규칙에 달려 있다.*

**화면 전체 데이터 계약**: 검색목록 19/19/17 · 상세 27/20 · 관심물건 22/12 ·
최근본물건 22/12 — **어긋남 0**. 뒤 둘은 `SELECT ai.*` 라 계약이 스키마에 묶여 있어
`auction_item` 실제 컬럼을 읽어 대조하게 만들었다.

★ **발견**: `thumbnail_url` 을 주는 API 는 `search.py` 뿐이고, **관심물건·최근 본 물건
화면에는 사진이 아예 없다.** 사용자는 검색목록에서 사진을 보고 담았는데 관심물건에서는
사진이 사라진다. 기존 문서에 언급이 없어 **미문서화 공백**으로 보인다.
고치지 않았다(제품 결정) — 대신 *어느 화면이든 `thumbnail_url` 을 그리면 그 화면 API 도
주어야 한다*는 규칙을 걸었다. 구현 경로는 이미 있다(`search.py` 의 `MIN(seq)` 배치 조회 +
`ResultThumbnail` 재사용).

가드 자신도 두 번 틀렸다 — `outline-` 를 대체 후보에 넣어 **자기 자신이 대체로** 잡혔고,
CSS 정규식이 `@supports` 를 선택자로 오인해 **변이 없이도 항상 실패**했다.
후자는 검출기를 **넣기 전에 검증**하는 방식으로 바꿨다(실제 0 / 주입 1 / 유틸리티 0).

전체: **42 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,429**.

### Sprint 222 - 폼 접근성 이름 (BUGS #150)

`/search` 의 아코디언을 **전부 펼친 뒤** 재측정했다. 접힌 상태만 보면 컨트롤이 5개뿐이라
**과소 측정**이 된다 — 화면 상태에 따라 달라지는 것은 그 상태를 만들어 놓고 재야 한다.
...
보이는 레이블이 `<span>` 이라 컨트롤과 **프로그래밍적으로 연결돼 있지 않다.**
최소/최대 select 가 나란히 둘이라 스크린리더로는 **어느 쪽인지도 알 수 없다.**
`placeholder` 는 입력을 시작하면 사라지므로 이름이 아니다(WCAG 3.3.2).

`aria-label` 은 **픽셀을 바꾸지 않고**, 이 저장소는 이미 그 패턴을 쓰고 있었다
(`시/도`·`시/군/구`·`법원`). 빠진 자리에 같은 방식을 적용했다 —
공용 컴포넌트 둘(`RangeSelect`/`PriceRangeSelect`)을 고치니 select 8개가 한 번에 해결됐다.

재측정 결과 **93/93 이름 있음**(수정 전 77/93).

**★ 이 검사의 앞선 판본이 거짓 결과를 냈다.** `<select[^>]*>` 로 여는 태그를 잡았더니
**`onChange={(e) => f(e)}` 의 `>` 에서 잘려** "select 7개 전부 이름 없음"이라고 보고했다.
브라우저로는 전부 이름이 있었으므로 **도구가 틀린 것**이었다. 중괄호 깊이를 추적하는
추출기로 바꾸고, 그 추출기를 **자기 검증**하도록 했다(이름 있는 샘플 / 없는 샘플).

전체: **42 PASSED / 0 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,440**.



### Sprint 223 - 모달 포커스 트랩 / 상태 메시지 알림 / main 랜드마크 / 큰글씨 rem

Sprint 221이 모달에 `role="dialog"`/`aria-modal`을 붙였지만 그것은 **스크린리더에게만**
하는 말이었다. 실제 키보드 포커스와 화면 변화 알림은 비어 있었다.

**BUGS #151 - 모달이 키보드 포커스를 가두지 않았다** (수정)

실브라우저 `/properties/505`, 진짜 마우스 클릭·진짜 Tab 키로 잰 값이다.

```
수정 전   모달을 연 직후 포커스     "대표 사진 크게 보기" <- 모달 **뒤**의 버튼
          모달 안 3개 / 화면 24개   21개가 오버레이 뒤에 그대로 살아 있다
          Tab 한 번                "전경도 1번 크게 보기"(top 415, left 346) = 가려진 버튼
          Escape 로 닫은 뒤         여는 버튼으로 복귀 안 함
수정 후   열기 -> '닫기' / Tab x3 순환 / Shift+Tab 역순환
          Escape -> **여는 버튼 바로 그 노드**로 복귀(=== 비교)
          모달 밖으로 강제 포커스 -> 곧바로 되돌아옴
```

`src/lib/useFocusTrap.ts` 신설, 두 모달에 배선. **픽셀 무변경.**

**★ 없는 결함을 만들 뻔했다.** 문서 뷰어의 **첫 열기만** 실패한다는 결과가 반복돼
"PDF iframe 이 포커스를 뺏는다"는 설명까지 세웠지만 틀렸다. 탭이 **보이지 않는 상태**
(`visibilityState: hidden`)에서 `element.focus()` 를 부르면 `activeElement` 는 바뀌지만
**focus/focusin 이벤트가 전혀 발생하지 않는다**(직접 단 리스너 0회 호출로 확인).
탭을 보이게 하고 진짜 마우스 클릭으로 재니 첫 열기부터 정상이었다.
*보이지 않는 탭에서 잰 포커스 값은 근거로 쓰지 않는다.*

**BUGS #152 - 나중에 나타나는 안내가 아무것도 읽히지 않았다** (수정)

```
수정 전   /search 의 aria-live / role=alert / role=status 요소   **0개**
수정 후   개별 상태 메시지 13곳에 alert|status
          검색 결과는 **항상 존재하는 sr-only 한 줄**(role=status, aria-live=polite)
          소프트 전환 후에도 같은 DOM 노드 유지, 글자만 변경(정렬 변경/0건 검색 둘 다 실측)
          렌더 크기 1x1 px - 화면에는 보이지 않는다
```

결과 목록 자체에 `aria-live` 를 달면 **0건일 때 그 문단이 통째로 사라져** 아무것도 알리지 못한다.
시/군/구 로드 실패는 `aria-describedby` 로 그 select 에 연결했다.

**BUGS #153 - 화면 4개에 `main` 랜드마크가 없었다** (수정)

기존 가드가 `<main>` 을 **저장소 전체로 합산**해 `>=1` 만 봐서 못 잡았다.

```
수정 전   /search 1 · /mypage 1(본문만)
          /properties/{id} 0 · /favorites 0 · /properties/recent 0 · /login 0
수정 후   화면 6개 전부 보유(로딩·실패 분기 포함)
          픽셀 무변경 확인: 상세 grid 622px 622px / 컨테이너 1320px / 가로 오버플로 0
```

가드도 **화면 루트(min-h-screen) 개수만큼 main** 으로 조였다 —
파일 단위 검사는 분기 하나가 빠져도 통과했다(변이로 확인).

**BUGS #154 - 검사 도구가 저장소 사본을 검사하고 있었다** (수정)

`test_console_encoding.py` 가 `.claude/worktrees/sprint95-...`(Sprint 95 커밋의 저장소
통째 사본)까지 훑었다. **스캔 298개 중 101개(34%)가 그 사본.** 제외 후 197개.
스테일 worktree 자체는 그대로 둔다(`git worktree remove` 는 승인 영역).

**큰글씨 - 닿지 않는 자리 8곳 -> 0곳**

```
text-[11px] -> text-[0.6875rem]   text-[10px] -> text-[0.625rem]
루트 16px    11px 그대로 (픽셀 무변경)
루트 32px    11px -> 22px (전에는 11px 고정)
2배 확대에서도 가로 오버플로 0
```

기술 기반은 끝났다. 남은 것은 **색·크기·간격을 얼마로 할지**라는 제품 결정뿐이다.

**성능 - 검색 API 의 SQL 횟수를 처음으로 쟀다**

```
size=1 / 3 / 9   전부 SQL **3회** (COUNT + 페이지 행 + auction_image 배치 MIN(seq))
상세 1건          7회, 전부 단건 조회
```

결과가 늘어도 SQL 이 늘지 않는다 = **N+1 아님**. 회귀로 잠갔다.

**관통 감사**

```
검색목록 썸네일 9/9   DB MIN(seq) == URL seq / 목록 == 상세 images[0] / HTTP 재요청 전부 성공
관심물건·최근본물건    썸네일 SKIP(제품 결정) - 최근 본 34개 중 사진 있는 것은 **4개**뿐
API <-> UI            유령 호출 0건 (라우트 42개)
모바일 실뷰포트        **여전히 확인하지 못했다** - resize_window(390x844) 후에도 innerWidth 1920
```

★ 조사 도구가 먼저 거짓말했다 — `app.routes` 만 읽어 "라우트 2개"라는 결과가 나왔다.
이 FastAPI 는 `include_router` 를 `_IncludedRouter` 로 지연 보관해 `path` 가 `None` 이다.
OpenAPI 스키마로 바꾸니 42개. 그리고 `GET /item/{id}/images/{seq}` 는 소스에 문자열이
없지만 **실제로 쓰인다**(프런트가 API 가 준 `thumbnail_url` 을 그대로 쓴다) —
grep 부재를 미사용으로 읽었다면 살아 있는 경로를 죽었다고 오판했을 것이다.

전체: **41 PASSED / 1 FAILED / 3 SKIPPED / 1 NO-VERDICT**.

유일한 FAILED 는 결함이 아니라 **가드가 옳게 말하는 것**이다 —
`test_schema_hygiene.py` 의 P0-B 가드가 *"추적 파일이 미추적 파일을 import 한다"*
(`src/app/properties/[id]/page.tsx` -> `src/lib/useFocusTrap.ts`)고 보고한다.
새 파일이 아직 git 에 추적되지 않아서이고, 해소는 `git add` 뿐이며 **승인 영역이라 하지 않았다.**
커밋할 때 `git add -A`(또는 그 파일을 명시)로 함께 담으면 즉시 초록으로 돌아온다.

### Sprint 224 - 목록 화면 대표 사진 / 검사 관측 공백 (2026-08-20)

시작할 때 문서가 적어 둔 기준선(41 PASSED / 1 FAILED)과 실제(40 / 2)가 달랐다.
문서가 틀린 것이 아니라 **날짜가 바뀌면서 하나가 더 무너진 것**이었고, 그 하나가
이번 스프린트에서 가장 값이 큰 발견으로 이어졌다.

**가드가 데이터가 낡자 눈을 감았다 (BUGS #155)**
...
물건 0건에서 `2 == 2` 로 공허하게 통과하고 있었다. 진짜 N+1 을 주입하고 고치기 전
방식으로 재 보니 **두 별표 모두 통과**했다 — 실제 성능 회귀가 조용히 초록불이 된다.

원인은 이 파일의 다른 모든 호출이 쓰는 `include_closed` 를 나중에 추가된 이 가드만
빠뜨린 것이다. 기본 검색은 `auction_date >= 오늘` 이라 크롤이 멈추면 0건이 된다
(DB 1,876건 보유 / 최신 매각기일 08-19 / 마지막 크롤 08-12).

**관심물건·최근 본 물건에 사진을 넣었다 (BUGS #156)**

이전 판이 "미문서화 공백, 제품 결정이라 고치지 않았다"로 남겨 둔 항목이다.
경로 규칙이 이미 두 곳에 흩어져 있어 두 화면을 더하면 넷이 되므로,
`api/v1/thumbnails.py` 하나로 모으고 `search.py`/`item.py` 도 그것을 쓰게 했다 —
**중복을 늘리지 않고 줄였다.** 프런트는 `ResultThumbnail` 을 `src/components/` 로
옮겨 세 화면이 공유한다.

```
실브라우저 최근 본 물건   카드 11개 중 사진 4개
                       <img> 4개 전부 naturalWidth > 0 (브라우저가 실제로 디코딩)
                       카드 폭 사진 유무 무관하게 411px / 가로 오버플로 0 / 콘솔 오류 0
실브라우저 관심물건       이 계정 0건이라 **운영 DB 사본**으로 확인 (카드 6개 중 사진 4개)
                       운영 DB favorites 는 0건 그대로 — 건드리지 않았다
```

변이 5/5 검출(N+1 / 키 제거 / 없는 URL / MIN->MAX / URL 규칙 어긋남).

**검사 하나가 프런트 계약 전체를 가리고 있었다 (BUGS #157)**

```
수정 전   pass 21  / fail 93 / skip 0     93건 전부 "물건이 0건입니다" 한 가지 원인
수정 후   pass 110 / fail  1 / skip 3
데이터 있으면(사본으로 실측)  pass 114 / fail 0 / skip 0
```

50개 검사 중 결과 데이터를 실제로 보는 것은 **단 하나**였는데, 전제가 `before()` 안에
있어 전부가 함께 무너졌다. 크롤이 하루만 멈춰도 접근성·라우팅 계약이 통째로 어두워지고,
화면에는 "93건 실패"로 보여 진짜 결함과 구별되지 않았다.

★ 곁다리로, 두 검사의 "결과 0건이면 건너뛴다" 분기가 **한 번도 동작한 적이 없었다** —
`homeHtml.includes('/properties/')` 가 헤더의 `/properties/recent` 링크에 항상 걸렸다.

★ 초록이 skip 덕분이 아님을 증명하려고, 사본에 미래 매각기일 300건을 넣고 그 사본을
읽는 API + Next 를 따로 띄워 같은 스위트를 돌렸다 — **114/0/0**.

**가드 갱신은 전부 강화 방향이다**

`test_search.py` 의 목록 화면 계약이 제공 필드를 `auction_item 컬럼 + {favorited_at}` 으로
**가정**하고 있어, 계산된 키(`thumbnail_url`)를 표현하지 못하고 **없는 결함을 보고했다.**
소스에서 읽을 수 있는 사실을 손으로 베껴 적으면 반드시 어긋난다 — 상세페이지처럼 AST 로
읽게 바꾸고, 스키마 결합(`row["X"]` 의 X 가 실제 컬럼인가)은 따로 보게 했다.

전체: **40 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,540**.

남은 FAILED 2건은 둘 다 가드가 옳게 말하는 것이고, 해소책이 각각 실제 크롤 실행과
`git add` 뿐이라 **승인 영역이라 하지 않았다.**

여전히 확인하지 못한 것: **모바일 실뷰포트.** 이번에는 iframe(390px) 으로 재려 했으나
앱 자신의 `X-Frame-Options: DENY` 에 막혔다 — 앱이 옳게 동작한 것이고 측정 경로가 닫힌 것이다.

### Sprint 225 - 접근성 주장 정정 / 가드 사각지대 (2026-08-20)

**화면 픽셀은 하나도 바꾸지 않았다.** 고친 것은 문서와 검사다.

**★ 문서가 하던 단정이 틀렸다 (BUGS #158)**

Sprint 219 는 헤더 내비(높이 16px)를 *"WCAG 2.5.8 의 24px 미만이라 **규격 위반**"* 이라
단정하고 그것을 근거로 개선 **1순위**에 올렸다(*"위반이라 디자인 논쟁 여지가 가장 적다"*).

SC 2.5.8 에는 예외가 다섯 개 있고 그중 **간격(Spacing) 예외**가 그대로 적용된다.
실브라우저에서 기하를 직접 계산했다.

```
/search 5개 · 상세 9개 · /mypage 5개 가 24px 미만
원이 겹치는 쌍                       **0쌍** (세 화면 전부)
가장 가까운 중심 간 거리              54px  (임계 24px 의 2.25배)
-> SC 2.5.8 위반 0건. 적합.
```

1순위의 근거가 사라진다. 헤더 확대는 규격 준수가 아니라 **순수 디자인 판단**이고,
진짜 규격 위반은 **대비(SC 1.4.3)** 쪽에만 있다. 같은 서술이 퍼져 있던 **6곳**을
전수 정정하고, 정정이 지워지면 검사가 울도록 문서 근거 토큰에 넣었다.

**가드가 보지 못하던 곳 (BUGS #159 / #160)**

```
스캔 범위가 src/app 뿐   -> src/components 의 text-xs 7 / gray-400 4 미집계
                            하필 SiteHeader/PrimaryNav = Sprint 219 가 지목한 헤더 내비
                            **공용 컴포넌트로 옮기면 검사에서 사라진다**
상한만 있음              -> 추적 밖의 더 나쁜 색(gray-200)으로 갈아타면 오히려 초록불
범위를 넓힌 첫 실행      -> 검출기 자신의 버그가 드러났다(max-w-[180px] 를 고정 폭으로 오인)
                            **범위가 좁으면 검사기 버그도 함께 숨는다**
```

**색 대비 실측** — 캔버스 래스터화로 oklch/lab 변환을 브라우저에 맡겼다.

```
text-gray-300 1.47 · text-gray-400 2.60 · text-red-400 2.89 · text-blue-500 3.76   미달
text-gray-500 4.84 · text-blue-600 5.25 · text-gray-600 7.56 · text-gray-700 10.30 통과
흰 글씨 on bg-blue-500  3.76   <- **주 CTA(검색/저장/정렬칩)**
/search 36개 중 20개 실패 · 상세 100개 중 49개 실패 (14px 미만 38개, 최악 1.47:1)
```

Sprint 219 표에 없던 것 둘: **주 CTA 버튼**과 **오류 문구(red-400)** 다.
오류 문구는 가장 읽혀야 하는 글자인데 본문 회색과 비슷한 수준이다.

★ 측정 도구가 또 거짓값을 낼 뻔했다 — Tailwind v4 는 **소스에 쓰인 클래스만** CSS 로
만들어서, 없는 클래스를 붙이면 상속색이 읽힌다(`text-blue-700` 이 `--foreground` 로
읽혀 17.93 이 나왔다). "클래스가 실제로 적용됐는가"를 먼저 보고, 아니면 **측정 불가**라고
말하게 고쳤다.

최소 교체안(같은 색조에서 한 단계씩)을 문서에 남겼다 — **적용은 제품 결정이라 하지 않았다.**

전체: **40 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,546**. 변이 4/4.

여전히 못 하는 것: **모바일 폭 판정.** 이번 2.5.8 결론은 데스크톱 1920px 기하이고,
좁은 폭에서 헤더 항목이 가까워지면 간격 예외가 깨져 **진짜 위반**이 될 수 있다.

### Sprint 226 - 제어문자로 굳은 이스케이프 (2026-08-20)

Sprint 225 문서를 쓰다가 **내가 만든 문서에** 백스페이스 바이트가 들어갔다. 원인을
확인하려고 저장소를 훑었더니 **내 것이 아닌 것이 하나 더** 있었다.

```
tests/source-contract.test.mjs   \b x2   <- git diff 비어 있음 = 원래 그랬다
```

**절대 실패할 수 없던 단언 (BUGS #161)**

```js
assert.ok(!/<0x08>formatPrice<0x08>/.test(src), '마이페이지가 축약 표기를 씁니다')
```

단어 경계로 쓰려던 것이 **백스페이스 문자 자체**로 굳어, 소스에 있을 리 없는 것을 찾게 됐다.
`!` 가 붙어 있으니 단언은 **항상 참**이다. 지키려던 것은 축약 표기가 12,900원을
"1만"(-22%)으로 만들어 **같은 결제가 화면마다 다른 금액으로 보이는** 결함이다.

★ 겉보기로는 알 수 없다 - 에디터가 그 바이트를 보여 주지 않고, 문법 오류도 아니고,
테스트는 초록불(23/23)이다. 지금 코드가 `formatWon` 을 써서 **결과까지 맞다.**
틀린 것은 "그것이 검증되고 있다"는 믿음뿐이었다.

```
결함 주입(formatWon -> formatPrice) 후
  수정 후 가드        [검출]  테스트 실패
  수정 전 가드(재현)   [놓침]  테스트 **통과**
```

**전수 탐색 + 상시 가드 (BUGS #162)**

390개 파일에서 `\0 \a \b \v \f \e` 계열을 전부 찾았다 - **발견 1곳**(위 그것뿐).
`test_console_encoding.py` 에 상시 검사를 넣었다(대상 242개, 고정 0개, 하한 80,
검출기 자체 검증 포함). 변이 3/3 검출.

**원인과 대처**: 파일을 도구를 거쳐 쓸 때 역슬래시가 한 겹 사라지면 두 글자가 제어문자
한 글자로 굳는다. 이번에도 실시간으로 났고 고치려던 스크립트마저 같은 경로에서 또 깨졌다.
**역슬래시가 들어가는 내용은 heredoc 으로 넘기지 말고 파일로 써서 실행한다.**

전체: **40 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,550**.

### Sprint 227 - 마이리스트 내보내기 (2026-08-20)

`docs/SPRINT219B` 가 **"안전"** 으로 분류해 둔 두 가지만 만들었다 —
형식 중립 CSV 내려받기와 탭 구분 클립보드 복사. **상대 서비스 전용 포맷은 만들지 않았다**
(실제 입력 형식을 여전히 확인하지 못했고, 추측하면 "붙여넣었는데 안 들어간다"가 된다).

**데이터를 왜곡하지 않는다**

```
병합 사건(실측 22.7%)   원본 문자열 그대로 - 어느 쪽이 대표인지는 법원이 정하지 않았다
가격                     숫자 원본 - 축약하면 12,900원이 "1만"(-22%)이 된다
없는 값                  빈 칸 ('-' 나 0 을 넣지 않는다)
유찰 0회                 "0"  (`!v` 로 판정하면 "없음"이 "모름"이 된다)
```

**회귀 — 이 저장소에서 처음으로 TypeScript 를 실행해 검증했다**

지금까지 프런트 검사는 소스 문자열 대조와 HTTP 블랙박스 둘뿐이라
"쉼표가 든 주소를 제대로 감쌌는가"를 볼 수 없었다. Node 24 가 타입 스트리핑을 기본
지원해서(`process.features.typescript === 'strip'`) **새 라이브러리 없이** `.ts` 를 직접
import 해 호출한다. `tests/export-list.test.mjs` 23검사.

그리고 우리 코드로 만든 것을 우리 코드로 파싱하면 같은 실수를 두 번 해도 통과하므로,
**파이썬 csv 모듈**(바깥 소비자)로 다시 읽어 확인했다.

```
BOM 있음 / 헤더 10열 / 모든 행 칸 수 일치 / '상가,오피스텔,근린시설' 한 칸 유지
가격 숫자 원본 / 한글 온전
cp949 로 읽으면 UnicodeDecodeError  -> BOM 이 필요한 근거
```

★ 도중에 "관심물건 0건"이 나와 헤맸는데 데이터가 없는 게 아니라 **토큰이 401** 이었다 —
검증 스크립트가 자기만의 JWT 비밀값을 만들어 서버(.env 값)와 달랐다.
*인증 실패가 "데이터 없음"으로 보이는 혼동*이라 스크립트에 이유를 남겼다.

★ 실브라우저에서 BOM 을 확인하다 도구가 또 거짓말했다 — `Blob.text()` 는 디코딩에서
**선행 BOM 을 제거한다.** 문자열로는 판정할 수 없고 `arrayBuffer()` 로 봐야 한다(있었다).

**가드가 내 새 코드를 두 번 잡았고 둘 다 옳았다**

```
text-xs 상한 초과        -> 상한을 올리지 않고 text-sm(14px)으로 바꿨다
상태 메시지를 두 노드로   -> 항상 존재하는 role=status 한 줄로 합쳤다
                            (가드 예외 정규식이 속성 순서에 의존했지만 **피하지 않고** 구조를 고쳤다)
```

**★ 곁다리 (BUGS #163)**: 그 과정에서 **상한 계수기가 주석을 코드로 세고 있었다**는 것이
드러났다. `text-sm 을 쓴 이유` 를 주석으로 적었더니 그 안의 `text-xs` 가 사용으로 잡혔다.
이 파일은 `_strip_comments()` 를 이미 갖고 있는데 계수기만 안 썼다.
반대 방향도 해롭다 — **주석을 지우면 상한에 여유가 생겨 진짜 사용이 늘어도 통과한다.**
고치고 상한을 코드 기준으로 재기준했다(text-xs 118 -> **117**). 변이 2/2.

전체: **40 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,550**.
프런트 **137검사 / 133 통과 / 1 실패(데이터 전제) / 3 skip**.

### Sprint 228 - 남의 사건 파일 / 묶기 재측정 (2026-08-20)

**★ 다운로드가 어느 사건의 것인지 아무도 확인하지 않았다 (BUGS #164)**

감사가 "정리 대상"으로만 적어 둔 고아 다운로드 8개의 **이름**에서 출발했다.
같은 매각물건명세서가 Chrome 중복 접미사와 함께 **네 번** 쌓여 있었다 —
네 번 받고 네 번 다 못 옮겼다는 뜻이다.

`wait_for_download()` 는 "새로 생긴 PDF" 를 집을 뿐 사건을 보지 않는다.

```
1. 사건 A 수집 -> 타임아웃 포기. **다운로드는 계속 진행 중**
2. 사건 B 수집 시작 -> before_files 스냅샷
3. A 완료 -> A.pdf 가 **새 파일**로 잡힌다
4. A 의 문서가 B 로 저장된다
```

저장물은 **진짜 PDF** 라 크기·해시·READY·화면 표시가 전부 정상이고 무결성 감사
[1]~[9] 를 통과한다. **사용자는 다른 사건의 명세서를 보고 입찰을 판단하게 된다.**

파일명의 사건번호로 대조하되 **확실할 때만 막는다** — 사건번호가 없는 감정평가서까지
막으면 멀쩡한 수집이 전부 죽는다. 병합 사건은 기존 판정 함수를 재사용했다.
회귀 14검사, 변이 4/4.

**묶기는 다시 재고 구현하지 않았다**

```
대기 + 기일이 남은 물건   **0개**   (Sprint 199 당시 2개)  ->  절감 0초
```

착수 조건이 더 나빠졌다. 이득 0인데 claim/retry 전제를 흔들 이유가 없다.

★ "만료 행 2,753개가 실제 작업을 굶기는가"를 의심했는데, **사본에서 실제 함수로 재니
16.1초**였다(문서 1건 이동 18.2초보다 싸다). **내 의심이 틀렸고, 고칠 것이 없었다.**
추정으로 최적화하지 않는다.

**파이프라인 전수 확인** — 서버를 띄운 상태에서 [9] 가 처음으로 판정됐다:
**API 가 광고한 URL 601개 전부 열림**(사진 45 + 문서 556). 서버가 없으면 이 항목은
"확인하지 못함"이고, 그것을 "이상 없음"으로 읽으면 안 된다.

전체: **40 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,564**.

### Sprint 229 - 변경 감지 감사 (2026-08-20)

Backlog #10 의 두 줄("없음과 실패를 구분" / "관측 공백과 변경 없음을 구분")을 기준으로
"0건"이 나올 때마다 **왜 0인가**를 되물었다. 운영 DB 는 건드리지 않았다.

**상태 어휘는 옳다** — READY / NO_IMAGE(없음) / FAILED(실패) / COLLECTING(모름)이
코드에서 분리돼 있고 모순도 없다(사진 없음인데 행 있음 0, READY인데 행 없음 0, 해시 없는 행 0).

**★ 그런데 "변경 0건"은 변경이 없다는 뜻이 아니었다**

```
doc_raw 556행 / doc_version 전부 1 (2 이상 0건)
큐의 refresh 행 0개
마지막 문서 수집 시도  2026-07-12   <- 39일 전
```

재수집이 39일간 한 번도 돌지 않았다. 바뀌었는지 **관측한 적이 없다.**
변경 감지 로직 자체는 합성 픽스처 회귀가 계속 검증한다 — 없는 것은 운영 관측뿐이다.

**★ 절대 실패할 수 없는 단언을 또 찾았다 (BUGS #165)**

```python
check_true("측정할 수 있다(조회 경로가 유효)", expired_collecting >= 0)
```

`COUNT(*)` 는 언제나 0 이상이다. 조인이 깨져도 통과한다. Sprint 226 의 #161 과 같은 계열.
**깨질 수 있는 불변식**(만료+진행중+고아 == 전체)으로 바꿨고, 그 덕에 **고아
`document_status` 행을 처음으로 세게 됐다**(현재 0개). 변이로 옛 판본이 놓치는 것을 확인했다.

**보류된 제품 결정의 비용이 16배로 자랐다**

```
                                 Sprint 73(08-14)   지금(08-20)
영원히 끝나지 않는 COLLECTING          183            2,921
큐 행이 아예 없는 물건 715 x 3종                      2,145
기본 검색에 뜨면서 COLLECTING 인 물건                     0
```

화면은 "수집중"(=기다리면 온다)이라 말하는데 아무리 기다려도 오지 않는다.
그래도 **결함이라 부르지 않는다** — 원인은 상태머신이 아니라 크롤 정지로 큐가 전부
만료된 것이고, 수집이 재개된 뒤에도 남는지는 그때 다시 재야 판정할 수 있다.
`document_status` 에 "대상 아님"을 만들지는 여전히 제품 결정이다.

전체: **40 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,564**.

### Sprint 230 - 무인 운전 / 스케줄 산술 / 다른 물건 사진 (2026-08-20)

목표를 *"Scheduler 가 붙으면 장시간 자동으로 도는 시스템"* 으로 놓고 승인 없이 가능한 것만 했다.

**★ 다른 물건의 사진이 저장될 수 있었다 (BUGS #166)**

코드가 스스로 위험을 적어 두고도 막지 않고 있었다 —
*"물건 사진에는 버튼이 없다. 잘못된 물건의 페이지에 있으면 그대로 잘못된 사진을 저장한다."*
그런데 물건번호를 못 찾으면 **경고만 남기고 첫 행으로 진행**했다.

저장물은 **진짜 사진**이라 크기·해시·READY·브라우저 표시가 전부 정상이고 무결성 감사
[1]~[9] 를 통과한다. 사용자는 다른 물건의 사진을 보고 입찰을 판단하게 된다.
문서는 버튼 id 가 물건을 특정해 안전하다 — **사진만 다르다.**

**모호할 때만** 거부하게 했다(후보 1개거나 물건번호 정보가 없으면 막지 않는다).
회귀 9검사, 변이 4/4.

**스케줄 산술의 마지막 다리 (BUGS #167)**

```
PriorityRefresh 01:50  ↔ config 검사 있음
DocWorker       02:00  ↔ config 검사 있음   창 종료 04:00
DailyCrawl      06:00  ← config 상수도 없고 **어떤 검사도 참조하지 않았다**
```

셋 중 **둘이 Chrome 을 띄우는데 서로를 막지 못한다**(각자 자기 락만 갖는다).
`DOC_WORKER_END_TIME` 을 07:00 으로 늘리면 곧바로 겹친다. 그 관계를 잠갔다.
하드코딩한 배치 목록 ↔ PS1 대조도 함께 넣었다(네 번째 진입점이 생기면 실패 은폐 검사가
그것만 비껴간다).

**장시간 무인 운전 시뮬레이션 신설**

7일 연속으로 실제 함수만 돌렸다(운영 DB 무접근).

```
일별 증가폭 [16, 16, 16, 16, 16, 16]   누수 없음
불변식 위반 0 / auction_item 28행 / document_status 106행
크래시 회수 시 refresh 의도 보존 / 재실행 멱등 / 만료 200행 starvation 없음
```

★ 첫 판본이 **공허하게 통과**했다 — `upsert_batch` 는 `auction` 에 쓰고 `auction_item` 은
`migrate_execute` 가 채우는데 그 단계를 빼먹어 조인 불변식 2개가 항상 빈 결과였다.

★ 변이 하나가 "놓침"으로 나왔는데 **변이가 결함을 못 만든 것**이었다(UNIQUE 가 막는다).
가드를 의심하기 전에 변이를 먼저 검증한다.

**이미지 체인 실브라우저 관통** — 화면 URL == 검색 API == 상세 API (4건 전부),
자산 URL 601개 전부 열림, 검색목록 1,876건/썸네일 9개, 상세 사진 5장.

★ 하마터면 없는 결함을 보고할 뻔했다. `naturalWidth: 0` 의 원인은 앱이 아니라
**탭이 hidden 이라 Chrome 이 lazy 이미지 로드를 미룬 것**이었다(서버는 정상 JPEG 200,
새 Image() 는 성공, currentSrc 는 비어 있었다). Sprint 223 포커스 함정과 같은 계열이다 —
**보이지 않는 탭에서 잰 lazy 이미지 상태는 근거로 쓰지 않는다.**

전체: **41 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,701**.

### Sprint 231 - 좁은 폭 레이아웃 실측 (2026-08-20)

Sprint 219 이래 계속 *"모바일에서 깨지지 않는다고 아직 주장할 수 없다"* 고 적어 왔다.
이번에 **처음으로 실제 레이아웃 값을 얻었다.**

**측정 경로부터 다시 확인했다** — `resize_window(390x844)` 는 성공을 반환하지만
`innerWidth` 는 1568 그대로고 `md:` 미디어쿼리도 여전히 참이다. 뷰포트는 못 줄인다.

**대신 서브트리를 고정 폭 컨테이너에 복제**해 브라우저가 그 폭의 레이아웃을 실제로
계산하게 했다(`clone.scrollWidth - w`). 가짜 산술이 아니라 진짜 줄바꿈/축소 계산이다.
...
★ **한계를 먼저 못 박는다.** 복제본도 실제 뷰포트로 미디어쿼리를 평가하므로
`md:`/`xl:` 로 열 수가 바뀌는 컨테이너에는 못 쓴다. 실제로 결과 그리드가
"288px 에서 157px 초과"로 나왔는데 **결함이 아니라 기법의 한계**였다(3열을 그리려 한 것).
오탐을 결함으로 보고하지 않았다.

★ 내보내기 줄(Sprint 227 에 내가 넣은 것)이 좁은 폭에서 넘칠 것으로 **의심했는데
틀렸다** — 상태 문구가 flex 안에서 축소되고 한글이 글자 단위로 줄바꿈된다.
산술로 추정했으면 없는 결함을 보고했을 것이다.

### Sprint 232~233 - 재시작 제거 / Admin 키 회귀 / 전면 Audit (2026-08-20)

**★ '사건을 못 찾았다'로 드라이버를 통째로 재시작하고 있었다 (BUGS #168)**

`MAX_ITEMS=10` 을 의심해 조사를 시작했는데 **그 가설은 틀렸다** — 매칭 실패 사건 62개 중
10건 초과 조합은 **2개뿐**(나머지 60개는 10건 이하)이었다. 가설을 폐기했다.

대신 그 과정에서 진짜 결함이 보였다. `go_to_case_detail()` 이 False 를 돌려주면
일반 예외로 올려 **드라이버를 재시작**하고 있었다. 그런데 False 의 이유는 둘 다
정상적인 판단이다 — 사건이 목록에 없거나, **Sprint 230 이 일부러 거부**한 것이다.
즉 옳은 판단이 재시작을 불렀다.
...
시간 자체는 크지 않다. **진짜 문제는 연쇄다** — 재시작이 실패하면 Sprint 137 방어가
그 날 실행 전체를 중단한다. 전용 예외로 분리해 재시작 없이 넘어가게 했다.
회귀 2종(대조군 포함), 변이 3/3.

**★ Admin API 키가 다시 사라졌다 (BUGS #169) — P1 이 아니라 P0**

체크리스트는 *"2026-08-16 두 키 모두 설정 확인, 정상 403/200 — UI 없음만 남음(P1)"* 이라
적고 있었다. 오늘 두 가지 독립된 방법으로 재니 **둘 다 `.env` 에 이름조차 없고**,
전 라우트를 토큰 없이 호출하니 `/admin/*` **13개 전부 500** 이었다.
`.env` 변동은 세 번째다(08-08 있음 → 08-13 없음 → 08-16 있음 → 08-20 없음).
운영자가 할 수 있는 일이 하나도 없다. 표를 P0 로 되돌렸다(`.env` 수정은 승인 영역).

**전면 Audit** — 라우트 38개(보호 16/공개 8, 공개 8개에 관리 기능 없음.
**"개인정보 없음"은 2026-08-28 에 사실이 아님이 밝혀졌다 — `docs/BUGS.md` #254**),
Cache(자산 304 절약 확인, JSON 캐시 헤더 없음), 성능(search 14.8~20.6ms, item 4.3ms),
접근성(컨트롤 167개 전부 이름 있음, focus-visible 위반 **0건**),
죽은 코드(304함수 중 미참조 **0개**).

**★ 도구 오류 4건을 걸러냈다.** MAX_ITEMS 가설 / 자산 ETag "없음"(헤더 대소문자) /
포커스 표시 "없음"(핸들러 안에서 읽은 타이밍) / `element.focus()` 로 focus-visible 판정.
3번에서 멈췄다면 **없는 결함 7건**을 보고했을 것이다.

전체: **41 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,709**.

### Sprint 234 - Admin Secret 계약 / MAX_ITEMS 실측 (2026-08-20)

**P0 Admin Secret — 로딩 경로 전수 Audit**
...
★ 측정 도구가 두 번 틀렸다 — `load_dotenv()` 없이 읽고, 스크래치에서 인자 없이 호출해
(`find_dotenv` 는 **호출한 파일의 디렉터리**부터 훑는다) *"JWT 시크릿 없음"* 이라는 거짓
결과를 냈다. 2차에서 멈췄다면 **없는 P0(인증 전체 장애)** 를 보고했을 것이다.
경로를 명시하니 88자로 정상이었다.

**회귀 신설 (BUGS #170)** — `test_admin_secret_contract.py` 30검사.
비어 있던 중간 상태를 잠갔다: **틀린 키 -> 403**(500 아님) / 등급 분리 /
**Secret 부재가 일반 사용자 API 에 영향 없음**(공개·인증 API 전부 200, Admin 만 500).
Admin 라우트는 OpenAPI 에서 16개 전수 추출한다. 변이 **4/4**.

**MAX_ITEMS = 10 실측** (정책 변경은 SKIP — 운영 판단)

```
상한 도달  104회 중 12회 (11.5%)      DB 상한 밖 물건 31건 (1.7%)
비용       물건당 3.5초 x 법원 60개    20 -> +0.6h / 30 -> +1.2h / 50 -> +2.3h
★ 진짜 병목은 일일 크롤이 아니라 **문서 큐** — 물건 1건 = 큐 4행, 창은 120분뿐
```

**큰글씨** — `text-[NNpx]`/`leading-[NNpx]`/인라인 fontSize px **전부 0개**.
유일한 `max-h-[420px]` 는 `<img>` 라 무관. **진짜 장애물 0개**, 남은 것은 제품 결정뿐.

**모바일** — 검색목록/버튼줄/상세 카드가 **320·360·390·430px 전부 넘침 0**.

**문서 ↔ 코드 이름 대조** — `DOC_WORKER_TEST_MODE`(실행 창 무력화 플래그)가 문서에
없어 보강(BUGS #171). `SUPABASE_ANON_KEY` 는 `.env` 에 있으나 읽는 코드 0곳.

전체: **42 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,736**.

### Sprint 235 - 문서 워커 처리 능력 (2026-08-20)

**★ 정확성 검사는 촘촘한데 능력을 보는 검사가 하나도 없었다 (BUGS #172)**

능력 부족은 오류를 남기지 않는다. 큐는 정상적으로 쌓이고 워커도 정상적으로 돌지만
따라잡지 못하고, **큐 길이만 매일 조금씩 늘어난다.**

```
실측   navigate 908회 / 처리 901건 -> 처리 1건당 이동 1.01회
       navigate 1회 중앙값 **10.9초** (904구간, p90 16.0)
       doc_type 수 = 물건당 이동 횟수 (워커가 행마다 이동한다)

능력   창 7,200초 / 4종 x 10.9초 = 물건당 43.6초  ->  하루 **165건**
공급   중앙값 106건 -> 감당 / 최대 278건 -> **113건 밀림**
       이론 600건(MAX_ITEMS 10 x 법원 60) -> **434건 초과**
```

★ 큐에 `image` 가 **0행**이다(Sprint 144 이전 적재분). 새 적재부터 4종이 되어 이동이 는다.
사진 수집 자체는 **0.0초**인데 그것을 위한 이동만 10.9초 더 든다.

**batching 이득 4.0배**(165 -> 661건/일)로 이론 최대까지 덮지만, claim 단위가
행 -> 물건으로 바뀌는 구조 변경이라 **하지 않았다**(운영 판단). 근거만 숫자로 남겼다.
**`MAX_ITEMS` 상향은 batching 다음**이라는 순서가 계산으로 확정됐다.

**회귀 신설** `test_worker_capacity.py` 15단언 — 세 값의 산술 관계(doc_type x 이동비용 vs
실행창 vs 실제공급)를 잠근다. 모델 근거(행마다 이동 / 한 행만 claim)도 코드에서 확인한다.
★ **래칫**을 함께 걸었다 — 중앙값만 보면 doc_type 6종까지 통과하기 때문이다.
변이 3/3, 그중 doc_type 추가는 **래칫만** 잡았다.

**7일 시뮬레이션을 다시 믿지 않고 검증** — stub 없음 / 이름 9개 전부 실제 함수 /
런타임 추적으로 claim 402회·skipped 200회·done 88회 실제 호출 확인.

전체: **43 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 6,751**.

### Sprint 235 이어서 - UI 와 backend 연결 감사 (2026-08-20)

OpenAPI 전 라우트(38개)와 프런트 소스 전수를 대조했다.

**★ 위험한 방향(UI 는 있는데 backend 가 없다)은 0건**

```
button 44개 전부 onClick/submit 을 갖는다   <- 누르면 아무 일도 안 하는 컨트롤 없음
form 2개 전부 onSubmit 보유
SearchForm 의 미지원 표시 3개는 **의도된 것** (test_search.py 가 실제 미지원과 대조한다)
```

검출기는 known-good/known-bad 로 자체 검증했다 — `onClick={() => f(a > b)}` 의 `>` 에서
태그가 잘리지 않는지 확인했다(이 저장소가 `<select[^>]*>` 로 당한 전례가 있다).

**backend 는 있는데 UI 가 없는 것 (기능 공백, 결함 아님)**

```
/api/v1/stats                        집계 - 화면 없음
/api/v1/document-stats               집계 - 화면 없음
/api/v1/payments/{id}                결제 상세 - 화면 없음 (목록도 없음)
/api/v1/payments/{id}/logs           결제 로그 - 화면 없음
/api/v1/registry-requests/{id}       신청 상세 - 목록(/registry-requests)만 마이페이지에 있다
/api/v1/admin/* 16개                 Admin UI 없음 (이미 알려진 P1)
```

★ `/api/v1/item/{id}/images/{seq}` 는 프런트 소스에 문자열로 **없지만 살아 있다** —
API 가 준 `thumbnail_url` 을 그대로 `<img src>` 에 넣기 때문이다.
**grep 부재를 미사용으로 읽으면 살아 있는 경로를 죽었다고 오판한다.**

**사용자 체인 최종 확인(실브라우저)**

```
총 1,876건 / 카드 20개 / 썸네일 9개 / 뷰포트 내 6개 **전부 디코딩**
(522x700 / 700x393 / 700x525 / 525x700)
가로 오버플로 0 · main 1 · h1 1 · 이름 없는 컨트롤 0
```

### Sprint 236 - 물건 단위 batching (2026-08-20)

**★ Sprint 235 의 숫자를 그대로 믿지 않고 다시 쟀더니 둘 다 틀렸다**

| | Sprint 235 | Sprint 236 실측 |
|---|---|---|
| 이동 1회 비용 | 10.9초 | **15.2초** |
| 현재 능력 | 165건/일 | **78건/일** |
| batching 이득 | 4.0배 | **1.97배** |

'navigate 10.9초'는 `법원 선택` 줄 사이 간격이었는데, 거기에는 **수집과 sleep 이
들어 있다** - 이동 비용이 아니라 **한 행의 전체 주기**였다.

```
모델 없이 잰 값   실행 7회 합계 897행 / 5.8시간 -> 행 1개당 **23.2초**
                 행/이동 = 1.00 (7회 전부)
이동 1회 15.2초   Sprint 147 독립 측정 = 이번 구간 분해(16.4초)와 일치
```

-> 예전 구조는 **실측 공급 중앙값(106건)조차 감당하지 못하고 있었다.**

**구현 결과**

```
이동      48회 -> **12회**  (물건 12 x 4종, 진짜 doc_worker.main() 실측)
처리량    78 -> **153건/일 (1.97배)**
공급      중앙값 106 -> **감당** / 최대 278 -> 여전히 밀림(부족 125)
큐 현황   3,498행 / 대기 2,753행 / 대기 물건 944개 -> 소진에 약 6.2일
```

★ **이동은 4배 주는데 처리량은 2배다.** 수집과 sleep 은 종류마다 그대로 든다.
Sprint 235 의 '4.0배'는 처리량이 아니라 이동 감소 배수였다.

**깨뜨리지 않은 것** (전부 행 단위 유지, 43검사로 고정)
재시도 예산 / 성공·실패 기록 / refresh 의도 / 부분 실패 / 사진 정확일치 /
집은 행 방치 없음 / 묶음 안 종결 행이 나머지를 버리지 않음. 변이 **8/8**.

**★ 공허했던 가드를 고쳤다** - Sprint 235 가 *"batching 이 들어오면 여기서 먼저 운다"* 고
적어 둔 검사가 `doc_worker.py:266` 의 **주석 한 줄**에 걸려 초록불을 유지했다.
주석·독스트링 제거 + **실제 호출** 확인으로 교체. 변이 C5(옛 이름을 주석에 삽입)가
**통과**하는 것으로 "주석을 코드로 읽지 않음"을 증명한다.

**다음 (승인 필요)** 실행 창 02:00~**05:38** 로 확대하면 최대 공급일을 덮고
06:00 크롤과 겹치지 않는다 - 가장 값싼 다음 수. `MAX_ITEMS` 상향은 그 다음.

전체: **44 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 7,007**.

### Sprint 236 이어서 - 출시 감사 (2026-08-20, 전부 이번 세션 실측)

**자산 체인 (읽기 전용, 운영 DB 무변경)**

```
auction_image 45행   고아 0 / 디스크 파일 없음 0 / 크기 0인 행 0
                     이미지 라우트 45개 전부 **200 + 본문 있음**
document_status      COLLECTING 5,069 / READY 556 / FAILED 3
  READY(문서) 556행  고아 0, 표본 80개 전부 **200**
검색 목록 썸네일       9개 전부 열림 (검사가 공허하지 않다 - 실제로 열어 봤다)
큐 done인데 화면 FAILED   **0행**
```

★ COLLECTING 5,069 은 결함이 아니라 **밀린 큐가 화면에 보이는 것**이다.
대기 물건 944개 / 능력 153건/일 -> 소진에 약 6.2일.

**UI 와 backend 연결** - 위험한 방향(UI 는 있는데 동작이 없다) **0건**.
button 44개 전부 onClick/submit, form 2개 전부 onSubmit. 검출기는 known-good/known-bad 자체검증.

**Cache / 성능 (HTTP 실측)**

```
이미지  ETag+LM 있음 -> 조건부 재요청 **304**, 235,194바이트 절약
문서    ETag+LM 있음 -> 조건부 재요청 **304**, 395,675바이트 절약
JSON    ETag/Last-Modified/Cache-Control **없음** (알려진 공백, 결함 아님)

검색 size=20   2.7ms      검색 size=100  4.8ms
상세           2.2ms      통계          2.0ms      문서상태 2.9ms
page 1 -> 94   2.6ms -> 4.2ms   깊은 페이지에서 급격히 나빠지지 않는다
```

★ 처음에 `limit`/`offset` 으로 재고 "페이지네이션이 무시된다"는 결론을 냈다.
**도구가 틀렸다** - 이 API 의 파라미터는 `page`/`size` 다(OpenAPI 로 확인).
다시 재니 size=5 -> 5건, size=100 -> 100건, page 94 -> 16건(1876 = 93x20 + 16)으로
정상이었다. *측정값이 이상하면 코드보다 도구를 먼저 의심한다* 를 그대로 적용했다.

**접근성 / 모바일**

```
검색   main 1 / h1 1 / 이름 없는 컨트롤 0 / alt 없는 img 0 / heading H1,H2
상세   main 1 / h1 1 / 이름 없는 컨트롤 0 / img 6개 전부 디코딩 / H1,H2,H3...
관심   main 1 / h1 1 / 이름 없는 컨트롤 0
가로 넘침  검색 326개 요소 + 상세 102개 요소 검사 -> **실질 0건**
```

★ **모바일 폭(320/360/390/430) 실측은 이번 세션에 하지 못했다.**
`resize_window` 가 성공을 보고하지만 뷰포트가 그대로였다(숨은 창, `outerWidth=0`).
**성공 보고를 그대로 믿지 않고** 뷰포트를 다시 재서 알았다.
대신 **breakpoint 와 무관한** min-content 너비로 쟀다 - 320px 에 못 들어가는 요소는
어떤 미디어쿼리로도 구제되지 않으므로 이 검사는 유효하다. 결과 0건.

걸러낸 거짓 후보 2개:
```
main 의 정렬칩 줄 (323px)   -> `overflow-x-auto` 가 붙어 있다. **의도된 가로 스크롤**
상세의 img (522px)          -> min-content 는 원본 너비다. 320 컨테이너에 넣으니
                              320x420 으로 렌더된다(max-width:100%). **측정 산물**
```

★ 관심물건은 **0건이라 카드 레이아웃이 실행되지 않았다.** 폭 검사가 통과한 것이
아니라 **볼 것이 없었다** - 빈 데이터를 통과로 읽지 않는다.

★ 검색 목록 사진 9개가 처음에 전부 `naturalWidth=0` 이었다. `document.visibilityState`
가 `hidden` 이라 lazy 이미지가 지연된 것이고, `loading=eager` 로 강제하니 9개 전부
디코딩됐다(522x700 등). **lazy 미로드를 이미지 결함으로 판정하지 않는다.**

**데이터 무결성** 운영 DB 세션 시작과 동일(auction_item 1876 / favorites 0 /
auction_image 45 / document_queue 3498 / document_status 5628 / recent_items 36).
`.env` 무변경(142바이트, Aug 11). git 인덱스 0. 제어문자 444파일 중 0. 스크래치 유출 0.

### Sprint 237 - 실행 창 / MAX_ITEMS / 캐시 (2026-08-20)

**실행 창 - 05:38 은 성립한다 (변경하지 않음)**

config 를 잠깐 바꿔 **실제 가드에 태워** 판정하고 되돌렸다(바이트 동일 확인).

```
04:00 통과 / 05:00 통과 / **05:38 통과** / 05:55 안전 상한
05:56 막힘(여유 4분) / 06:00 막힘(여유 0분) / 01:00 막힘(창 음수)
```

★ 가드에 구멍이 둘 있었다 (BUGS #175)
- `종료 <= 크롤시작` 이라 **06:00 == 06:00 을 통과**시켰다. 그런데 워커는 종료 시각에
  딱 멈추지 않는다 - 처리 중이던 행 하나는 지나서도 끝난다(실측 최대 **42.2초**).
  -> 여유 **5분** 요구로 강화.
- 창이 음수면 `limit >= window` 가 **언제나 참**이라 공허했다. -> 창 양수 검사 추가.

**MAX_ITEMS - 하나의 손잡이가 두 가지를 돌린다 (BUGS #174, 값 무변경)**

```
crawl_court()        = 공급 상한 (정책)
go_to_case_detail()  = 조회 창 (검색 반경)   <- 정책이 아닌데 같은 손잡이
```

진짜 함수로 절단 재현(19건 페이지 -> 10건, 19번째는 **못 찾음**).
크롤 로그 1,698회 중 **12.1%가 상한에 걸린다.**

★ 자료가 10에서 오른쪽으로 잘려 있어 "올리면 공급이 얼마"는 **알 수 없다.** 역산했다:

```
능력 153건/일 vs 공급 중앙값 106건 -> 여유 **1.44배**
창 05:38 이면 277건/일             -> 여유 **2.61배**
=> 창을 먼저 넓히고, MAX_ITEMS 는 그 다음
```

**★ 내 가설이 틀렸다 - 28% 미도달은 지금 결함이 아니다**

로그의 "상세페이지 미도달 28.1%"를 MAX_ITEMS 탓으로 의심했으나, 실행 시점 기준
재판정에서 **81%가 이미 지난 기일**이었고 MAX_ITEMS 로 설명되는 것은 **2건**뿐이었다.
게다가 그 로그는 **2026-08-02 까지**(형제 재사용 0회 / 정확 일치 0회) -
기일 경과 방어가 들어오기 **전**이다. 현재 결함으로 보고하지 않는다.
같은 이유로 시간 상수도 batching 이후 실측이 아니다 - **크롤 재개 후 재측정 필요.**

**캐시 (BUGS #176)** - 인증 요청 + 검증자 없는 응답에만 `Cache-Control: no-store`.
파일 응답은 제외 - 덮으면 실측 **630KB** 의 304 절약이 사라진다.

**관심물건/최근본** - favorites 0건이라 **최근본 실데이터**로 카드 경로를 태웠다:
카드 12 / 이미지 4장 전부 디코딩(522x700 등) / 주소·가격·링크 정상.
썸네일 계약은 사진 보유 4건에 정확히 4개 - 첫 표본이 사진 없는 사용자여서
**거짓 결함으로 볼 뻔했다.**

**모바일** - `truncate` 요소를 min-content 로 재면 과대보고한다(잘릴 뿐 안 넘친다).
클리핑 제외 + 실제 상자 측정으로 교체 -> 카드 **31개 전부 320/360/390/430 넘침 0**.
iframe 으로 진짜 viewport 를 만들려다 앱의 `X-Frame-Options: DENY` 에 막혔다 -
**측정 편의로 보안 헤더를 약화하지 않는다.**

**접근성** - px 기반 글자 크기 **0개**(전부 rem = 확대에 반응) / 탭 타깃 간격 위반 **0**
(최소 중심거리 54px) / 라벨 없는 입력 0 / focus 표시를 지우는 규칙 0 / 진짜 고정 치수 0.

전체: **46 PASSED / 2 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 7,064**. 변이 **9/9**.
---

## 2026-08-24 (Sprint 251~254)

이번 세션의 축은 **"검증되지 않은 채 돌고 있는 코드"** 다. 전체 스위트 합산 커버리지로
아무 테스트도 실행하지 않는 자리를 찾아 하나씩 열었고, 그 자리에서 동시성 결함 **4건**
(BUGS #180~#183)이 나와 전부 고쳤다. 넷 다 **동시 실행이 있을 때만** 드러나는 부류라
지금 화면에는 안 보이지만, 스케줄러를 등록하는 순간(승인 영역)부터 성립한다.

특히 #182 는 P0-A 와 직접 이어진다 - 크롤을 켜는 순간 브라우저가 죽으면 **로그와 종료
코드가 "정상"이라고 말한다.** 등록 전에 고쳐 두는 것이 맞다고 보고 처리했다.

방법론에서 반복해 확인한 것 하나: **살아남은 변이를 먼저 의심한다.** 이번에 세 번
있었는데 두 번은 검사의 구멍이었고(R4: 상수를 읽는 검사는 그 상수의 변경을 막지
못한다 / T7: 단위 검사는 제품의 방어가 옳음만 증명하고 호출부가 그것을 쓰는지는
증명하지 못한다) 한 번은 코드의 군더더기였다(도달 불가능한 분기를 도로 걷어냈다).
판단 기준은 하나다 - **다른 결과를 낼 수 있는 입력이 있는가.**

그리고 내가 두 번 틀렸을 때 **저장소의 가드가 두 번 다 먼저 잡았다**(cp949 출력 리터럴,
선언되지 않은 third-party 의존). 가드를 쌓아 온 이유가 그것이다.

자세한 내용: `docs/SPRINT254_CLAIM_RACE_BRANCHES.md`, `docs/BUGS.md` #180~#183,
`docs/BETA_RELEASE_CHECKLIST.md` 2026-08-24 절.

전체: **53 PASSED / 1 FAILED / 3 SKIPPED / 1 NO-VERDICT, 단언 8,123** /
node **187/191** / tsc 0 / eslint 0. 양쪽 1 FAIL 은 P0-A(승인 영역) 하나가 원인이다.


---

## 2026-08-26 (낮, 개발 머신) — 가드가 보지 못하던 자리들

이번 순회의 축은 **"검사가 스스로 정한 시야"** 다. 결함 6건(BUGS #220~#225)을 고쳤는데
그중 **넷이 제품 코드가 아니라 가드의 범위** 문제였다. 제품은 붉지 않았고, 검사가
자기 시야 밖을 조용히 통과시키고 있었다.

```
#220  면적 필터가 migration 025 미적용 DB 에서 검색 전체를 500 으로 만든다
      -> 같은 커밋 안에서 응답 쪽은 방어하고 필터 쪽은 안 했다. 변이 8/8
#221  .bat 3개의 한글 주석이 cmd 를 오파싱. run_daily.bat 은 **성공 경로에서 exit 255**
      이고 logs/ 조차 만들지 않는다(= 파이썬이 한 번도 안 돈다). 설명을 .md 로 옮기고 ASCII 화
#222  머신 역할 게이트가 §11 에만 걸려 §11-b 는 두 머신에서 **동시에 만족 불가**. 변이 3/3
#223  체크리스트 두 절이 동시에 "가장 최신"을 주장(숫자가 다르다). 세는 가드 신설
#224  지역 가드가 "오염" 부류를 통째로 못 봄. 같은 판정이 두 벌이었다(207 vs 208). 변이 4/4
#225  등기부/구독 핸들러 5개가 늦은 응답 가드 없음 -> **다른 물건의 등기부를 받는다**.
      검사가 "가드를 이미 가진 곳"만 훑는 옵트인 구조였다. 변이 2/2
```

**방법론에서 반복해 확인한 것 둘.**

1. **측정값이 뜻밖이면 코드보다 도구를 먼저 의심한다** — 두 번 걸렸다. `.bat` 를 이 셸에서
   돌렸을 때는 오류가 **0줄**이었는데, 그 셸의 cmd 가 코드페이지 **65001** 을 물려받은
   탓이었다(시스템은 949, 스케줄러도 949). 그리고 가드 확인 스크립트가 **한글 주석 안의
   "await "** 를 코드로 읽어 오탐을 냈다.

2. **살아남은 변이를 먼저 의심하되, 변이 자체가 빗나갔을 가능성도 함께 의심한다** —
   세 번 살아남았다. 둘은 검사의 구멍이었고(자체 검증 입력이 두 조건을 갈라 놓지 못했다 /
   두 컬럼을 한꺼번에 지워 한쪽만 보는 실수를 못 잡았다) 하나는 **변이가 엉뚱한 함수에
   적용된 것**이었다. 나머지 하나는 코드의 군더더기라 도달 불가능한 분기를 걷어냈다.

**그리고 내가 쓴 숫자 하나가 틀렸다** — 주석에 "PRAGMA 는 0.01ms 미만"이라고 **짐작해**
적었다가 재 보니 **0.128ms**(13배)였고 가장 싼 실제 쿼리보다도 비쌌다. 고쳐 적었다.

전체: **python 52 통과 / 4 실패**(단언 9,438) · **node 196건 192 pass / 0 fail / 4 skip** ·
tsc 0 / eslint 0 / build 0 / `auction.db` md5 무변경.
실패 4건은 전부 **이 머신 DB 가 뒤처져 있어서**이고(021~025 미적용 + 지역 backfill 미적용)
둘 다 승인 영역이라 실행하지 않았다. backfill 은 **dry-run 으로 424건 영향 범위를 확정**해 뒀다.

자세한 내용: `docs/BUGS.md` #220~#225, `docs/BETA_RELEASE_CHECKLIST.md` 최상단 절,
`docs/BATCH_SCRIPTS.md`(신설 — 배치 주석을 ASCII 로 옮기며 한글 설명을 여기로 뺐다).

### 같은 날 이어서 — 결제 경로와 문서 신선도 (BUGS #227~#228)

앞 절에 이어 Audit 루프를 한 바퀴 더 돌았다. 이번에는 **아직 손대지 않은 영역**부터 갔다.

```
#227  할인 기간을 **문자열로 비교** -> 월을 한 자리로 적으면 할인이 끝나지 않거나
      시작되지 않는다. 오류도 로그도 없이 청구 금액만 달라진다. 변이 6/7
      (남은 하나는 2,557일 전수 대조로 **동등 변이임을 증명**했다)
#228  출시 판단 문서의 `Last Updated` 가 **19일 뒤처져** 있었다. 그 사이 절이 17개 붙었다.
      머리말 날짜 == 최신 절 날짜를 강제하는 11-e 를 신설. 변이 2/2
```

`#227` 은 지금 죽어 있는 경로다 — 카탈로그에 기간 항목이 하나도 없다. 그래도 고친 이유는
그 표의 주석이 **"이벤트를 붙일 때 값만 바꾸면 된다"** 고 적어 두었기 때문이다.
프로그래머가 아닌 사람이 고치는 표라면, 값이 틀렸을 때 **조용히 금액이 달라져서는 안 된다.**
그래서 판정을 날짜 객체로 바꾸고 표 자체를 **부팅에서 검증**하게 했다(정의만 두지 않고
최상단에서 실제로 부르는지 AST 로 고정).

### 결함을 찾지 못한 곳 — 근거를 남긴다

```
환불 동시성          변이 5/5 검출 (부분 환불 겹침으로 원금 초과 환불 불가)
경로 파라미터 576회   적대적 값 12종 x 전 라우트 x 헤더 3종 -> 5xx·예외 **0건**
라우트 커버리지       41개 전부 어떤 테스트에든 등장 (미등장 0건)
상세 API N+1         자산을 0->45개로 늘려도 SELECT **고정 7회**
죽은 코드            제품 최상위 함수 중 아무도 안 부르는 것 **0개**
무료 횟수 월 경계     `used_at` 문자열 비교는 쓰는 곳이 하나이고 형식이 같아 안전.
                     다만 **1일에 공백 구분으로 쓰면 세어지지 않는다**는 것을 실측해
                     그 형식 계약을 검사로 고정했다(#227 과 같은 부류의 잠복 위험)
```

전체: **python 52 통과 / 4 실패**(단언 9,469) — 실패 4건은 앞 절과 동일하게 전부
이 머신 DB 가 뒤처져서이고 승인 영역이다.

### 이어서 — 파일 서빙 경로 (BUGS #229)

Audit 루프를 한 바퀴 더 돌며 **아직 안 본 곳**(문서/사진 서빙, 요청 본문, Admin 감사 로그)
을 갔다. 결함 1건.

```
#229  경로 담기 가드가 **막아야 할 입력에서 스스로 죽는다**
      다른 드라이브 경로 -> os.path.join 이 베이스를 갈아치우고
      -> os.path.commonpath 가 ValueError -> 404 대신 **500**
      저장소가 이미 3곳(admin.py / image_assets.py / repair_document_status.py)에서
      고쳐 놓고, **사용자에게 파일을 내보내는 라우트 3곳**을 빠뜨린 상태였다. 변이 3/3
```

★ `api/v1/registry.py` 는 주석에 *"documents.py 와 동일한 방식"* 이라고 적고
**고쳐지지 않은 판을 베껴** 왔다. 복제된 판정은 결함까지 복제된다(#224 와 같은 모양).
그래서 라우트별 검사에 더해 **제품 코드의 모든 `commonpath` 사용처가 ValueError 를
다루는지** 훑는 구조 검사(9-b)를 함께 넣었다 — 결함이 "여섯 중 셋"이었으므로
개별 라우트만 세면 네 번째가 또 샌다.

★ **그 구조 검사가 먼저 거짓 양성을 냈다.** `except (OSError, ValueError)` 튜플 형태를
못 알아보고 멀쩡한 `crawler/image_assets.py` 를 결함으로 잡았다. 가드가 멀쩡한 코드를
결함이라 부르면 사람이 가드를 끈다 — 판정을 고치고 자체 검증에 튜플 형태와
`except OSError`(ValueError 를 안 잡는 경우)를 양쪽으로 넣었다.

### 결함을 찾지 못한 곳 (이번 회차)

```
요청 본문 퍼징        600회(본문 20종 x 변경 라우트 x 헤더 3종) -> 5xx·예외 **0건**
Admin 감사 로그       변경 라우트 5개 전부 record_audit 호출 (AST 확인)
DOC_WORKER_START_TIME  제품 코드가 안 쓴다 -> Sprint 237 이 이미 문서화한 사실. 결함 아님
filter/ 죽은 코드      api_server import 후 적재 모듈 **0개** -> test_filter.py 의 전제가 참
```

전체: **python 52 통과 / 4 실패**(단언 9,513) — 실패 4건은 앞 절들과 동일한 승인 영역이다.

### 이어서 — 수집 파이프라인 설정 전수 추적 (BUGS #230)

`DOC_WORKER_START_TIME` 처럼 **"설정에는 있는데 실행 경로가 안 쓰는 값"** 을 표적으로
`config/settings.py` 의 상수 10개를 *정의 → 호출부 → 실행 경로 → 런타임 → 테스트 → 문서*
로 하나씩 따라갔다.

```
제품이 import 하지 않는 6개 중
  살아 있는 계약 4개   DOC_WORKER_START_TIME / PRIORITY_REFRESH_TIME  -> .ps1 등록 시각과 묶임
                       DOC_TYPE_LIST -> enqueue_documents() 루프와 소스 대조
                       DOC_COLLECT_SECONDS_PER_ROW -> 재수집 상한 부등식의 입력
                       (값을 바꾸면 test_schema_hygiene 가 붉어진다 — 변이 3/3)
  이미 기록된 dead      COURTS (Sprint 19). 실동작은 config/courts.py ALL_COURTS(60개)
  ★ 새 결함            SIDO_LIST -> BUGS #230
```

**#230 은 상수가 죽은 것이 아니라 계약이 죽어 있었다.** 시도 어휘가 세 곳
(config / 화면 / normalizer)에 복사돼 있는데 어느 둘도 묶여 있지 않았다.
화면에만 시도를 더하면 `extract_sido()` 가 못 알아보고, `search.py` 가 원본을 그대로
`WHERE sido = ?` 에 넣어 **오류도 안내도 없이 0건**이 된다(재현함).

★ 이번에도 **내 가드가 먼저 거짓 양성을 냈다** — 화면 목록과 config 목록의 *순서까지*
같으라고 썼다가 멀쩡한 코드를 붉게 만들었다. 화면 순서는 드롭다운 표시 순서(UX 결정)이고
config 쪽은 membership 어휘라 순서에 의미가 없다. 같아야 하는 것은 **집합**이다.
(#229 의 `except (OSError, ValueError)` 오탐에 이어 두 번째 — 가드를 만들 때마다
"멀쩡한 코드를 붉게 만들지 않는가"를 함께 봐야 한다.)

★ 그리고 **normalizer 축이 실제로 발동하는지 따로 확인했다.** 변이 M1~M3 은 전부
집합 검사가 먼저 잡아 그 축이 한 번도 안 울렸다. 집합을 통과시키는 변이(M4: 화면과
config 양쪽에 추가)를 일부러 만들어 그 축만 붉어지는 것을 봤다 — **울리는 것을 보지 않고
"축을 넣었다"고 하면 그 축은 있으나 마나다.**

### 결함을 찾지 못한 곳 (이번 회차)

```
크롤 파이프라인 예외    bare except **0곳**. `except Exception: pass` 20곳은 전부
                       driver.quit()/close()/팝업닫기/DOM 탐지 — 삼키는 것이 옳은 자리
enqueue_documents()    입력 행당 22us 로 **선형**(큐 1천->3만5천 행에서 평평).
                       하루 262물건 기준 약 6ms. 병목 아님
MAX_RETRY              crawler/court_crawler.py 의 재시도 루프가 실제로 쓴다.
                       test_court_crawl_recovery 가 핀. 죽지 않았다
#222 게이트 양방향      미선언 -> 통과 / operational -> 대조하고 실제로 실패.
                       이빨이 판정 가능한 머신 쪽에 남아 있음을 실측
```

전체: **python 52 통과 / 4 실패**(단언 9,521) — 실패 4건은 앞 절들과 동일한 승인 영역이다.

### 이어서 — 설정의 동적 접근과 환경변수 축 (BUGS #231~#232)

앞 절의 설정 추적을 **grep 으로 끝내지 않기 위해** 동적 접근(`getattr`/`vars`/`globals`/
`__dict__`)과 환경변수까지 따라갔다. 결함 2건.

```
#231  `조회 창 >= 공급 상한` 검사가 **오늘 기준 공허**했다
      getattr(cfg, "CASE_LOOKUP_MAX_ROWS", cfg.MAX_ITEMS) 의 폴백이 늘 걸려
      단언이 `X >= X` -> MAX_ITEMS 를 1/10/999/0/-5 무엇으로 바꿔도 통과(실측)
      -> 판정을 함수로 빼고 합성 값으로 **오늘 이빨**을 줬다. 변이 4/4
         (M3 = 실제로 CASE_LOOKUP_MAX_ROWS=5 를 추가 -> 진짜 분기가 붉어진다)
#232  환경변수 문서 상태표가 15일 stale + **"이름은 있는데 값이 비어 있다"** 를 안 다뤘다
      -> stale 정정, 빈 값 상태를 #205/#206 과 함께 표에 추가,
         표 머리에 **"머신별 스냅샷"** 임을 못박음(#222 와 같은 부류)
      -> 코드가 읽는 변수가 전부 문서에 있는지 검사 신설. 변이 4/4
```

**★ 이번 회차에 내 검출기가 세 번 틀렸고, 세 번 다 실측으로 잡았다.**

1. **간접 참조를 놓쳤다** — `_env_flag("X", ...)` 로 이름을 인자에 넘겨 읽는 두 변수를
   "문서에만 있는 이름"으로 오판했다. `os.getenv` 를 감싸 **런타임 추적**으로 대조해 알았다.
2. **부분 문자열에 속았다** — 문서 대조를 `name in doc` 로 썼다가, 이름을 더 긴 이름으로
   바꾸는 변이를 놓쳤다. 단어 경계로 고치고 그 함정을 자체 검증에 넣었다.
3. **출력 리터럴에 em-dash 를 썼다** — `test_console_encoding` 이 잡았다.
   cp949 콘솔에서 터질 자리였고, #221 과 같은 인코딩 계열이다.
   **내가 만든 회귀를 저장소의 가드가 먼저 잡았다.**

### 결함을 찾지 못한 곳 (이번 회차)

```
동적 접근            제품 코드에 getattr/vars/globals 로 설정을 읽는 곳 **0곳**
환경변수 문서 대조    제품이 읽는 13개 전부 문서에 있음(누락 0). 런타임 추적 12개와 교차 확인
문서에만 있는 이름    KG_*/SMTP_*/SMS_*/SENTRY_DSN/SLACK/GA4 는 **미도입 연동의 계획 항목**.
                     문서가 §B/§C 로 이미 그렇게 분류하고 있다. 드리프트 아님
SUPABASE_ANON_KEY    .env 에 있으나 백엔드가 안 읽는다 -> 문서 §발견 2 가 **이미 기록**해 둠
```

전체: **python 52 통과 / 4 실패**(단언 9,521 -> 9,535).

### 이어서 — dead code 의 **영향 범위** 추적 (BUGS #233)

설정 Audit 의 마지막 조각. `settings.COURTS` 가 "이미 dead 로 기록됨"이라는 사실을
확인만 하고 넘기지 않고, **죽은 것이 위험한가**를 따졌다.

```
settings.COURTS  5개  CourtInfo(code='B000210',       name='서울중앙지방법원')
ALL_COURTS      60개  CourtInfo(code='서울중앙지방법원', name='서울중앙지방법원')
둘 다 List[config.settings.CourtInfo]  ->  run_courts() 에 **타입 검사 없이** 들어간다
```

변이로 실증: `mvp_scraper.py` 의 import 한 줄을 바꾸고 크롤 관련 검사 9개를 돌렸더니
**새로 붉어진 검사 0개**. 그 상태에서 크롤 대상이 60개에서 5개로 줄고 `court_code` 가
`B000210` 이 되어 문서·사진 서빙 경로가 전부 어긋난다.

★ 한 건이 잡힌 것처럼 보였으나 **변이 없이도 같은 실패가 나는지 대조**해서 기존 #224
데이터 실패임을 확인했다. *변이 결과도 그대로 믿지 않는다.*

★ 그리고 모듈 변수만 보는 첫 가드는 `run_courts(ALL_COURTS[:5], ...)` 를 놓쳤다 —
**실제로 넘기는 인자**를 AST 로 보게 고쳐 슬라이스/필터까지 잡는다. 변이 3/3.

**SKIP**: `settings.COURTS` 삭제는 제품 코드 삭제라 승인 영역. 다만 삭제 전까지의
위험은 이번 가드가 막는다.

전체: **python 52 통과 / 4 실패**(단언 9,535 -> 9,545).

### 이어서 — COURTS 확정, 검출 로직 개선, 중복 로직 전수 탐색 (BUGS #234)

**① COURTS dead 확정** — 정의 1곳 + 문서 언급 8곳뿐, 제품·테스트 참조 0.
`CourtInfo` 타입 사용처 / `ALL_COURTS` import 경로 / court code 가 crawler -> storage -> API 로
흐르는 경로를 전부 따라갔다. 판정과 근거는 `docs/BUGS.md` 의 **"설정 전수 판정표"** 에 있다.

★ 파급을 한 단계 더 팠더니 #233 이 생각보다 나빴다 — `code == name` 은 장식이 아니라
**크롤과 문서 워커를 잇는 조인 키**다. `base_crawler.go_to_case_detail()` 이 큐 행마다
`next((c for c in ALL_COURTS if c.code == court_code), None)` 로 조회하므로,
`B000210` 체계로 크롤하면 **모든 큐 행에서 매칭 실패 -> 문서 수집이 통째로 멈춘다.**
(실 DB 대조: ALL_COURTS 에 없는 court_code **0건**)

**② 검출 로직을 고쳤다** — #224 기존 실패가 변이 결과에 섞이는 일을 두 번 겪었다.
문자열로 거르면 다음에 또 틀리므로 **기준선 차집합**만 보는 하네스로 바꿨다.

```
baseline = 변이 없이 돌린 실패 집합
판정      = mutated - baseline
```

#233 을 이 하네스로 재검증했다 — 기준선 5건(지역 데이터 3 + 스키마 드리프트 2)을
명시적으로 제외하고 **3/3 검출**, 복원 후 기준선과 동일함까지 확인.

**③ 나머지 zero-usage 설정 판정** — dead 1(`COURTS`) / 테스트·계약용 5 / runtime 4.
"사용처 0건"이 곧 dead 가 아니었다: 5개는 `.ps1` 등록 시각·enqueue 루프·재수집 부등식과
묶인 **살아 있는 계약**이었고, 값을 바꾸면 실제로 붉어진다(변이 확인).

**④ 중복 로직 전수 탐색 -> #234** — AST 해시로 본문 동일 쌍을 찾아
`normalize_price` == `parse_price` 를 발견. 파이프라인의 **다른 단계**에서 같은 값을
해석하고 있어, 한쪽만 고치면 "검증은 통과했는데 저장 금액이 다른" 상태가 된다.
재현으로 확인: 고치기 전에는 한쪽만 바꿔도 **검증 쪽 검사가 초록**이었다.

**⑤ 프런트 Audit** — 미사용 모듈 0, fetch 소비자 8개 전부 실패를 구분해 말한다.
★ 내 정규식이 제네릭 호출을 못 잡아 과소 보고했고, 이어 오탐도 하나 냈다(둘 다 확인 후 정정).

전체: **python 52 통과 / 4 실패**(단언 9,545).

### 이어서 — "바꿔 끼울 수 있는 쌍" 전수 탐색 (신규 결함 0건)

#233 후속. 모듈 수준 컨테이너 상수 **59개**를 모양별로 묶어, 용도가 인접해 **조용히
틀릴 수 있는** 쌍을 실제로 맞바꿔 봤다.

```
ALL_COURTS / settings.COURTS                    -> ★ 가드 없었다 (#233 로 봉인 완료)
_BUILDING_HEADS / _LAND_HEADS                   -> test_normalizer 가 잡는다
_ALLOWED_SYMMETRIC / _ALLOWED_ASYMMETRIC_ALGS   -> 인증 검사 2종이 잡는다
PAYMENT_TRANSITIONS / SUBSCRIPTION_TRANSITIONS  -> test_race_conditions 가 잡는다
```

**#233 이 유일한 구멍이었고 지금은 닫혀 있다.** 신규 결함 0건.

★ 이 회차에 **변이를 두 번 잘못 만들었고 둘 다 결과를 의심해서 잡았다.**
하나는 치환이 아예 적용되지 않았고(하네스가 `[NOT-APPLIED]` 로 알려 줬다),
하나는 이름만 맞바꾼 **알파 치환**이라 의미가 같은 프로그램이었다(값을 바꾸니 즉시 검출).
*"검출 0"을 곧바로 "가드 없음"으로 읽지 않는다 — 변이가 정말 의미를 바꿨는지부터 본다.*

전체: **python 52 통과 / 4 실패**(단언 9,545) · node 196건 192 pass.

### 이어서 — 공허한 PASS 전수 탐색 (BUGS #235)

#231(공허한 단언)을 일반화해 테스트 전체에서 **구조적으로 항상 참인 단언**을 AST 로 훑었다.
후보 **22건 중 21건이 오탐**이었고 1건이 진짜였다.

```
오탐 21건  ids_of(path)==ids_of(path)  -> 매 호출이 실 HTTP 요청. 정렬 결정성 검사
           mod._lock() is not mod._lock() -> 매번 새 객체여야 한다는 의미 있는 단언
           check_true(..., True) 계열   -> `except` 블록의 "예상한 예외가 떴다" 마커
진짜  1건  6-B 미추적 import 가드 -> 미추적 소스가 0개라 **패턴 매칭이 한 줄도 안 돈다**
```

★ **오탐 21건을 하나씩 열어 확인했다.** 자동 판정만 믿고 보고했으면 멀쩡한 관용구
21개를 결함으로 올릴 뻔했다.

★ **#235 검증을 처음에 잘못했다** — 실제 탐지 경로를 태우려고 미추적 모듈을 만들고
추적 파일이 import 하게 했는데 `[PASS]` 가 나왔다. 원인은 제품이 아니라 **변이 미적용**
(CRLF 파일에 `
` 으로 치환)이었다. 삽입 여부를 먼저 단언하니 즉시 검출됐다
(`api/rate_limit.py:44 -> api/qa_ghost_module.py`).
**이 세션에서 내 검출기·변이가 틀린 것이 여섯 번째다** — 매번 결과를 의심해서 잡았다.

전체: **python 52 통과 / 4 실패**(단언 9,545 -> 9,553).

### 이어서 — 경계 변이 캠페인 (BUGS #236)

*"mutation 을 넣어도 실패하지 않는 테스트"* 를 추측이 아니라 **재서** 찾았다.
경계 비교 `>=`/`<=` **34개**를 세고 돈·보안 모듈부터 한 지점씩 뒤집었다.

```
검출 5 / 생존 3
  생존 -> 창 포함 경계(now-60 과 정확히 같은 시각)      -> 양방향 검사 추가
  생존 -> 잔여 0 환불의 오류 코드(문서화된 공개 계약)   -> 검사 추가
  생존 -> 축출 임계 10,000 vs 10,001                    -> **무해 판정, 고치지 않음**
```

★ **먼저 내 하네스가 틀렸다.** `limit <= 0` 변이가 생존으로 나왔는데 테스트에는 그 케이스가
있었다. 앞뒤가 안 맞아 직접 태워 보니 변이본이 **IndexError 로 크래시**했고,
크래시는 `[FAIL]` 줄을 남기지 않아 `[FAIL]` 만 세던 하네스가 "생존"으로 읽고 있었다.
**종료코드도 신호로** 세도록 고치니 생존이 4→3 으로 줄었다.
*검출기가 놓치는 신호가 무엇인지부터 확인한다* — 이 세션에서 일곱 번째다.

★ **고치지 않은 것에도 근거를 남겼다.** 축출 임계 1개 차이는 상한의 목적(무한 증가 방지)을
해치지 않는다. 동등 변이는 아니지만 무해하다 — 근거 없이 검사를 늘리지 않는다.

전체: **python 52 통과 / 4 실패**(단언 9,553 -> 9,558).

### 이어서 — 경계 변이 캠페인 완주 (BUGS #237)

남은 26개 지점을 마저 돌려 **34개 전수**를 마쳤다.

```
검출 9  ·  수정 4  ·  무해 판정 6(전부 근거 기록)
```

**#237** — `calc_priority` 의 "3일 뒤 / 7일 뒤" 검사가 실제로는 `days_left=2 / 6` 을 보고
있어 **경계에 한 번도 닿지 않았다.** `strptime`(자정)과 `now`(시각 포함)의 차이를
`.days` 가 내림하기 때문에 **days_left = 달력일수 − 1** 이다. 진짜 경계는 달력 4일 / 8일 뒤.
제품 동작은 그대로 두고(정책 변경이라) **경계와 오프셋 자체를 고정**했다.

★ **이 캠페인에서 내 검출이 두 번 틀렸다.**
크래시를 "생존"으로 셌고(`[FAIL]` 줄이 안 남는다), 0바이트 가드의 **소유 테스트를
빠뜨렸다**(`test_asset_pipeline` 미포함). 제대로 넣으니 즉시 검출된다.
**survivor 를 곧바로 결함으로 확정했다면 허위 결함 2건을 보고했을 것이다.**

전체: **python 52 통과 / 4 실패**(단언 9,558 -> 9,563).

### 이어서 — survivor 10개 전수 분류 (BUGS #238)

3개 lead(`MIN_IMAGE_BYTES` 하한 / docstring "초과" / `doc_exists` parity)를
**실제 파일로 경계값을 태워** 확정했다.

```
사진   0->F  1023->F  **1024->T**  1025->T      (>= 1024, 이상)
문서   0->F     1->T   1023->T                  (>  0,    초과)
```

**판정: DOCUMENTATION DRIFT. 코드는 옳다.** 쓰기(`>= 1024`)와 서빙(`< 1024`)이 **정확한
여집합**이고 상수도 공유한다. 하한이 문서와 다른 것도 의도다(1KB 미만 = 잘린 내려받기).

★ 그래도 고친 이유: 그 docstring 은 *"쓰는 쪽과 읽는 쪽이 갈라지면 화면은 READY인데
뷰어는 404"* 를 경고하는 자리다. 거기서 "초과"라고 적어 두면 다음 사람이 `>=` 를 `>` 로
정정하고 **정확히 1024바이트인 사진이 조용히 사라진다.** 문구가 곧 함정이었다.

**최종 분류: REAL BUG 0 · WEAK TEST 4(수정) · DOC DRIFT 1(수정) · FALSE POSITIVE 1 ·
INTENTIONAL/EQUIVALENCE 4.**

★ **그리고 세 번째 다리를 놓치고 있었다.** 처음엔 쓰기·서빙 둘만 못박았는데 변이
`save_auction_images` 의 `size < MIN` -> `<=` 가 살아남았다. 그 자리가 어긋나면 #148 의
**정반대**가 된다 — 파일도 있고 서빙도 200 인데 `auction_image` **행이 안 생겨**
받아 놓고도 영원히 안 보인다. 썸네일은 크기를 안 보고 행만 읽기 때문이다.

런타임 전 구간 실측으로 **여섯 지점이 같은 경계**임을 확인하고, 세 다리 + 썸네일 전제까지
**변이 5/5** 로 고정했다.

★ 그 과정에서 **BUGS #232 의 부분 문자열 함정을 두 번째로 밟았다** —
`"MIN_IMAGE_BYTES" in db_src` 가 `_MIN_IMAGE_BYTES = 2048  # 자기 숫자` 변이를 놓쳤다.
import 문을 정규식으로 찾고 하드코딩 부재까지 함께 보도록 고쳤다.

전체: **python 52 통과 / 4 실패**(단언 9,563 -> 9,573).

### 이어서 — 하한 경계를 **런타임 end-to-end** 로 재검증 (BUGS #238 보강)

직전 회차의 파리티 검사는 *"세 다리가 같은 상수·같은 방향을 쓰는가"* 를 **소스로** 봤다.
그것만으로는 **정말 그렇게 동작하는가**를 말할 수 없어(경로 해석·조기 반환이 끼면
소스가 옳아도 결과가 다르다) 임시 DB 에서 `save_auction_images()` 를 실제로 불렀다.

```
bytes   saved   auction_image행   썸네일seq   서빙
1023      0            0           None      404     <- #148 이 **구조적으로 성립 불가**
1024      1            1           있음       200
1025      1            1           있음       200
```

`1023` 에서 썸네일이 없다는 것이 핵심이다 — 행이 안 생기므로
*"목록엔 보이는데 열면 404"* 를 만들 수가 없다.

★ **이 검사를 쓰다가 내가 먼저 틀렸다.** `path` 에 상대경로를 넘겼더니 1024바이트도
`saved=0` 이 나와 하마터면 *"하한 가드가 과하다"* 고 오판할 뻔했다.
`save_auction_images()` 는 `getsize(path)` 를 준 그대로 쓰고, 실 호출부는 **절대경로**를
넘긴다. **검사가 실 호출부와 같은 형태로 부르는지부터 확인해야 한다** — 이 세션에서
내 검증이 틀린 것이 여덟 번째다.

런타임 검사 변이 **4/4** 검출(DB기록 `<`->`<=`, 가드 무력화, 서빙 `<`->`<=`, 쓰기 `>=`->`>`).

전체: **python 52 통과 / 4 실패**(단언 9,577 -> 9,614).

### 이어서 — 라이브 API 경계 프로브에서 **죽은 검색 경로**를 찾았다 (BUGS #239)

문서를 읽고 "문제 없음"으로 끝내지 않으려고, 서버 둘(FastAPI 8000 / Next 3000)을 실제로
띄우고 검색 API에 경계값을 **직접 태웠다.** 거기서 나온 것이 이번 결함이다.

```
건물면적 min 13종 x 토지면적 min 12종 = UI 드롭다운 156 조합
  -> 결과가 1건이라도 나오는 조합 : 0 / 156      (종결물건 포함에서도 0)
  한쪽만 주면                      : 152건 / 125건
```

**두 면적을 함께 주면 언제나 0건이었다.** 오류도 안내도 없다.

원인은 `backfill_area.py` 의 **"커버리지 99.3%"** 를 잘못 읽은 것이다. 그 99.3% 는
*"둘 중 하나라도 가진"* 비율인데, 검색 백엔드와 검색 폼이 *"각 컬럼의 보유율"* 로 읽고
**"면적 미상 = 차량/선박 16행"** 이라고 적어 뒀다. 전수 재측 결과:

```
건물면적만 1,535(60.0%)  토지면적만 1,006(39.3%)  ★ 둘 다 보유 0  둘 다 없음 17

건물면적 조건이 버리는 행 1,023(40.0%)   토지면적 조건이 버리는 행 1,552(60.7%)
가시 재고 280건 기준       128건(45.7%)                      153건(54.6%)
   빠지는 것: 전답 44 · 임야 19 …          빠지는 것: 아파트 45 · 다세대 23 · 오피스텔 20 …
```

두 컬럼은 독립 속성이 아니라 **판별 합집합**이다(한 물건은 하나만 갖는다). AND 로 묶으면
만족 가능한 행이 **구조적으로 0** — 데이터가 늘어도 영원히 0이다.

**수정:** `면적 조건 = 계열들의 OR, 계열 안 min/max 는 AND`.
한 계열만 주면 항이 하나라 **기존 동작과 완전히 같다**(152/152 · 127/127 · 1,523/980
실측 동일). 두 계열은 이제 정확한 합집합이다(1,523 + 980 = 2,503 확인).

★ **기존 테스트가 통과하고 있었던 이유를 확인했다.** `test_search.py` 의 면적 fixture 는
4행이 전부 `아파트` 이고 1~3번이 **건물·토지를 둘 다** 갖고 있다 — 실데이터에 **0행**인
모양이다. 틀린 전제가 코드·주석·fixture 세 곳에 함께 박혀 있어서, 무엇을 단언해도
이 결함이 드러나지 않았다. **통과가 곧 "확인했다"가 아니다.**

★ **보안 가드를 우회하지 않았다.** 첫 구현이 컬럼명을 f-string 으로 조립했다가
`test_schema_hygiene` 의 "WHERE 조각이 전부 상수"(SPRINT107)에 걸렸다. 가드를 느슨하게
하는 대신 조각을 **전부 소스 리터럴**로 바꿨고, 새로 만든 예외가 **이름만 믿는 구멍**이
되지 않도록 `_names_bound_only_to_str_literals()` 로 *"그 변수의 재료가 정말 리터럴인가"*
를 AST 로 따로 못박았다(변이 3/3).

★ **내 검사가 두 번 틀렸고 둘 다 변이가 잡았다.** (1) 소스 계약에서 그냥 `" OR ".join`
을 찾았더니 **바로 위 `property_type` 이 같은 표현을 써서** 면적을 AND 로 되돌려도
초록이었다. (2) 고친 검사를 **줄 단위**로 썼더니 구현을 두 줄로 감싸자 붉어졌다.
식 단위로 보도록 다시 고쳤다. 변이 앵커가 **CRLF 로 두 번 미적용**된 것도
하네스의 `[NOT-APPLIED]` 가 알려 줘서 "생존"으로 오독하지 않았다.

변이 **12/12 검출 · 생존 0**(제품 8 · 소스계약 1 · SQL 리터럴 3).

전체: **python 56 통과 / 0 실패**(단언 9,640 -> 9,682) ·
**node 198건 197 pass / 0 fail / 1 skip**(서버 2개 기동 상태).
tsc 0 · eslint 0 · `npm run build` 성공.

### 이어서 — "보내는데 안 읽는" 파라미터를 **구조적으로** 막았다 (BUGS #239 후속)

#239 의 뿌리는 결합 규칙이었지만, 그 **한 단계 위의 실패 형태**가 이 저장소에 이미
두 번 있었다.

```
BUGS #123   면적 4종을 프런트가 보내는데 백엔드가 읽지 않았다
            -> "사용자가 면적을 좁혀도 결과가 그대로였다. 오류도 안내도 없다."
BUGS #239   면적이 구현된 뒤에도 두 조건을 함께 주면 항상 0건
```

FastAPI 는 **모르는 쿼리 파라미터를 조용히 무시한다.** 오타 하나, 아직 없는 필터 하나면
200 이 돌아오고 사용자에게는 "그런 물건이 없다"로 보인다. 그래서 계약을 검사로 못박았다.

실측(현재 상태): 프런트가 보내는 24개 중 백엔드가 안 받는 것은 **`special_conditions` 하나**
뿐이고, 그것은 UI 가 "준비 중입니다"라 **사용자가 값을 넣을 수 없다**(도달 불가).
즉 지금 살아 있는 결함은 없다 — 막은 것은 **다음 번**이다.

목록은 손으로 적지 않는다. 양쪽 다 유도한다.

```
보내는 쪽  SearchForm.tsx 의 `query.<name> =` / `query['<name>'] =` 전수
받는 쪽    /openapi.json 의 /api/v1/search 파라미터 전수
```

검사 4개: 공허하지 않음(양쪽을 실제로 뽑았는가) · 무시되는 파라미터 0 ·
**예외 목록에 죽은 항목 0**(특수조건 UI 를 여는 날 자동으로 붉어진다) ·
"무시된다"를 소스가 아니라 **응답 total 비교**로 실측.

★ **여기서 내 검사의 사각지대를 변이가 찾아냈다.** 처음엔 점 표기(`query.x =`)만 봤는데,
대괄호 표기(`query['x'] =`)로 바꾸면 검사가 **조용히 눈이 먼다.** 두 표기를 다 보도록
고치고 재확인했다.

```
P1 백엔드에 없는 필터 추가          검출        P5 대괄호 표기로 없는 필터 추가   검출
P2 파라미터명 오타                  검출        P6 대괄호 표기로 오타            검출
P3 프런트가 안 보내게 됨(죽은 예외)  검출        P4b 점->대괄호 전면 전환         통과(의미 동등, 기대대로)
```

★ P4 를 처음 만들었을 때 **생존**으로 나왔는데, 원인은 가드가 아니라 **변이가 반쪽만
적용된 것**이었다(`query.case_no =` 가 두 줄인데 한 줄만 바꿨다). 그대로 믿었으면
"공허 검사가 무력하다"는 **허위 결함**을 보고할 뻔했다. 전량 치환으로 다시 재서 확정했다.

전체: **python 56 통과 / 0 실패**(단언 9,682) ·
**node 202건 201 pass / 0 fail / 1 skip** · tsc 0 · eslint 0.

### 이어서 — 규칙이 두 벌이 되는 것을 그 자리에서 막았다 + 죽은 설정 제거

**(1) `KNOWN_UNSUPPORTED` 가 두 벌이 됐다 — 정본 하나로 합쳤다**

위에서 node 쪽에 파라미터 계약 검사를 하나 더 붙이면서 같은 목록이 두 파일에 생겼다.
이 저장소가 반복해 사고를 낸 모양 그대로다(BUGS #107/#112/#136/#161/#204 — *"같은 어휘가
두 곳에 있으면 갈라진다"*). 한쪽에서만 항목을 빼면 다른 쪽이 계속 눈감아 준다.

목록만 `tests/_search_param_contract.mjs` 로 빼고 **셋이 함께 읽는다.**

```
tests/source-contract.test.mjs     소스의 `Query(` 선언과 대조   (서버 불필요)
tests/frontend-contract.test.mjs   살아 있는 /openapi.json·응답과 대조
test_search.py                     필터가 실제로 거르는지와 대조
```

★ **옮기는 순간 `test_search.py` 가 붉어졌다** — 나는 소비자가 둘인 줄 알았는데
**셋이었다.** 파이썬 쪽이 이미 그 목록을 파싱해 읽고 있었다(2026-08-21 에 같은 원칙으로
심어 둔 검사다). 내가 몰랐던 세 번째 소비자를 **단일 정본 규칙이 스스로 알려 준 것**이다.
정본 이동은 그래서 안전했다 — 놓쳤으면 파이썬 검사가 조용히 빈 집합을 쓰게 됐을 것이다.

변이 3/3 을 **세 소비자 전부**가 검출한다(목록 비우기 / 죽은 예외 넣기 / export 제거).

**(2) `AUCTION_DB_PATH` — 읽는 곳이 없는 죽은 설정**

테스트 3개가 스크래치 DB 를 만들 때 `os.environ["AUCTION_DB_PATH"] = path` 를 세우고
있었는데, **그 이름을 읽는 코드가 저장소에 하나도 없다**(전수 grep). 실제 경로 전환은
바로 다음 줄의 `db.DB_PATH = path` 하나가 전부다.

무해하지만 **함정이다** — "환경변수가 DB 를 돌린다"고 믿고 `DB_PATH` 대입을 지우면
그 테스트는 **운영 DB 에 쓴다.** 지워도 세 파일 전부 그대로 통과하는 것을 확인하고
제거한 뒤, 그 자리에 무엇이 진짜 수단인지 남겼다.

(그 사고 자체는 이미 `run_python_tests.py` 가 파일마다 운영 DB **md5 지문**을 재서
잡는다 — 이번 전체 실행에서도 변경 보고 0건이었다. 죽은 설정만 걷어낸 것이다.)

**함께 실측한 것(결함 아님, 다시 파지 않기 위해 기록)**

```
인증 표면 42 라우트 전수, 토큰 없이 호출   admin 16개 전부 403 · 사용자 데이터 전부 401
                                          공개 5개(search/item/stats/plans/document-stats)만 200
문서·사진 전달 경계                       경로탈출 404 · 널바이트 400 · 대소문자 정상 · 남의 사진 404
페이지네이션 무결성                        정렬 7종 x 방향 2 = 14조합 전 페이지 훑기
                                          중복 0 · 누락 0 (280건 전수)
정렬 단조성                                14조합 전부 단조 True
DB 무결성                                  고아 0 · 중복 0 · FK 위반 0 · integrity_check ok
                                          bid_rate 불일치 0 · 가격 역전 1건은 병합사건 반올림(250원)
응답시간(12회 중앙값)                       전 경로 5~16ms, 1초 초과 0
보안 헤더                                  Next 4종 / API 2종. CSP·HSTS 는 문서화된 SKIP(승인·배포 영역)
```

★ `READY 인데 doc_raw 없음 17건` 을 처음에 결함으로 셌는데 **내 질의가 틀렸다** —
17건 전부 `doc_type='IMAGE'` 이고 사진은 `doc_raw` 가 아니라 `auction_image` 에 있다.
올바른 짝으로 다시 재니 양방향 모두 0이었다(IMAGE↔auction_image, 그 외↔doc_raw).
**보고 전에 다시 재서 허위 결함 1건을 걸렀다.**

전체: **python 56 통과 / 0 실패**(단언 9,682) · **node 202건 201 pass / 0 fail / 1 skip** ·
tsc 0 · eslint 0 · build 성공 · 운영 DB 지문 변경 0.

### 이어서 — 같은 면적을 두 번 계산하고 있었다, 둘 다 틀렸다 (BUGS #240)

#239 를 고치다 알아챈 것: **면적을 화면과 필터가 각각 계산한다.**

```
화면(검색 카드)  src/lib/format.ts:parseArea()   full_address 대괄호를 JS 로 파싱
필터/DB          normalizer.extract_areas()      같은 문자열을 파이썬으로 파싱
```

BUGS #204 가 경계하는 *"같은 어휘가 두 곳"* 그대로다. 추측하지 않고 **운영 DB 2,558행
전부에 두 함수를 각각 돌려 대조**했다.

```
일치 2,544  ·  값이 다름 1  ·  JS만 없음 13  ·  DB만 없음 0
```

그 14건에서 **양쪽 각각의 결함**이 나왔다.

**A. 프런트가 천단위 쉼표에서 앞자리를 잃는다** — `[0-9]+` 가 쉼표에서 끊겨 **쉼표
뒤부터** 매치됐다. `1,000㎡` 는 **0㎡** 가 된다. 실데이터 id=443(평택 공장)이
14,438.85㎡ 인데 카드에 **438.85㎡** 로 찍혔다(32배). 오류도 빈칸도 아닌 **그럴듯한
작은 숫자**라 알아챌 방법이 없다. 백엔드는 처음부터 옳았다.

**B. 백엔드가 구분호실을 1동 전체로 읽는다** — 옛 등기의 평/홉/작 표기
`... 1동 1층192평6홉9작 ... 내3층1평4홉6작` 에서 앞의 층 목록은 **건물 전체**이고
이 물건은 `내 ...` 뒤의 **4.8㎡ 사무실 한 칸**이다. 평 토큰을 전부 더해 **2,509㎡**
가 들어갔다(520배). 그 값이 **면적 검색을 탄다.**

★ **B 는 프런트가 이미 옳게 판단해 둔 것이었다.** `format.ts` 주석이
*"홉/작 하위 단위에 층 목록이 중복돼 있어 일괄 환산하면 틀린 숫자를 보여줄 위험이 있다.
아무것도 안 보여주는 편이 낫다"* 라고 적고 있었다. **결정은 이미 있었고 백엔드만
따르지 않았다** — 그래서 이 수정은 새 제품 판단이 아니라 기존 판단의 적용이다.
(대지권 표기에서 내린 판단과도 같은 형태다.)

전수 재계산으로 영향을 확정했다: **2,558행 중 바뀌는 행 2개, 2,556행 무변경.**
대조 결과도 `값이 다름 1 -> 0` 이 됐다.

함께 정정: `normalizer.py` 의 *"평 단위 실데이터에 1행"* -> 실제 **13행**
(#239 의 "16행"과 같은 종류의 낡은 숫자다).

회귀는 **양쪽에 같은 입력·같은 기대값**을 넣어 한쪽만 고치면 반대쪽이 붉어지게 했다
(`test_normalizer.py` / `tests/format.test.mjs`). 변이 **7/7 검출 · 생존 0**.

★ **DB 의 2행은 아직 옛 값이다.** `backfill_area.py --apply --force` 로 정정되지만
**운영 DB 쓰기라 이번 세션에서는 실행하지 않았다**(승인 영역). 그때까지 그 2행은
면적 검색에서 실제보다 크게 잡힌다.

전체: **python 56 통과 / 0 실패**(단언 9,682 -> 9,688) ·
**node 207건 206 pass / 0 fail / 1 skip** · tsc 0 · eslint 0 · build 성공.

### 이어서 — 붉은 감사 게이트를 오탐으로 넘기지 않았더니 진짜였다 (BUGS #241)

저장소가 가진 감사 스크립트 4개를 실제로 돌렸다. `audit_asset_integrity.py` 가
**exit 1 / 어긋남 43건**으로 붉었고, 그중 17건이 이런 모양이었다.

```
[9] API 가 광고한 자산 URL 이 실제로 열리는가
    /api/v1/item/1450/documents/IMAGE -> 400
```

처음엔 *"감사 도구가 IMAGE 를 문서로 잘못 다루는군"* 이라고 읽었다 — 바로 앞에서
내가 `doc_raw` 짝을 잘못 맞춰 허위 결함을 만든 직후라 더 그렇게 보였다.
**그래서 API 응답을 직접 열어 봤다.**
...
**그 URL 을 광고하는 쪽은 감사 도구가 아니라 API 였다.** 도구가 옳았다.

원인은 규칙의 부재가 아니라 **조건의 범위**였다. `_document_entry()` 는 이미
`servable`(= `DOC_TYPE_FILES` 에 있는 종류인가)을 계산해 두고 그것을 `file_size` 를
잴지에만 썼다. URL 은 `status==READY` 하나만 봤고, `document_status` 에는
`doc_type='IMAGE'` 행이 있어 사진을 받으면 READY 가 된다. 그래서 바로 그 자리 주석이

> *"열 수 없는 주소를 건네고 프런트가 404를 받아 보게 하는 것보다,
>   없다는 사실을 응답에 담는 편이 정직하다"*

라고 적어 둔 규칙이 **정확히 반대 결과**를 냈다. 사진은 0~N장이라 단일 서빙 파일이
없고 `images[]` 로 나간다.

프런트는 `.filter(doc_type !== 'IMAGE')` 로 걸러 내며 버티고 있었지만 그것은
**소비자 한 곳의 우회**일 뿐 API 계약은 거짓이었다 — 그 우회를 모르는 감사 도구가
그래서 붉었던 것이다.

**수정**: `openable = ready and servable` 하나로 `available`/`viewer_url`/`download_url`
셋을 함께 건다(셋은 같은 주장이다). 수집 상태는 `status` 가 그대로 전한다.
...
남은 26건은 코드가 아니라 **데이터 정리**다(고아 디렉터리 1 / 고아 큐 행 18 /
못 옮긴 내려받기 7). 스크립트 자신이 "정리는 승인 영역"이라고 적고 있어 손대지 않았다.

회귀는 종류 이름을 적지 않고 **광고된 URL 을 전부 실제로 호출해 200 인지** 본다
(`test_asset_pipeline.py`, 신규). 변이 **5/5 검출** — 되돌리기 3종뿐 아니라
**IMAGE 행을 통째로 없애는 과잉 수정(C5)도 잡는다.**

**함께 돌린 다른 감사 3개**: `audit_auth_health`(ES256 주 경로 정상, HS256 시크릿
부재는 승인 영역) · `audit_schedule_health`(모순 없음, 기본 검색 잔여 280건/최종 09-09) ·
`audit_test_reality`(실행 줄이 적은 검사 6개 — 전부 드리프트 가드이거나 자식 프로세스
측정 한계로, 결함 아님) — 전부 exit 0.

전체: **python 56 통과 / 0 실패**(단언 9,688 -> 9,727) ·
**node 207건 206 pass / 0 fail / 1 skip** · tsc 0 · eslint 0 · build 성공.

### 이 세션 종료 상태 · 다음 Sprint Backlog (2026-08-26)

**게이트 (전부 실제 실행)**

```
python   run_python_tests.py   통과 56 | 실패 0 | 건너뜀 3 | 판정없음 1   (단언 9,727 · 129s)
node     npm run test:frontend 207건 / 206 pass / 0 fail / 1 skip        (서버 2개 기동)
tsc      0 errors        eslint  0 problems        npm run build  성공
audit    auth_health 0 · schedule_health 0 · test_reality 0 · viewport 0(정상18/결함0)
         asset_integrity 1(26건, 전부 데이터 정리·승인 영역 — 코드 결함 0)
         contrast 1(444곳, 문서화된 P2 디자인 결정)
운영 DB  md5 96b1b366... / 6,873,088B — 이 세션 내내 **읽기 전용**(mode=ro),
         파일별 지문 감시에서 변경 보고 0건
```

**이 세션에 닫은 것**: BUGS #239 · #240 · #241 · #242 (전부 회귀 + 변이 고정)

**다음 Sprint Backlog — 승인 없이 할 수 있는 것부터**

```
1. [데이터] backfill_area.py --apply --force
   #240 의 잔여. auction_item 2행(id=6495 / 13584)이 아직 옛 값(3,771㎡ / 2,509㎡)이고
   그 값이 면적 검색을 탄다. 전수 재계산으로 **바뀌는 행이 그 2개뿐**임을 확인해 뒀다.
   -> 운영 DB 쓰기라 승인 필요. 백업 후 실행.

2. [제품 결정] 면적 조건에서 '면적 미상'을 계속 제외할 것인가 (#239 남은 결정)
   지금은 제외한다(기존 규약). 다만 그 대가가 16행이 아니라 **가시 재고의 46~55%** 임이
   이제 측정돼 있다. 화면에 "면적 미상 N건 제외됨"을 노출할지 PM 판단.

3. [디자인 결정] 명암비 444곳 (기록된 P2)
   blue-500 -> blue-600, gray-400 -> gray-500 한 단계면 대부분 해소된다는 실측이
   BUGS #9379 절에 있다. 브랜드 색 변경이라 디자인 결정.
   함께: `🤍`(즐겨찾기 안 됨)가 흰 카드 위 흰 하트다 — 아이콘 교체 여부(#242).

4. [데이터 정리] audit_asset_integrity 잔여 26건
   고아 디렉터리 1 / 고아 큐 행 18(전부 SKIPPED_EXPIRED·done, 수집 비용 0) /
   못 옮긴 내려받기 7(11.6MB). 스크립트 자신이 "정리는 승인 영역"이라 적어 둠.

5. [커버리지] 로그인 화면 24칸이 뷰포트 감사에서 측정 불가
   헤드리스에 세션이 없어 /login 으로 튕긴다. 도구가 **통과로 세지 않고** 정직하게
   분리해 두었다. 실계정 세션이 필요하므로 승인 영역.

6. [기술부채] `api/v1/item.py` 만 커밋된 blob 이 CRLF 다(나머지는 전부 LF).
   `core.autocrlf=true` 라 diff 가 **파일 전체 재작성처럼** 보인다
   (실제 변경 25 insert / 3 delete, `git diff --ignore-all-space` 로 확인).
   다음 커밋 때 자동 정규화되므로 그때 한 번 해소된다. 이 세션이 만든 것이 아니다.

7. [기술부채] `filter/` 3모듈은 어떤 진입점에서도 도달 불가(전수 import 그래프 확인).
   `report_generator` / `scoring_engine` 은 테스트 참조도 0. 삭제는 승인 영역.

8. [P0/P1 — 전부 외부 승인] KG이니시스 실연동 · Supabase Site URL ·
   Admin 화면 · Admin 개별 신원(custom claim) · SUPABASE_JWT_SECRET(.env)
```

## [2026-08-27] 크롤 -> DB -> 문서/검색 파이프라인 집중 감사

테스트가 56/0 이라는 것으로 운영 안정성을 판정하지 않기로 하고, **실제 크롤 저장 경로를
사본 DB 에서 규모별로 직접 돌려** 봤다. 거기서 매일 크롤을 통째로 죽이는 결함이 나왔다.

### ★ 최대 발견 — 유니크 사건 16,384건에서 파이프라인 전체가 죽는다 (BUGS #243)
...
`IN ((?,?),...)` 에 넣었는데, 쌍당 `?` 가 2개라 SQLite 상한(32,766)에 정확히 그 지점에서 닿는다.
느려지는 것이 아니라 **멈춘다.** rollback 은 깨끗해 부분 커밋은 없지만
`auction_case / auction_item / document_status` 가 **전부 0건**이 되고
`run_daily.bat` 에는 `[FAILED]` 만 남는다.

지금 운영 DB 는 유니크 사건 1,882건(상한의 11%)이라 아직 닿지 않는다. 60개 법원을
매일 전수로 도는 구조라 수집 범위가 넓어지면 닿는 수다.

**수정**: `chunked_for_sql()` / `sql_variable_limit()` 신설, 상한을 **커넥션에 직접 물어본다.**
같은 형태였던 `load_rights_data.py` / `load_spec_data.py` 의 정리 DELETE 도 함께 나눴다.

### 성능 — 수정 전/후

```
                수정 전                      수정 후
2,558행         0.10s                        0.10s      (동일 - 청크가 1개)
10,000행        0.36s                        0.42s
30,000행        ★ 크래시                     1.25s
60,000행        ★ 크래시                     2.50s
120,000행       ★ 크래시                     5.05s
처리량          16,383건에서 절벽             2,558~120,000 전 구간 ~24,000행/초 (평탄)
```

### 재발 방지 — 인벤토리 가드 (변이 3/3)

`IN (...)` 을 만드는 지점을 **AST 로 전수 추출**해 인벤토리와 대조한다(제품 코드 11곳).
새 지점이 생기면 *"입력이 작다는 근거"* 또는 *"chunked_for_sql 로 나눴다"* 중 하나를
적어야 통과한다. 죽은 인벤토리 항목도 잡고, "chunked 라고 적어 둔 곳이 **실제로**
그 함수를 쓰는지"까지 확인한다(이름만 믿지 않는다).

### ★ 두 번째 발견 — 체크포인트 손상이 완전히 침묵했다 (BUGS #244)

`_load_all()` 의 `except Exception: return {}` 는 **60개 법원을 처음부터 다시 긁는다**는
뜻인데 로그가 **한 줄도** 없었다(실측: 빈 문자열). 운영자는 밤새 돈 재크롤의 원인을
알 방법이 없다. 기존 테스트는 "크래시하지 않는가"만 봐서 그 침묵을 통과시켰다.
동작은 그대로 두고(재수집이 옳은 선택이다) **보이게만** 했다. 변이 5/5.

### 밤새 자동 실행 요건 — 전부 실측으로 확인

```
프로세스 재시작            RunLock(파일락) + 5시간 stale 인수    코드 확인
중간 실패 후 재실행        실패 주입 -> 부분 커밋 0 -> 재실행 정상 완료   ✔ 실측
동일 데이터 재수집 중복    같은 배치 3중복 -> 1행 / 재크롤 -> UPDATE      ✔ 실측
일부 실패가 전체를 안 죽임 물건별 except + 드라이버 재시작              코드 확인
                          (재시작 자체가 실패하면 이번 실행만 조기 종료)
실패 기록                  document_collect_failures 3행 + 로그          ✔ 실측
retry 상한                 MAX_DOC_RETRY=3, 소진 시 failed              코드 확인
무한 retry 금지            SKIPPED_EXPIRED/UNSUPPORTED 는 부활 안 함     코드 확인
queue <-> DB 정합성        큐done↔화면READY 불일치 **0건** 양방향        ✔ 실측
transaction rollback       실패 주입 시 전 테이블 무변경                ✔ 실측
false success 금지         commit 실패 -> 예외(성공 dict 아님)          ✔ 실측
                          제약 위반 -> failed 카운트가 실제와 일치
worker crash 복구          in_progress 2행(08-26 07:14 잔재) -> pending  ✔ 실측
                          갓 집어간 행(10분 미만)은 뺏지 않음            ✔ 실측
메모리                     같은 데이터 20회 반복 -> tracemalloc 증가 0   ✔ 실측
                          peak 은 선형(5k 9.4MB / 20k 36.7MB / 60k 112MB)
로그로 성공/실패 판단      체크포인트 침묵 해소(#244)                    ✔ 수정
이미 처리된 재처리 방지    pending 인데 이미 READY: **0건**              ✔ 실측
                          고아 대기 행: **0건**
```

### 그 밖의 실측 (결함 아님 — 다시 파지 않기 위해 기록)

```
중복 저장                  (court_code,case_no,item_no) 중복 0 / auction_image 중복 0
                          document_status 중복 0
UNIQUE/INDEX 충돌          integrity_check ok / foreign_key_check 0건
정규화 유실                auction -> auction_item full_address 불일치 0
크롤↔서빙 정합성           auction_item 건수 == /api/v1/search total
기형 입력                  빈문자/None/10k자/개행/따옴표/이모지/음수/공백 item_no
                          -> 8종 전부 예외 없이 처리, 파이프라인 관통 정상
큐 커버리지                큐 행 없는 auction_item 641건(25%)이지만
                          **그중 기일이 남은 물건은 0건** (화면에 보이는 물건은 전부 예약됨)
doc_type 대소문자          큐(소문자)/상태(대문자)는 QUEUE_TO_DOC_STATUS_TYPE 한 곳에서 변환.
                          맵 누락 0 / 미지의 값은 경고 후 건너뜀
```

### 측정으로 드러난 **스케일 절벽 두 개** (수정 아님 — 승인 영역)

```
1) journal_mode=delete 의 읽기 지연 (busy_timeout 5초)
   크롤 마이그레이션이 도는 동안 API 읽기가 얼마나 멈추는가 — 읽기 스레드 4개 실측

              delete(현재)        WAL
   2,558행     32ms               37ms
   20,000행    606ms              34ms
   60,000행    1,876ms            37ms
   150,000행   ★ 4,787ms          68ms      <- 5초 busy_timeout 코앞
   읽기 성공수 1,249               9,356     (150k 기준, 7.5배)

   `delete` 는 읽기 지연이 **쓰기 시간에 비례**하고 150k 부근에서 실패로 넘어간다.
   WAL 은 규모와 무관하게 평탄하다(쓰기는 5.41s -> 7.71s 로 조금 느려진다).
   -> journal_mode 변경은 운영 DB 파일 형식 변경(-wal/-shm, 백업 절차 변경)이라 **SKIP**.

2) 접두 중복 인덱스 6개
   auction_item 4 + auction_case 1 + document_status 1. 지우면
   60,000행 쓰기 2.24s -> 1.96s (**12.5% 단축**), 읽기 계획 회귀 **0**
   (주요 쿼리 6종 전부 더 넓은 복합 인덱스로 그대로 인덱스 탐색).
   -> 스키마 마이그레이션이라 **SKIP**. 근거와 수치만 남긴다.
```

### ★ 방법론 사고 — stale bytecode 가 변이 결과를 오염시켰다

변이 D3 는 `vars_per_item=2` -> `1` 로 **바이트 수가 같다.** 하네스가 파일을 되돌려도
`.pyc` 가 (mtime, size) 검증을 통과해 **되돌리기 전 바이트코드가 계속 실행됐다.**
수정을 넣고도 실규모 재현이 계속 실패했고, `inspect.getsource()` 는 파일을 읽으므로
**새 소스를 보여 줘서** 더 헷갈렸다. `dis.dis()` 로 `LOAD_CONST (1)` 을 보고서야 확정했다.

**이후 모든 변이는 `PYTHONDONTWRITEBYTECODE=1` 로 다시 돌렸다.** 그러지 않으면
같은 길이의 치환이 캐시에 남아 허위 검출/허위 생존을 만든다.

### 게이트 (전부 실제 실행)

```
python   통과 56 | 실패 0 | 건너뜀 3 | 판정없음 1   단언 9,740 -> 9,752
node     207건 / 206 pass / 0 fail / 1 skip        (서버 2개 기동)
tsc 0 · eslint 0 · npm run build 성공
변이     14/14 검출 · 생존 0  (청킹 6 · 체크포인트 5 · 인벤토리 가드 3)
운영 DB  md5 96b1b366... / 6,873,088B — 세션 전후 **바이트 동일**
         (모든 실측은 사본에서, 읽기는 mode=ro)
```

### 이어서 — 파이프라인 **마지막 단계가 배선돼 있지 않았다** (BUGS #245)

배치 3개(`run_daily` / `run_doc_worker` / `run_priority_refresh`)가 부르는 파이썬
스크립트를 전수로 뽑아 파이프라인 단계와 대조했다.

```
mvp_scraper.py       크롤 -> auction                    ✔ 배선됨
migrate_execute.py   -> auction_item / document_status  ✔ 배선됨
doc_worker.py        문서·사진 -> doc_raw / auction_image ✔ 배선됨
refresh_priority.py  큐 우선순위                        ✔ 배선됨
load_rights_data.py  문서 -> 권리분석                   ★ 어디에도 없음
load_spec_data.py    명세서 -> 임차인 권리              ★ 어디에도 없음
```

실측이 그 결과를 정확히 보여 준다.

```
2026-08-25  STATUS 수집 161건 / 파싱 161건 (100%)   <- 손으로 돌린 날
2026-08-26  STATUS 수집  16건 / 파싱   0건 (  0%)   <- doc_worker 만 돈 날
```

**실패가 아니라 부재라 어떤 실패 카운터에도 안 잡힌다** — 큐는 done, 파일도 있고,
`document_status` 는 READY 다. 다른 모든 지표가 초록인 채로 그 다음이 없다.

지금 사용자 영향은 0이다(미파싱 16건 전부 기일 경과). 그러나 **격차는 매일 자란다.**

**조치**: `audit_schedule_health.py` 에 축 [6] 신설 — 수집일별 파싱률을 찍고 0%인 날을
지목한다. 판정부를 순수 함수로 뽑아 `--selftest` 가 직접 태운다(변이 5/5).
**배선 자체는 SKIP** — 두 스크립트에 `purge_orphans()` DELETE 경로가 있어 야간 자동
실행으로 바꾸는 것은 운영 판단이 필요하다. 제안 diff 는 BUGS #245 에 적어 뒀다.

★ **여기서 내 검사가 두 번 틀렸고 둘 다 변이가 잡았다.** 새 검사를
`if fails: return 1` **뒤에** 넣어 5개 변이가 전부 생존했고(실패해도 아무도 안 본다),
고친 뒤에도 `"16건" in _blob` 이 일자별 줄의 *"수집 16건"* 에 걸려 요약 줄을 통째로
지워도 통과했다. 요약 줄 자체를 지목하도록 다시 고쳤다.

### 추가 실측 (결함 아님)

```
refresh_queue_priority()   1,657행 0.02s / 100,000행 0.50s (200k행/초) — 병목 아님
mvp_scraper 종료 계약      수집 0건 / 저장 0건 / 전 법원 실패 -> exit 1
                          부분 실패는 성공이되 WARNING 으로 반드시 남긴다
배치 3개                   단계마다 errorlevel 검사 + 각자의 [FAILED] 마커
                          인터프리터 미발견도 loud-fail
```

### 최종 게이트

```
python   통과 56 | 실패 0 | 건너뜀 3 | 판정없음 1   단언 9,756
node     207건 / 206 pass / 0 fail / 1 skip
tsc 0 · eslint 0 · build 성공
audit    auth_health 0 · schedule_health 0 · test_reality 0
         asset_integrity 1 (26건, 전부 데이터 정리·승인 영역)
변이     19/19 검출 · 생존 0
         (청킹 6 · 체크포인트 5 · 인벤토리 가드 3 · 파싱 축 5)
운영 DB  md5 96b1b366... / 6,873,088B — 세션 전후 **바이트 동일**
```

### 다음 Sprint Backlog (파이프라인)

```
1. [승인] load_rights_data.py / load_spec_data.py 를 run_doc_worker.bat 에 배선
   (BUGS #245. purge_orphans DELETE 경로 때문에 운영 판단 필요. 제안 diff 있음)
2. [승인] journal_mode=WAL 전환
   150k 규모에서 읽기 지연 4,787ms -> 68ms, 읽기 처리량 7.5배.
   운영 DB 파일 형식 변경(-wal/-shm)이라 백업 절차 함께 검토
3. [승인] 접두 중복 인덱스 6개 DROP 마이그레이션
   60k 쓰기 12.5% 단축, 읽기 계획 회귀 0. 스키마 변경이라 승인 영역
4. [코드] migrate_execute 의 물건별 `SELECT * FROM auction_item` (N+1)
   지금은 병목이 아니다(전 구간 24k행/초 평탄). 위 1~3 뒤에도 남으면 그때 본다
5. [운영] auction 테이블은 정리 정책이 없다 — 매일 전량을 다시 읽어 UPSERT 한다
   120k 에서 5.05s 라 아직 무해. 보존 기간 정책은 제품 판단
```

---

## 2026-08-28 Sprint 270 — 마이리스트 **가져오기** (경쟁사 마이페이지 정보 이관)

상세: `docs/SPRINT270_MYLIST_IMPORT.md`

### 무엇을 만들었나

Sprint 227 **내보내기**의 짝. 사용자가 다른 곳에서 관리하던 관심물건 목록을
**손으로 복사해** 붙여넣으면 파싱 -> 매칭 -> 확인 -> 담기까지 이어진다.

```
Frontend  /favorites/import (신규 화면) + /favorites 카드의 메모 편집
   ->     POST /api/v1/favorites/import/preview   읽기 전용. 저장 0
   ->     POST /api/v1/favorites/import/commit    확정된 item_id 만. 부분 성공
   ->     PUT  /api/v1/favorites/{id}/note        메모/태그 통째 교체
   ->     GET  /api/v1/favorites                  memo/tags/note_source **가산**
Backend   normalizer/mylist_import.py (순수 파서/매칭) + api/v1/favorite_import.py
DB        migration 026 favorite_notes (순수 가산)
```

**외부 상용 서비스에 로그인/크롤링하지 않는다.** 두 모듈에 HTTP 클라이언트가
한 줄도 없고 입력은 붙여넣은 문자열뿐이다.

### 왜 이번에는 만들 수 있었나 — 방향이 반대이기 때문

`SPRINT219B` 가 "상대 형식을 몰라 만들 수 없다"고 한 것은 **내보내기**다(우리가
상대 형식을 맞춰야 한다). **가져오기는 우리가 형식을 정할 필요가 없다** — 사람이
이미 클립보드에 담아 온 것을 읽기만 하면 된다. 그래서 형식을 가정하지 않고
우리 CSV/TSV 되붙이기 · 탭 구분 표 · 자유 텍스트 세 갈래를 모두 받는다.

### 실측이 만든 제약을 그대로 지켰다

```
병합 사건 22.7%    쪼개 대표를 고르지 않는다. 구성요소가 겹치면 같은 물건
                   (crawler/resume.py 와 **같은 규칙**. 접두 오검출은 그대로 걸러진다)
물건번호 33.5%     사건번호만으로 특정되지 않으면 AMBIGUOUS -> **사람이 고른다**
법원명 60종        표기가 달라 0건이 되면 **좁히기를 되돌린다**
                   (표기 차이로 "없습니다"라고 말하는 것이 조용한 오답이다)
못 찾은 줄         버리지 않는다. 원문(raw)과 사유를 그대로 돌려준다
```

### 승인 영역을 건드리지 않는 설계

migration 026 의 운영 적용은 승인 영역이다. 그래서 **테이블이 없어도 죽지 않는다** —
`GET /favorites` 는 있을 때만 JOIN 하고(무조건 JOIN 이면 부가 정보 하나 때문에
관심물건 목록 전체가 500 이 된다), 커밋은 `note_written:false` 로, 메모 편집은
`FAVORITE_NOTE_UNAVAILABLE` 로 **정직하게** 알린다. 화면도 "준비 중"을 표시한다.

### ★ 변이 테스트가 내 검사의 공백 2건을 잡았다

```
생존 1  "빈 메모로 기존 메모를 지운다"  -> 내 검사는 셋 다 비워 재커밋해서
        _upsert_note() 의 UPSERT 에 **닿지도 않았다**. "출처만 갱신" 경로를 추가
생존 2  "남의 관심물건에도 메모를 쓸 수 있다" -> 내 검사는 **아무도 담지 않은**
        물건으로 시험해 조건을 빼도 똑같이 거절됐다. 다른 사용자가 담은 물건으로 교체
```

둘 다 "검사는 있는데 그 자리를 지나지 않는" 유형이다. 최종 **15/15 검출, 생존 0**.

### 게이트

```
python   통과 58 | 실패 4 | 건너뜀 3 | 판정없음 1   단언 10,459
         실패 4건은 **전부 이 머신의 auction.db 가 마이그레이션 020 에 머물러 있어서**다
         (아래 참고). 이번 변경과 무관하며, 세션 시작 전부터 같은 상태였다
node     207건 / 203 pass / 0 fail / 4 skip (서버 2개 기동)
         skip 4건은 DOJOONPASS_DATA_ROLE 미선언 게이트(데스크탑3 = 개발/QA)
tsc 0 · eslint 0 · npm run build 성공
변이     15/15 검출 · 생존 0 (PYTHONDONTWRITEBYTECODE=1)
운영 DB  md5 2976ba1200b894778d56ca96395c2abb — 세션 전후 **바이트 동일**
```

### ★ 이 머신의 `auction.db` 는 마이그레이션 **020** 이다 (선행 발견, 이번 변경과 무관)

```
migration_history  20행, 마지막 020_create_auction_image.sql
미적용             021 022 023 024 025 (그리고 이번 026)
```

이 하나가 4개 검사를 붉게 만든다.

```
test_schema_hygiene   "every .sql recorded as applied" + 021 이 지울 중복 인덱스 4개
test_bootstrap        fresh 스키마 vs 이 DB 의 컬럼/인덱스 드리프트
                      (building_area/land_area=025, court_code=024, favorite_notes=026)
test_auction_identity 러너 선행 검사에서 "마이그레이션을 먼저 돌리라"며 중단
test_pipeline_integrity  이쪽은 별개다 - sido/sigungu 지역값 드리프트(데이터)
```

**사본에서는 021~026 이 전부 깨끗하게 적용된다**(실측: 적용 26건 /
`integrity_check ok` / `foreign_key_check` 0건 / `auction_item` 1,876행 보존).
적용 자체는 **승인 영역이라 SKIP** 했다.


## 2026-08-28 Sprint 271 — 공개 엔드포인트가 임차인 실명을 내보내고 있었다 (BUGS #254)

### 어떻게 찾았나

API 인증 표면을 전수로 다시 그렸다(라우트 49개, 메서드별 의존성 대조). 인가 자체는
`test_api_regression.test_authz_coverage` 가 `include_in_schema=False` 로 검사가
줄어드는 것까지 막고 있어 **견고했다.** 문제는 인가가 아니라 **공개 경로가 무엇을
내보내는가** 였다.

```
GET /api/v1/item/{item_id}      비인증 공개 (PUBLIC_ENDPOINTS 등록)

본체 auction_item     필드를 하나씩 적어 내보낸다   <- 화이트리스트
곁딸린 3개 테이블      dict(row) 로 행 전체         <- 여기
```
...
tenant_rights 519행   tenant_name 240행이 실명 / occupied_area 475행이 전체 주소
                      deposit 125 / monthly_rent 33 / move_in_date 211 / fixed_date 81
```

이름 + 주소 + 보증금 + 전입일이 한 묶음으로, 로그인 없이, id 만 바꾸며 수집된다.

### 감사 문서 두 곳이 정반대로 적고 있었다
...
검사가 없어서 **틀린 채로 남았고**, 그 문장을 읽은 다음 감사들은 이 표면을 다시 보지
않았다. 두 줄 다 정정했다.

### 고친 것 — 응답은 한 글자도 바뀌지 않는다

`api/v1/item.py` 에 `_TENANT_FIELDS`(12) / `_CASE_FIELDS`(9) / `_RIGHTS_FIELDS`(21)
와 `_project()`. **목록은 지금 나가고 있는 컬럼 그대로**다(집합·순서 동일 실측, 실서버
응답 키 재확인). 좁히지 않은 이유는 그것이 API 계약 축소라 소비자를 먼저 옮겨야 해서다.
여기서 막는 것은 **앞으로 늘어나는 것** — 마이그레이션이 컬럼을 추가해도 목록에 적기
전에는 공개되지 않는다.

컬럼 **순서로 판정하지 않는다**: migration 024 가 `auction_case` 순서를 바꾼다(집합은
동일). 순서로 보면 승인된 마이그레이션이 적용되는 날 엉뚱하게 붉어진다.

### 같은 부류 전수 확인

```
api/v1/favorite_import.py   _ITEM_COLUMNS + _candidate_view() 로 이미 두 번 좁힌다
api/v1/admin.py:744         명시 컬럼(user_id + 집계)
api/v1/admin.py:784         SELECT * -> dict(r). **관리자 전용**이라 위험도가 다르다
```

공개 경로의 구멍은 하나뿐이었고 닫혔다.

### 신설 `test_public_endpoint_exposure.py` (단언 31)

```
1  화이트리스트 == 실제 테이블 컬럼      새 컬럼이 생기면 이름을 대며 실패한다
2  dict(row) 로 되돌아가지 않는다        1번이 무력해지는 것을 막는다
3  개인정보가 공개로 나간다는 사실       "없음" 주장이 조용히 돌아오지 못하게 한다
```

변이 **3/3 검출, 생존 0**. 3번은 만들자마자 실제로 붉게 떴다 — 그 두 문서 줄을 잡은 것이
이 검사의 첫 일이었다.

### 세션 중 스스로 만든 회귀 하나 (즉시 제거)

문서를 `newline="
"` 로 저장해 `docs/CHANGELOG.md`(HEAD 가 CRLF)를 **통째로 LF 로
뒤집었다.** `test_schema_hygiene` 이 바로 잡았다("CRLF 로 커밋된 파일을 LF 로 다시 쓴
적 없다"). CHANGELOG/BUGS 를 원래 줄끝으로 되돌려 해소.

### 남긴 결정 — 마스킹은 승인 영역

임차인 성명을 `김○○` 로 가릴지는 제품·법무 판단이라 정하지 않았다. 성명은 **대항력
판단의 근거**라 가리면 권리분석이 약해진다. 절충안(로그인 사용자에게만 실명)은 공개
엔드포인트의 인가 정책 변경이라 승인 없이 하지 않는다. `docs/BUGS.md` #254 P0 등록.

### 게이트

```
python   통과 59 | 실패 4 | 건너뜀 3 | 판정없음 1   단언 10,486  (기준선 58/4/3/1, 10,455)
         실패 4건은 세션 전부터 같은 상태 — 이 머신의 auction.db 가 마이그레이션 020
node     207건 / 203 pass / 0 fail / 4 skip (서버 2개 기동)
tsc 0 · eslint 0 · npm run build 성공
성능     item 상세 평균 4.82ms (p95 5.99) · search 4.50ms
         _project 비용 20행당 +4.1us = 4.8ms 의 0.09%, 측정 불가 수준
운영 DB  md5 2976ba1200b894778d56ca96395c2abb — 세션 전후 **바이트 동일**
```


## 2026-08-28 Sprint 271 — 큰글씨 토글 UI (MD 미완 기능 구현)

### 왜 이것이었나

MD 전수에서 **미구현 사용자 기능**을 찾았다. 복사/내보내기는 이미 있었고
(`src/app/favorites/ExportButtons.tsx` — 클립보드 + CSV), 검색·필터·정렬·즐겨찾기·
마이페이지도 있었다. 체크되지 않은 채 남은 것은 하나였다.
...
기반은 끝나 있었다 — Tailwind v4 크기 토큰이 rem 이고 임의 px 글자가 0곳이라
**루트 글꼴 하나만 키우면 화면의 모든 글자가 따라 커진다.** 없던 것은 사용자가
그것을 켤 수단뿐이었다. 실측으로 그 전제를 다시 확인한 뒤 구현했다.

### 구현 — 정본 하나

```
src/lib/textSize.ts        단계·배율·저장키·구독·부트스크립트 생성  <- 정본
src/components/TextSizeToggle.tsx   컨트롤 (값을 하나도 직접 적지 않는다)
src/app/layout.tsx         첫 페인트 전 반영 (FOUC 방지)
src/components/SiteHeader.tsx       8개 화면 공용 헤더에 연결
```

배율을 CSS 나 부트 스크립트에 따로 적지 않는다 — 부트 스크립트 **문자열 자체를
같은 상수에서 만들어 낸다.** 두 곳에 적으면 "새로고침 직후 크기"와 "토글이 말하는
크기"가 갈리는 날이 온다.

`useEffect` + `setState` 대신 `useSyncExternalStore` 를 쓴다. 글자 크기는 React
바깥(루트 엘리먼트 + localStorage)에 사는 값이고, 이 저장소의 lint 규칙
(`react-hooks/set-state-in-effect`)도 그 패턴을 막는다.

### 브라우저 실측 (next dev + 실제 Chrome)

```
root      16px -> 20px          본문   12px -> 15px (상세는 15/17.5px)
저장       localStorage kokchal.textSize = xlarge
새로고침    유지됨 (data-text-size=xlarge, aria-pressed 이동)
전파       / 와 /properties/53 양쪽에서 동작, 문서 가로 스크롤 없음
```

### 검사가 두 번 나를 잡았다

```
1) text-xs 사용 120 (상한 119)
   -> 사다리를 text-sm/base/lg 로 올렸다. **작은 글자를 벗어나게 해 주는
      컨트롤이 작은 글자를 늘리는 것**은 앞뒤가 맞지 않는다. 탭 타깃도 커졌다.
2) "어느 화면에도 확대 차단이 없다" 위반
   -> 내 **주석**이 금지 토큰을 글자 그대로 담고 있었다(설명하려다 걸렸다).
      검사를 약화하지 않고 문장을 바꿨다.
```

둘 다 상한을 올리거나 검사를 고치는 쪽이 아니라 **내 코드를 고쳐서** 해결했다.

### 회귀

`tests/text-size.test.mjs` 20검사 — 정상/경계값(최대 125% 상한, 최소 100%)/
오류(이상 저장값·저장소 예외)/배선(헤더 연결·부트 스크립트·정본 단일성·접근성).
변이 **5/5 검출, 생존 0**: 토글 제거 / 부트 스크립트 제거 / 두 단계 같은 배율 /
이상 저장값 그대로 적용 / `aria-pressed` 제거.

### 탭 타깃도 44px 로 맞췄다

기존 화면의 미달 컨트롤(44/53)은 **제품 디자인 결정**이라 건드리지 않았다. 다만
새로 만드는 컨트롤의 크기는 고를 수 있고, 하필 **큰글씨가 필요한 사용자를 위한
컨트롤**이 손가락에 안 잡히면 앞뒤가 맞지 않는다.

```
브라우저 실측   보통 44x44px / 최대 글씨 55x55px (함께 커진다)
그룹 폭 134px, 헤더 높이 79px, 가로 넘침 없음
```

### 곁가지 — 검증하다 운영 DB 에 한 줄이 들어갔다

브라우저로 `/properties/53` 을 여는 순간 **최근조회 기록 기능이 정상 동작해**
`recent_items` 에 한 줄이 들어갔다(id 28126, 17:47:10). 제품이 설계대로 한 일이고
이 머신은 개발/QA 역할이다. 크롤 데이터는 무변경을 확인했다 —
auction_item 1876 / auction_case 1384 / document_status 5628 / document_queue 3498.
지우지 않는다(사용자 데이터 삭제는 승인 영역이고, 정당한 기록이다).

```
python   통과 59 | 실패 4 | 건너뜀 3 | 판정없음 1   단언 10,490
         실패 4건은 세션 전부터 같은 상태 — 이 머신의 auction.db 가 마이그레이션 020
node     227건 / 223 pass / 0 fail / 4 skip   (207 -> 227, 이번 20건)
tsc 0 · eslint 0 · npm run build 성공
접근성   test_frontend_accessibility.py 전부 통과
```


## 2026-08-28 Sprint 272 — 미해결로 남아 있던 버그 둘을 실측으로 종결하고, 그 구멍을 검사로 막았다

### MD 전수 재확인 (§1)

지난 스프린트가 큰글씨를 구현한 뒤 남은 사용자 기능을 다시 훑었다. **문서가 미완이라고
적은 것과 코드가 다른 경우가 셋** 있었다.

```
FRONTEND_MASTER_SPEC:385  "subscriptions/me 아직 프론트 미사용"
    -> mypage/page.tsx:144 가 **이미 부르고 있다**. 문서가 낡았다
BUGS #33  "물건종류 69개 중 60개가 항상 0건 — 미해결"
    -> PROPERTY_TYPE_ALIASES 가 생기며 해소됐는데 문서만 미해결
BUGS #17  "/properties 목록 id 불일치 — Spec 결정 대기"
    -> Sprint 51 이 redirect 로 이미 해결. 이 줄만 남아 있었다
```

`#1` 원칙("문서에 적혀 있다고 완료 처리하지 않는다")은 **반대 방향으로도** 성립한다 —
문서가 미해결이라고 적었다고 미해결인 것도 아니다.

### #33 재측정 — 원증상은 사라졌고, 지키는 검사가 없었다
...
남은 0건 항목(묘지/광업권/덤프트럭)은 **그런 물건이 아직 없다**는 사실이지 어휘 문제가
아니다. 그런데 이 계약을 지키는 검사가 하나도 없었다 — 크롤러가 새 표기를 저장하거나
누가 트리 항목을 고치면 그 물건은 **오류도 없이 그냥 0건이 된다.** #33 이 정확히 그렇게
생겼고 발견까지 오래 걸린 이유도 그것이다.

`test_property_type_vocabulary.py` 신설(양방향: 도달성 / 과다매칭 / 죽은 별칭 /
다리가 실제로 무게를 지는가). 변이 **4/4 검출** — 하나는 **별칭표를 비워 #33 당시를
재현**한 것이다.

### #17 재측정 — 규칙은 지켜지고 있는데 지키는 것이 문서뿐이었다

```
Supabase 로 경매 데이터를 직접 조회하는 화면    0건 (src/ 전수)
지역 formatPrice 중복 정의                    0건 (정의는 lib/format.ts 하나)
로그아웃                                      공용 SiteHeader 로 이동
```

`docs/CLAUDE.md` 가 *"auction data always comes from the Python API, never queried
from Supabase directly"* 라고 적어 두었을 뿐이었다. #17 의 피해가 **조용한 오답**
(404 도 없이 엉뚱한 물건이 열림)이었던 만큼, 재발해도 침묵한다.

`tests/supabase-boundary.test.mjs` 신설. Supabase 클라이언트 사용 자체는 허용하고
(`auth.*` 만), `.from('...')` 데이터 질의만 막는다. 변이 **3/3 검출** — 하나는
**`/properties` 가 다시 목록을 그리는 #17 당시 상태**다.

### 고치지 않은 것과 그 이유

```
#69  만료 물건의 문서가 영원히 "수집중"        [제품 결정 필요]
     5,049건. 사용자에게 실제로 보인다(관심물건에 담아 둔 물건이 만료되면 영구히).
     "대상이 아님"을 나타낼 상태값이 없고, 새 상태를 만들지 화면을 다르게 그릴지는
     상태머신·문구 결정이다. 사진 쪽에도 만료 처리가 없어 "이미 결정된 패턴의
     이식"도 아니다. `docs/CLAUDE.md` 의 "모르면 질문한다" 를 따른다.
#254 임차인 실명이 공개 경로로 나간다          [제품·법무 결정 필요]
KG이니시스 실 API / 외부 키 발급               [외부 승인 필요]
마이그레이션 021~026 적용                      [운영 DB 변경 — 승인 필요]
```

### 게이트

```
python   통과 60 | 실패 4 | 건너뜀 3 | 판정없음 1   단언 10,483   (59 -> 60)
         실패 4건은 세션 전과 동일 — 이 머신의 auction.db 가 마이그레이션 020
node     232건 / 228 pass / 0 fail / 4 skip        (227 -> 232)
tsc 0 · eslint 0 · npm run build 성공
운영 DB  md5 d721a36f24a0fdc130a71d8dbe3c4023 — 이번 세션 무변경
```
...
## 2026-09-04 Sprint 296 — Frankenstein 전수 검토: 사본을 정리하고, "지워도 되나"를 기계가 답하게 했다

전문: `docs/SPRINT296_FRANKENSTEIN_COPY_PROVENANCE.md`

### 고친 것 셋 (전부 동작 무변경, 실데이터로 확인)

```
repair_document_status.py   뷰어 파일명표가 **손으로 적은 사본**이었다.
                            "api/v1/documents.py 와 같아야 한다"는 주석만 있고
                            그것을 강제하는 검사가 없었다 — 같은 계열의 다른 사본
                            (doc_paths / backfill)은 전부 대조되고 있었고, 이 파일의
                            **경로 규칙만** Sprint 153 에 정본 호출로 바뀌어 있었다.
                            반쪽만 옮겨진 상태였다.
                            -> DOC_TYPE_FILES = dict(CANONICAL_DOC_FILENAME)
                            실데이터 scan() 결과 종전과 완전 동일(fix=0/already=1278/
                            missing=7632/total=8910)

api/v1/favorites.py         get_item_summary() 가 SELECT * 로 14필드 물건 dict 를
                            조립했는데 유일한 호출부는 **있다/없다만 보고 버렸다.**
                            목록 응답은 그 사이 get_favorites() 안으로 옮겨가 필드가
                            다섯 개 늘었다 — 같은 개념의 모양이 두 벌이 됐고, 이름은
                            **쓰이지 않는 쪽이 더 그럴듯했다.**
                            -> item_exists() 로 좁혔다(favorite_import 의 정본과 같은 모양).
                            응답 키/404 판정/id 범위 가드 전부 그대로.

test_crawl_orchestration.py 게이트에서 이 파일만 3건 실패. **환경 탓으로 넘기지 않았다** —
                            logs/mvp_scraper.lock 소유자 PID 11648 이 **살아 있는 크롤**
                            (03:00 시작)이었고, main() 은 설계대로 건너뛴 것이었다.
                            이 파일은 DB(#251)/CSV(#266)를 이미 격리하면서 **락만**
                            운영 경로였다. -> LOCK_PATH 격리 + 앞뒤 대조로 "이 검사가
                            운영 락을 건드리지 않았다"를 스스로 단언하게 했다.
```

### ★ OneDrive 충돌 사본 — "어느 쪽이 최신인가"를 이제 기계가 답한다

2026-08-27 이래 이 부채는 *"사람이 둘을 읽고 골라야 한다"* 로 미뤄져 있었고, 기존
검사는 **개수만** 셌다. 그런데 그 판단은 읽을 필요가 없다 — 사본의 blob 이 정본의
**어느 과거 커밋과 같으면** git 이 이미 보관 중인 판본이다.

`test_schema_hygiene.py` 의 기존 검사에 **이어 붙였다**(새 검사를 만들지 않는다,
Sprint 276 의 교훈). 실행 결과:

```
추적 중인 소스 사본 7개 → **전부 정본의 과거 판본**  (git rm --cached 로 빼도 잃는 내용 0)
.cov_ 산출물 1개        → 판정 보류(정본 없음)
★ 정본 이력에 없는 내용을 가진 충돌 사본이 없다: []
```

`test_admin_secret_contract` 사본은 정본보다 **줄이 많은데도**(435 vs 418) 과거
판본이었다 — **줄 수로 최신을 판단하면 틀린다**는 반례라 남긴다.

반대 경우(사본 쪽이 새 작업이고 정본이 되돌려진 판본)에는 이제 검사가 먼저 붉어진다.
그때 사본을 지우면 그 작업은 어디에도 없기 때문이다.

### 근거를 남긴 "깨끗한" 축

```
auction ↔ auction_item   2,834행 × 13필드 대조   drift 0     (쓰기 경로가 하나임을 실증)
면적 백엔드 ↔ 프런트      2,834행                 불일치 0 / 폴백 발동 0
identity 중복            (case_id,item_no) 0 / (court_code,case_no) 0 / FK 위반 0
(case_no,item_no) 중복 15건은 전부 **다른 법원** — 법원별 독립 채번이라 정상
frontend 우회            src/ 전수: 맨 fetch 0 / sqlite 0 / 레거시 /properties 는 redirect 뿐
라우트 중복              46개 중 0
```

병합사건 순서 갈라짐은 **여전히 1건**이고 상한 검사가 지키고 있다. 근본 수정은
기존 638행 재키잉이라 승인 영역 — 그대로 BLOCKED 다.
