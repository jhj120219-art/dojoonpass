            ...
            try:
                ...
                if result["success"]:
                    ...
                    if not asset_recorded:
                        ...
                        mark_queue_failed(
                            item["id"], item["retry_count"], item.get("claim_token"),
                            reason="사진 %d장을 받았으나 DB에 한 장도 기록되지 않았다"
                    ...
                    mark_queue_failed(
                        ...
                        reason=(result or {}).get("reason")
                               or "collect_document 가 실패를 돌려줌(세부 사유 없음)")
                ...
                mark_queue_failed(item["id"], item["retry_count"], item.get("claim_token"),
                                  reason="CaseNotReachable: %s" % str(e))
                time_module.sleep(1)
                continue
