...
# ---------------------------------------------------------------------------
# 검증 규칙(PASS/FAIL 판정 근거) 자체는 `test_validation_engine.py`가 담당한다.
#
# 2026-08-13 Sprint 83: 한때 이 파일에도 같은 규칙 검사가 들어 있었다. 두 파일이
# `ValidationEngine.validate()`의 같은 규칙을 각자 검증하면 규칙이 바뀔 때 한쪽만
# 고쳐질 수 있고, 읽는 사람은 어느 쪽이 기준인지 알 수 없다.
#
# 이 파일의 책임은 **로그 기록의 무결성**이다(파일명 그대로). 판정 규칙은 전용 파일에
# 두고, 이 파일에만 있던 두 검사(BUGS #92 도로명 오탐 회귀 / 병합사건 번호 형식)는
# 거기로 옮겼다 — 커버리지 손실 없이 중복만 없앴다.
# ---------------------------------------------------------------------------
def run():
    try:
