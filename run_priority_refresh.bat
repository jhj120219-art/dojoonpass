...
if not defined PY (
    ...
    echo [FAILED] Python 인터프리터를 찾을 수 없습니다 ^(run_priority_refresh.bat^) at %date% %time% >> logs\doc_run.log
    exit /b 1
)

REM ---------------------------------------------------------------------------
REM 실행 결과 기록 (2026-08-11 Sprint 55, BUGS #47)
REM run_daily.bat에만 있던 errorlevel 검사/성공 마커를 여기에도 맞춘다.
REM 실제로 2026-08-02에 이 스크립트가 traceback으로 죽었지만, 그 사실이
REM doc_run.log 안쪽 줄에만 남고 스케줄러 결과에는 드러나지 않았다.
REM ---------------------------------------------------------------------------
