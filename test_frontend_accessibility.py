...
# ★ 2026-08-28 — 마이리스트 가져오기 화면(`/favorites/import`) 신설분만큼 재기준했다.
#
#       text-xs   117 -> 119   상태 배지 2개 (기존 카드의 배지와 **같은 크기**)
#
#   재기준의 근거를 남긴다 - 이 표는 "늘리는 것을 막는" 래칫이므로 올릴 때는 무엇이
#   왜 늘었는지 적어야 한다(Sprint 225 가 세운 규율).
#
#     * 새 화면의 **본문 문장은 전부 `text-sm` + `text-gray-500` 이상**이다.
#       설명·원문·후보 목록·안내 문구 어느 것도 text-xs/gray-400 을 쓰지 않는다.
#     * `text-gray-400` 은 **늘리지 않았다.** 처음에는 placeholder 색을
#       `src/app/login/page.tsx` 의 관례대로 gray-400 으로 뒀다가, 래칫을 올리는
#       대신 `placeholder:text-gray-500` 으로 낮췄다 - 상한도 그대로고 대비도 낫다.
#     * 늘어난 `text-xs` 는 **상태 배지 2개뿐**이다. 검색/관심물건 카드의 배지와 같은
#       크기라 여기서만 다르게 하면 오히려 화면이 어긋난다.
#     * `text-[0.6875rem]` 은 **늘리지 않았다** - 태그 칩을 새 크기로 만들려다
#       기존 `text-xs` 로 되돌렸다(새 글자 크기를 만들지 않는다).
CEILINGS = {
    "text-xs": 119,
    "text-[11px]": 0,
    "text-[10px]": 0,
